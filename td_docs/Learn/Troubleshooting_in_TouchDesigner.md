---
url: https://docs.derivative.ca/Troubleshooting_in_TouchDesigner
category: Learn
title: Troubleshooting_in_TouchDesigner
---
...
**`debug(*objects)`**     The `debug` command prints any number of objects to the textport. It works very much like the Python `print` command, but will print objects' string "representations", which are more detailed for some objects. The `debug` output will also append the script and line number from which `debug` was called.     Example: `debug("Testcomp:", myComp.path, myComp.par.display)`
...
In rare cases there may be undetected memory hardware corruption. See [Diagnosing Memory Corruption Using Global Flags](https://docs.derivative.ca/Diagnosing_Memory_Corruption_Using_Global_Flags "Diagnosing Memory Corruption Using Global Flags").

###  ToeExpand and ToeCollapse Utilities

In rare cases, you may want to disassemble a `.toe` file into something ASCII readable. To do this, use the **[Toeexpand](https://docs.derivative.ca/Toeexpand "Toeexpand")** and **[Toecollapse](https://docs.derivative.ca/Toecollapse "Toecollapse")** programs provided with TouchDesigner in its installation directory. Reasons to use these include fixing something in a file that won't load or searching for a hard-to-find bit of text. This also allows you to use a "diff" program to compare two `.toe` files find all differences.
