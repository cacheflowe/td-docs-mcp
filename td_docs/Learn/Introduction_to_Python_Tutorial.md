...
To start with, create a [Text TOP](../TOPs/Text_TOP.md "Text TOP") using the [OP Create Dialog](https://docs.derivative.ca/OP_Create_Dialog "OP Create Dialog") by selecting _Text_ from the TOP family. If the [Parameter Dialog](../Glossary/Parameter_Dialog.md "Parameter Dialog") is not open, press p keyboard shortcut to open it.
...
You can see the current frame changing now in both the parameter and the TOP display. Breaking down your short expression, there are two parts: `absTime` is TouchDesigner's [absolute time](../Python/AbsTime_Class.md "AbsTime Class") object, the `.` tells Python you want information inside it (called a member), and `frame` is the specific thing you want from `absTime`.
...
For the next example, put down a [Rectangle TOP](../TOPs/Rectangle_TOP.md "Rectangle TOP") next to your Text TOP. The `op` command (op stands for "operator") lets you navigate to other operators and get information about them. In the Text TOP's 'Text' parameter, enter `op('rectangle1').type`. You should now have this:
...
Parameters on operators can be accessed by using the [par](../SOPs/Par_Class.md "Par Class") object. To start, expand the 'Size' parameter on _rectangle1_. You'll notice that there are two separate values, 'sizex' and 'sizey', which are the actual parameter names. Put 'sizey' into [expression mode](../Glossary/Parameter_Mode.md "Parameter Mode") and enter
...
Another common use of parameter expressions is to access [CHOP](../Glossary/CHOP.md "CHOP") channel data. For this example, put down a [Beat CHOP](../CHOPs/Beat_CHOP.md "Beat CHOP"). Rather than typing the expression this time, we are going to use drag/drop to create it. Follow these steps:
...
When working with Python in TouchDesigner it is very helpful to have a [Textport](../Glossary/Textport.md "Textport") open. For quick tasks, you can open a floating textport by pressing **alt-t** , but for more extended Python work like this tutorial, opening a textport pane is recommended. To do this:
...
Before we dive into working with Python in DATs, you may want to set up an external editor if you haven't already. DATs and the textport can be used for editing, but external editors provide more extensive tools including syntax highlighting, code folding, etc. To set this up, see [Editing DAT Text in an External Editor](../Glossary/DAT.md#Editing_DAT_Text_in_an_External_Editor "DAT").
...
Callbacks let you use Python to react to many different kinds of changes in TouchDesigner. Some operators will have callback DATs attached to them when you create them, such as [Timer CHOP](../CHOPs/Timer_CHOP.md "Timer CHOP"), while other callback DATs stand alone as their own operator types, such as [Parameter Execute DAT](../DATs/Parameter_Execute_DAT.md "Parameter Execute DAT").
...
Execute DATs are DAT operators that provide callbacks for events in TouchDesigner. For example, the example above uses a Parameter Execute DAT to react to parameter value changes. Here are the execute DATs and what they react to:
  * **[CHOP Execute](../DATs/CHOP_Execute_DAT.md "CHOP Execute DAT")** - changes in CHOP channels
  * **[DAT Execute](../DATs/DAT_Execute_DAT.md "DAT Execute DAT")** - changes in a DAT table.
  * **[OP Execute](../DATs/OP_Execute_DAT.md "OP Execute DAT")** - general operator changes, such as name, children, wiring, etc.
  * **[Panel Execute](../Glossary/Panel_Execute_DAT.md "Panel Execute DAT")** - interactions with a [Panel Component](../Glossary/Panel_Component.md "Panel Component") (UI).
  * **[Parameter Execute](../DATs/Parameter_Execute_DAT.md "Parameter Execute DAT")** - changes to parameter values and settings
  * **[Execute](../DATs/Execute_DAT.md "Execute DAT")** - system events, such as startup, file save, operator creation, etc.
...
Script OPs are operators that provide callbacks that actually create the data of the operator itself. When you place one of these operators, there will be a callback DAT attached to it automatically. In the DAT, you can edit the `onCook` callback to define the operator's behavior when it cooks. Here is a list of the script OPs:
  * **[Script CHOP](../CHOPs/Script_CHOP.md "Script CHOP")** - use Python to create a CHOP and its channels.
  * **[Script DAT](../DATs/Script_DAT.md "Script DAT")** - use Python to create a DAT table.
  * **[Script SOP](../SOPs/Script_SOP.md "Script SOP")** - use Python to create a SOP using points, curves, meshes etc.
  * **[Script TOP](../TOPs/Script_TOP.md "Script TOP")** - use Python to create a TOP image.

See [OP Snippets](OP_Snippets.md "OP Snippets") for examples of all these.

##  Extensions

[Extensions](../Glossary/Extensions.md "Extensions") are Python classes that allow you to build data and functionality into your custom [components](../Glossary/Component.md "Component"). There is a lot to extensions and we will only brush the surface in the tutorial below. For a more in-depth look at Extensions, look [here](../Glossary/Extensions.md "Extensions"). This simple technique for adding Python attributes and functions is incredibly powerful for giving custom components extended Python functionality.

###  Creating An Extension

To create a default extension, follow these steps:
  1. Create a new [Container COMP](../Glossary/Container_COMP.md "Container COMP")
  2. Use the [RMB Menu](../Glossary/RMB_Menu.md "RMB Menu") on the new component to select 'Customize Component...'
...
For this example, create a [Text TOP](../TOPs/Text_TOP.md "Text TOP") here in the container. Change the Text TOP's 'Text' parameter to expression mode (click the blue square) and enter the following expression:
...
To create **Component Modules** that are accessible anywhere inside a component, you must place them in a specific child of that component, namely `local/modules`. You can create this path if it doesn't already exist by creating a [Base COMP](../COMPs/Base_COMP.md "Base COMP") called _local_ with another Base COMP called _modules_ inside it.
...
To avoid this, use the [ Module On Demand](../Python/MOD_Class.md "MOD Class") or `mod` object.
