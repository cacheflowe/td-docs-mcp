...
The Limit SOP creates geometry from samples fed to it by [CHOPs](../Glossary/CHOP.md "CHOP"). It creates geometry at every point in the sample. Different types of geometry can be created using the Output Type parameter on the **Channels Page**.

[limitSOP_Class](Limit_SOP_Class.md "LimitSOP Class")
...
**Note:** If using a [Copy SOP](Copy_SOP.md "Copy SOP"), turn on the Use Template Point Attributes option in the Copy SOP's **Attributes Page** to allow the geometry to inherit the point attributes.
...
Extra Information for the Limit SOP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
