...
[trimSOP_Class](Trim_SOP_Class.md "TrimSOP Class")
...
**For example:** Use the [Carve SOP](Carve_SOP.md "Carve SOP") to extract four profiles: two in U, and two in V. Pipe that into a Trim SOP and turn this option off. The four profiles will define a region to be trimmed. Notice that the profile end-points do not coincide, and the profiles are not parametrically continuous, nor are they created in the proper order. Despite all this, the Trim SOP is able to figure out the hole.
...
Extra Information for the Trim SOP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
