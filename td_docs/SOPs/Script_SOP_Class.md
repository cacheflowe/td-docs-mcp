---
url: https://docs.derivative.ca/ScriptSOP_Class
category: SOPs
title: ScriptSOP_Class
---

# scriptSOP Class

This class inherits from the [ SOP class](SOP_Class.md "SOP Class"). It references a specific [Script SOP](Script_SOP.md "Script SOP").
...
These methods can be used by including them in the onCook callback of the Script SOP. Using them outside the callback may cause unexpected results.
...
Copy geometry from the specified [SOP](SOP.md "SOP") operator.
  * sop - The SOP to copy geometry from. Geometry currently in this SOP will be removed.

`appendPoint()`→ `Point`:

Append a [point](Point_Class.md "Point Class") to this SOP. The appended point will be returned.

`appendPoly(numVertices, closed=True, addPoints=True)`→ `Poly`:

Append a [poly](../Python/Poly_Class.md "Poly Class") to this SOP. Returns the appended polygon. The returned polygon, is a list of vertices, and if `addPoints=True`, then `polygon[0].point.x` can be set to the x value of the first point of the polygon, for example.
  * numVertices - Specifies the initial number of [vertices](../Python/Vertex_Class.md "Vertex Class").
  * closed - (Keyword, Optional) Specifies whether or not the last [vertex](../Python/Vertex_Class.md "Vertex Class") of the polygon will connect to the first. An open polygon will be drawn as a line.
  * addPoints - (Keyword, Optional) If True, a new [point](Point_Class.md "Point Class") will be attached to each [vertex](../Python/Vertex_Class.md "Vertex Class"), otherwise the [vertex](../Python/Vertex_Class.md "Vertex Class") point references will need to be manually set afterwards. Manually add them when creating [polygons](../Python/Poly_Class.md "Poly Class") with shared [vertices](../Python/Vertex_Class.md "Vertex Class").

`appendBezier(numVertices, closed=False, order=4, addPoints=True)`→ `Bezier`:

Append a [Bezier](../Python/Bezier_Class.md "Bezier Class") to this SOP. Returns the appended Bezier.
  * numVertices - Specifies the initial number of [vertices](../Python/Vertex_Class.md "Vertex Class"). The number of [vertices](../Python/Vertex_Class.md "Vertex Class") **must** correspond to the order (degree-1) and closed/open state of the curve. For closed curves, the number of vertices must be a multiple of the degree. For open curves, it must be one more than a multiple of the degree.
...
```
  ...
  * closed - (Keyword, Optional) Specifies whether or not the last [vertex](../Python/Vertex_Class.md "Vertex Class") of the curve will connect to the first. An open Bezier will be drawn as a line.
  * order - (Keyword, Optional) Specifies the degree of the Bezier. By default it creates cubic (order=4) Beziers.
  * addPoints - (Keyword, Optional) If True, a new [point](Point_Class.md "Point Class") will be attached to each [vertex](../Python/Vertex_Class.md "Vertex Class"), otherwise the [vertex](../Python/Vertex_Class.md "Vertex Class") point references will need to be manually set afterwards. Use this option when creating Beziers with shared vertices.

`appendMesh(numRows, numCols, closedU=False, closedV=False, addPoints=True)`→ `Mesh`:

Append a [mesh](../Python/Mesh_Class.md "Mesh Class") to this SOP. Returns the appended mesh.
  ...
  * addPoints - (Keyword, Optional) If True, a new [point](Point_Class.md "Point Class") will be attached to each [vertex](../Python/Vertex_Class.md "Vertex Class"), otherwise the [vertex](../Python/Vertex_Class.md "Vertex Class") point references will need to be manually set afterwards. Use this option when creating [meshes](../Python/Mesh_Class.md "Mesh Class") with shared [vertices](../Python/Vertex_Class.md "Vertex Class").

`createPointGroup(str)`→ `None`:

Creates a point [group](../Python/Group_Class.md "Group Class") with input string name. Raises an exception if the group already exists.

`createPrimGroup(str)`→ `None`:

Creates a primitive [group](../Python/Group_Class.md "Group Class") with input string name. Raises an exception if the group already exists.

`appendCustomPage(name)`→ `Page`:

Add a new [page](../Python/Page_Class.md "Page Class") of custom parameters. See [Page Class](../Python/Page_Class.md "Page Class") for more details.
...
Remove all custom parameters from the Script SOP. Returns number of destroyed pars.
...
Get or set [Compare Flag](https://docs.derivative.ca/Compare_Flag "Compare Flag").

`template` → `bool` :

Get or set [Template Flag](https://docs.derivative.ca/Template_Flag "Template Flag").

`points` → `td.Points` **(Read Only)** :

The set of [points](../Python/Points_Class.md "Points Class") contained in this SOP.

`prims` → `td.Prims` **(Read Only)** :

The set of [primitives](Prims_Class.md "Prims Class") contained in this SOP.

`numPoints` → `int` **(Read Only)** :

The number of [points](../Python/Points_Class.md "Points Class") contained in this SOP. This value updates independently of time dependent geometry.

`numVertices` → `int` **(Read Only)** :

The number of [vertices](../Python/Vertex_Class.md "Vertex Class") contained in all primitives within this SOP. This value updates independently of time dependent geometry.

`numPrims` → `int` **(Read Only)** :

The number of [primitivies](Prims_Class.md "Prims Class") contained in this SOP. This value updates independently of time dependent geometry.

`pointAttribs` → `td.Attributes` **(Read Only)** :

The set of point [attributes](../Python/Attributes_Class.md "Attributes Class") defined in this SOP.

`primAttribs` → `td.Attributes` **(Read Only)** :

The set of primitive [attributes](../Python/Attributes_Class.md "Attributes Class") defined in this SOP.

`vertexAttribs` → `td.Attributes` **(Read Only)** :

The set of vertex [attributes](../Python/Attributes_Class.md "Attributes Class") defined in this SOP.

`pointGroups` → `dict` **(Read Only)** :

Returns a dictionary of point [groups](../Python/Group_Class.md "Group Class") defined for this SOP.

`primGroups` → `dict` **(Read Only)** :

Returns a dictionary of primitive [groups](../Python/Group_Class.md "Group Class") defined for this SOP.

`center` → `tdu.Position` :

Get or set the barycentric coordinate of this operator's geometry. It is expressed as a [Position](../Python/Position_Class.md "Position Class").

`min` → `tdu.Position` **(Read Only)** :

The minimum coordinates of this operator's geometry along each dimension, expressed as a [Position](../Python/Position_Class.md "Position Class").

`max` → `tdu.Position` **(Read Only)** :

The maximum coordinates of this operator's geometry along each dimension, expressed as [Position](../Python/Position_Class.md "Position Class").

`size` → `tdu.Position` **(Read Only)** :

The size of this operator's geometry along each dimension, expressed as a [Position](../Python/Position_Class.md "Position Class").

## Methods

`bounds(display=False, render=False)`→ `Bounds`:

Returns an object with the bounds, center and size of the SOP's geometry. _Note:_ This method replaces the deprecated computeBounds method. The method works the same, but the keywords now default to false.
  * display - (Keyword, Optional) If set to True, only [SOP operators](SOP_Class.md "SOP Class") whose[ display flag](../Glossary/Display_Flag.md "Display Flag") are turned on are included in the calculation. When False, the states of the display flag is ignored.
  * render - (Keyword, Optional) If set to True, only [SOP operators](SOP_Class.md "SOP Class") whose[ render flag](../Glossary/Render_Flag.md "Render Flag") are turned on are included in the calculation. When False, the state of the render flag is ignored.
...
An intermediate [parameter collection](../Python/ParCollection_Class.md "ParCollection Class") object, from which a specific [parameter](Par_Class.md "Par Class") can be found.
...
A list of all [built-in parameters](Par_Class.md "Par Class").

`customPars` → `List[Par]` **(Read Only)** :

A list of all [custom parameters](Par_Class.md "Par Class").
...
True if the operator is a [SOP](SOP.md "SOP").
...
If the expression already resides in a parameter, use that parameters [evalExpression()](Par_Class.md "Par Class") method instead.
