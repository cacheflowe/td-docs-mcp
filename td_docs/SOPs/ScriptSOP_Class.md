...
This class inherits from the [ SOP class](https://docs.derivative.ca/SOP_Class "SOP Class"). It references a specific [Script SOP](https://docs.derivative.ca/Script_SOP "Script SOP").
...
`copy(sop)`→ `None`:
Copy geometry from the specified [SOP](https://docs.derivative.ca/SOP "SOP") operator.
  * sop - The SOP to copy geometry from. Geometry currently in this SOP will be removed.

`appendPoint()`→ `Point`:
Append a [point](https://docs.derivative.ca/Point_Class "Point Class") to this SOP. The appended point will be returned.

`appendPoly(numVertices, closed=True, addPoints=True)`→ `Poly`:
Append a [poly](https://docs.derivative.ca/Poly_Class "Poly Class") to this SOP. Returns the appended polygon. The returned polygon, is a list of vertices, and if `addPoints=True`, then `polygon[0].point.x` can be set to the x value of the first point of the polygon, for example.
  * numVertices - Specifies the initial number of [vertices](https://docs.derivative.ca/Vertex_Class "Vertex Class").
  * closed - (Keyword, Optional) Specifies whether or not the last [vertex](https://docs.derivative.ca/Vertex_Class "Vertex Class") of the polygon will connect to the first. An open polygon will be drawn as a line.
  * addPoints - (Keyword, Optional) If True, a new [point](https://docs.derivative.ca/Point_Class "Point Class") will be attached to each [vertex](https://docs.derivative.ca/Vertex_Class "Vertex Class"), otherwise the [vertex](https://docs.derivative.ca/Vertex_Class "Vertex Class") point references will need to be manually set afterwards. Manually add them when creating [polygons](https://docs.derivative.ca/Poly_Class "Poly Class") with shared [vertices](https://docs.derivative.ca/Vertex_Class "Vertex Class").

`appendBezier(numVertices, closed=False, order=4, addPoints=True)`→ `Bezier`:
Append a [Bezier](https://docs.derivative.ca/Bezier_Class "Bezier Class") to this SOP. Returns the appended Bezier.
  * numVertices - Specifies the initial number of [vertices](https://docs.derivative.ca/Vertex_Class "Vertex Class"). The number of [vertices](https://docs.derivative.ca/Vertex_Class "Vertex Class") **must** correspond to the order (degree-1) and closed/open state of the curve. For closed curves, the number of vertices must be a multiple of the degree. For open curves, it must be one more than a multiple of the degree.
...
```
  ...
  * closed - (Keyword, Optional) Specifies whether or not the last [vertex](https://docs.derivative.ca/Vertex_Class "Vertex Class") of the curve will connect to the first. An open Bezier will be drawn as a line.
  * order - (Keyword, Optional) Specifies the degree of the Bezier. By default it creates cubic (order=4) Beziers.
  * addPoints - (Keyword, Optional) If True, a new [point](https://docs.derivative.ca/Point_Class "Point Class") will be attached to each [vertex](https://docs.derivative.ca/Vertex_Class "Vertex Class"), otherwise the [vertex](https://docs.derivative.ca/Vertex_Class "Vertex Class") point references will need to be manually set afterwards. Use this option when creating Beziers with shared vertices.

`appendMesh(numRows, numCols, closedU=False, closedV=False, addPoints=True)`→ `Mesh`:
Append a [mesh](https://docs.derivative.ca/Mesh_Class "Mesh Class") to this SOP. Returns the appended mesh.
  ...
  * addPoints - (Keyword, Optional) If True, a new [point](https://docs.derivative.ca/Point_Class "Point Class") will be attached to each [vertex](https://docs.derivative.ca/Vertex_Class "Vertex Class"), otherwise the [vertex](https://docs.derivative.ca/Vertex_Class "Vertex Class") point references will need to be manually set afterwards. Use this option when creating [meshes](https://docs.derivative.ca/Mesh_Class "Mesh Class") with shared [vertices](https://docs.derivative.ca/Vertex_Class "Vertex Class").

`createPointGroup(str)`→ `None`:
Creates a point [group](https://docs.derivative.ca/Group_Class "Group Class") with input string name. Raises an exception if the group already exists.

`createPrimGroup(str)`→ `None`:
Creates a primitive [group](https://docs.derivative.ca/Group_Class "Group Class") with input string name. Raises an exception if the group already exists.

`appendCustomPage(name)`→ `Page`:
Add a new [page](https://docs.derivative.ca/Page_Class "Page Class") of custom parameters. See [Page Class](https://docs.derivative.ca/Page_Class "Page Class") for more details.
...
The set of [points](https://docs.derivative.ca/Points_Class "Points Class") contained in this SOP.

`prims` → `td.Prims` **(Read Only)** :
The set of [primitives](https://docs.derivative.ca/Prims_Class "Prims Class") contained in this SOP.

`numPoints` → `int` **(Read Only)** :
The number of [points](https://docs.derivative.ca/Points_Class "Points Class") contained in this SOP.

`numVertices` → `int` **(Read Only)** :
The number of [vertices](https://docs.derivative.ca/Vertex_Class "Vertex Class") contained in all primitives within this SOP.

`numPrims` → `int` **(Read Only)** :
The number of [primitivies](https://docs.derivative.ca/Prims_Class "Prims Class") contained in this SOP.

`pointAttribs` → `td.Attributes` **(Read Only)** :
The set of point [attributes](https://docs.derivative.ca/Attributes_Class "Attributes Class") defined in this SOP.

`primAttribs` → `td.Attributes` **(Read Only)** :
The set of primitive [attributes](https://docs.derivative.ca/Attributes_Class "Attributes Class") defined in this SOP.

`vertexAttribs` → `td.Attributes` **(Read Only)** :
The set of vertex [attributes](https://docs.derivative.ca/Attributes_Class "Attributes Class") defined in this SOP.

`pointGroups` → `dict` **(Read Only)** :
Returns a dictionary of point [groups](https://docs.derivative.ca/Group_Class "Group Class") defined for this SOP.

`primGroups` → `dict` **(Read Only)** :
Returns a dictionary of primitive [groups](https://docs.derivative.ca/Group_Class "Group Class") defined for this SOP.

`center` → `tdu.Position` :
Get or set the barycentric coordinate of this operator's geometry. It is expressed as a [Position](https://docs.derivative.ca/Position_Class "Position Class").

`min` → `tdu.Position` **(Read Only)** :
The minimum coordinates of this operator's geometry along each dimension, expressed as a [Position](https://docs.derivative.ca/Position_Class "Position Class").

`max` → `tdu.Position` **(Read Only)** :
The maximum coordinates of this operator's geometry along each dimension, expressed as [Position](https://docs.derivative.ca/Position_Class "Position Class").

`size` → `tdu.Position` **(Read Only)** :
The size of this operator's geometry along each dimension, expressed as a [Position](https://docs.derivative.ca/Position_Class "Position Class").
## Methods
...
Returns an object with the bounds, center and size of the SOP's geometry. _Note:_ This method replaces the deprecated computeBounds method. The method works the same, but the keywords now default to false.
  * display - (Keyword, Optional) If set to True, only [SOP operators](https://docs.derivative.ca/SOP_Class "SOP Class") whose[ display flag](https://docs.derivative.ca/Display_Flag "Display Flag") are turned on are included in the calculation. When False, the states of the display flag is ignored.
  * render - (Keyword, Optional) If set to True, only [SOP operators](https://docs.derivative.ca/SOP_Class "SOP Class") whose[ render flag](https://docs.derivative.ca/Render_Flag "Render Flag") are turned on are included in the calculation. When False, the state of the render flag is ignored.
