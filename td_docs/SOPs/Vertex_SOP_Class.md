---
url: https://docs.derivative.ca/VertexSOP_Class
category: SOPs
title: VertexSOP_Class
---

# vertexSOP Class

This class inherits from the [ SOP class](SOP_Class.md "SOP Class"). It references a specific [Vertex SOP](Vertex_SOP.md "Vertex SOP").

## Members

`inputColor` → `Tuple[float, float, float, float]` **(Read Only)** :

The current point or vertex color being evaluated, from the first input, or a default if not present, expressed as a 4-tuple (r, g, b, a).

`inputColor2` → `Tuple[float, float, float, float]` **(Read Only)** :

The current point or vertex color being evaluated, from the second input, or a default if not present, expressed as a 4-tuple (r, g, b, a).

`inputNormal` → `Tuple[float, float, float]` **(Read Only)** :

The current point or vertex normal being evaluated, from the first input, or a default if not present, expressed as a 3-tuple (n1, n2, n3).

`inputNormal2` → `Tuple[float, float, float]` **(Read Only)** :

The current point or vertex normal being evaluated, from the second input, or a default if not present, expressed as a 3-tuple (n1, n2, n3).

`inputTexture` → `Tuple[float, float, float]` **(Read Only)** :

The current point or vertex texture being evaluated, from the first input, or a default if not present, expressed as a 3-tuple (u, v, w).

`inputTexture2` → `Tuple[float, float, float]` **(Read Only)** :

The current point or vertex texture being evaluated, from the second input, or a default if not present, expressed as a 3-tuple (u, v, w).

`inputVertex` → `Vertex` **(Read Only)** :

The current [vertex](../Python/Vertex_Class.md "Vertex Class") being evaluated, from the first input.

`inputVertex2` → `Vertex` **(Read Only)** :

The current [vertex](../Python/Vertex_Class.md "Vertex Class") being evaluated, from the second input.
