---
url: https://docs.derivative.ca/PrimitiveSOP_Class
category: SOPs
title: PrimitiveSOP_Class
---

# primitiveSOP Class

This class inherits from the [ SOP class](SOP_Class.md "SOP Class"). It references a specific [Primitive SOP](Primitive_SOP.md "Primitive SOP").

## Members

`inputColor` → `Tuple[float, float, float, float]` **(Read Only)** :

The current primitive color being evaluated or a default if not present, expressed as a 4-tuple (r, g, b, a).

`inputPrim` → `Prim` **(Read Only)** :

The current [primitive](../Python/Prim_Class.md "Prim Class") being evaluated.
