...
The Width (width) attribute affects the line width in the [Line MAT](../MATs/Line_MAT.md "Line MAT"). The Scale attribute (pscale) affects particle size, and in the [Line MAT](../MATs/Line_MAT.md "Line MAT") it affects the dot size at each point.

**Tip** : For greater flexibility, use the [Script SOP](Script_SOP.md "Script SOP").

[pointSOP_Class](Point_SOP_Class.md "PointSOP Class")
...
If you select 'No Texture' from the menu above, the uv texture coordinate attribute will be removed from the SOP.
  ...
  * Texture `mapv` -
  * Texture `mapw` -
- Width(Line MAT) `dowidth` - ⊞ - Select between keeping the width attribute or adding a new width. This Width (width) attribute is used exclusively with [Line MAT](../MATs/Line_MAT.md "Line MAT") to control line width when the material is rendered.
  * Keep Width `off` -
  * New Width `on` -
...
Extra Information for the Point SOP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
