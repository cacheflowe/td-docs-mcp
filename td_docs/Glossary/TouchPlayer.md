...
##  How TouchPlayer Works
  ...
  * TouchPlayer opens any [file type](../Interoperability/File_Types.md "File Types") supported by TouchDesigner and runs in [Perform Mode](Perform_Mode.md "Perform Mode") only.
  * For [.toe](.toe.md ".toe") files, the perform mode settings are customizable by the author of the TouchDesigner file.
...
Drag and Drop any [compatible file](../Interoperability/File_Types.md "File Types") into this dialog to open it in TouchPlayer.
...
Although TouchPlayer will play any `.toe` or `.tox` file, some guidelines and best practices should be followed.
  ...
  * Be Aware that TouchPlayer Non-Commercial will open confirmation dialogs when it attempts to access network connections, the [Video Device In TOP](../TOPs/Video_Device_In_TOP.md "Video Device In TOP"), or the [Audio Device In CHOP](../CHOPs/Audio_Device_In_CHOP.md "Audio Device In CHOP"). These confirmation dialogs will not open when using TouchPlayer Commercial or Pro.
  * TouchPlayer will open all supported image & movie files, as well as [.toe](.toe.md ".toe") & [.tox](.tox.md ".tox").
  * Although TouchPlayer does not support the network editing interface, you can open a floating parameter window on any operator via scripting, namely the `OP.openParameters()` call. See [OP Class](../Python/OP_Class.md). Or you can embed the parameter dialog in your window with a [Parameter COMP](../COMPs/Parameter_COMP.md "Parameter COMP").
