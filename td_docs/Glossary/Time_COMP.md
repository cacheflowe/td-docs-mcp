...
The Time Component is often used in TouchDesigner to allow a component to have its own timeline / clock, this is called [Component Time](https://docs.derivative.ca/Component_Time "Component Time"). It is useful for holding some parts of your system stationary while others are allowed to play forward. A Time Component's location must be in the `/local` network of a [Component](Component.md "Component") for the Time Component to create a **Component Time**.
...
[timeCOMP_Class](../COMPs/Time_COMP_Class.md "TimeCOMP Class")
