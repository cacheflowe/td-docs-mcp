---
url: https://docs.derivative.ca/Light_COMP
category: Glossary
title: Light_COMP
---
...
See [Geometry Viewer](../Interoperability/Geometry_Viewer.md "Geometry Viewer") to learn about how you can inspect from a light.

[lightCOMP_Class](../COMPs/Light_COMP_Class.md "LightCOMP Class")
...
- Light Color `c` - ⊞ - You can modify the color of a light here by adjusting the red, green, and blue parameters. Alternatively, clicking on the color swatch will open a dialog with HSV and/or RGB sliders allowing interactive color picking with a preview of the selected color.
  ...
  * Green `cg` -
  * Blue `cb` -
- Dimmer `dimmer` - This parameter changes the intensity of the light without affecting its hue. Lights with Dimmer intensity below 0.001 are ignored. This optimization allows lights that are set to 0.0 to not be calculated in a [Render TOP](../TOPs/Render_TOP.md "Render TOP").
...
[![Objects16.gif](https://docs.derivative.ca/images/3/38/Objects16.gif)](https://docs.derivative.ca/File:Objects16.gif)
...
- Projector Map `projmap` - ⊞ - The path to a [TOP](../TOPs/TOP.md "TOP") used for the light's projector map.
...
Enabling shadows will cause this node to render the scene in a depth-only pass to create a shadow map. This map will then be used by [Render TOPs](../TOPs/Render_TOP.md "Render TOP") when rendering to create shadows for objects lit by this light. The shadow map that is created can be obtained using a [Depth TOP](../TOPs/Depth_TOP.md "Depth TOP"), for cases where custom rendering is being done.
...
- Shadow Resolution `shadowresolution` - ⊞ - The resolution of the shadow's texture map used for the calculation.
  * `shadowresolution1` -
  * `shadowresolution2` -
- Custom Shadow Map `shadowmap` - The path to a [TOP](../TOPs/TOP.md "TOP") used for the light's shadow map. See also [Rendering Shadows](https://docs.derivative.ca/Rendering_Shadows "Rendering Shadows").
...
Extra Information for the Light COMP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
