...
The Movie File In TOP loads movies, still images, or a sequence of still images into TOPs. It will read images in `.jpg`, `.gif`, `.tif`, or `.bmp` format. It will read movies in QuickTime's `.mov` format, `.mp4`, `.mpg`, `.mpeg`, `.avi`, `.wmv`, `.dpx`, [Cineform](../Interoperability/GoPro_Cineform.md "GoPro Cineform") and [Hap](../Interoperability/Hap.md "Hap") Q formats (including Hap Q with Alpha).
...
For a complete list, see [File Types](../Interoperability/File_Types.md "File Types").

Examine the state of a Movie File In TOP by attaching an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP") to it (see below on Info CHOP). This will show info like movie length, resolution, the number of images per second (the `sample_rate` channel), and whether there is audio in the file. It also shows dynamic information like movie open status, current frame, readahead frames and queue size, dropped frame count, CPU decode time and GPU upload time.
...
See also the page on Movie File In TOP optimizations [Movie Playback](https://docs.derivative.ca/Movie_Playback "Movie Playback"), plus [Hap](../Interoperability/Hap.md "Hap"), [Movie File Out TOP](../TOPs/Movie_File_Out_TOP.md "Movie File Out TOP"), [Cineform](../Interoperability/GoPro_Cineform.md "GoPro Cineform") and [Video Stream In TOP](../TOPs/Video_Stream_In_TOP.md "Video Stream In TOP").

[moviefileinTOP_Class](../TOPs/Movie_File_In_TOP_Class.md "MoviefileinTOP Class")

## Parameters - Play Page
- File `file` - The path and name of the image or movie file to load. Image and movie formats are those found in [File Types](../Interoperability/File_Types.md "File Types"). You can specify files on the internet using `http://` ...
...
- Loop Crossfade Unit `loopcrossfadeunit` - ⊞ - Select the units for this parameter from Index, Frames, Seconds, and Fraction (percentage).
  ...
  * S `seconds` -
  * % `fraction` -
- Timecode Object/CHOP/DAT `timecodeop` - Set the movie index with a reference to a timecode. Should be a reference to either a CHOP with channels 'hour', 'second', 'minute', 'frame', a DAT with a timecode string in its first cell, or a [Timecode Class](../TOPs/Timecode_Class.md "Timecode Class") object.
...
Extra Information for the Movie File In TOP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
