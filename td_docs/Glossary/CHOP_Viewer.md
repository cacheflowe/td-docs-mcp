...
The path for the CHOP viewer can be set to a particular CHOP or a network. The [Display Flag](https://docs.derivative.ca/Display_Flag "Display Flag") of the CHOP(s) must be turned on to view the channels in the CHOP viewer. When set to a network (ie no specific CHOP), the viewer can display channel from multiple CHOPs. The viewer can be divided into multiple graphs in 3 modes, Graph Per Single Channel, Graph Per CHOP, or Graph Per Channel Name.
