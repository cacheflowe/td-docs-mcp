...
The Geometry Component is a 3D surface or [3D Object](Object.md "Object") that you render in TouchDesigner with a [Render TOP](../TOPs/Render_TOP.md "Render TOP"). [Lights](Light_COMP.md "Light COMP"), [Cameras](Camera_COMP.md "Camera COMP") and other Components affect the scene, but are not visible surfaces.

Each Geometry Component contains a [Network](Network.md "Network") containing [POPs](../POPs/POP.md "POP") (and any other node types), where POPs are the operators that define its 3D shape. The 3D surfaces can be [polygons](Polygon.md "Polygon") (triangles and quadrelaterals), Point primitives used for point clouds and [particles](../POPs/Particle_POP.md "Particle POP"), Line Strips and 2-point Lines. Polygons can be shaded as solid-shaded surfaces or as wireframes.

In the Geometry COMP's network
  * the [POPs](../POPs/POP.md "POP") (Point Operators) whose [Render Flag](Render_Flag.md "Render Flag") is On are rendered by a [Render TOP](../TOPs/Render_TOP.md "Render TOP"). More than one POP can be turned on for rendering in a Geometry component.
  * The POPs whose [Display Flag](Display_Flag.md "Display Flag") is on are seen in the [Geometry Viewer](../Interoperability/Geometry_Viewer.md "Geometry Viewer") of the Geometry COMP and Camera COMPs in the same network.

NOTE: Most often users turn on/off the Render and Display flags together so they are always in the same state.

See [Geometry Viewer](../Interoperability/Geometry_Viewer.md "Geometry Viewer") to learn about how you can inspect the POPs that are turned on for display.

Some POPs, like the [Texture Map POP](../POPs/Texture_Map_POP.md "Texture Map POP") determine how texture images wrap and fit on the surface.

Every Geometry component needs a [Material](../MATs/MAT.md "MAT") (MAT) operator to apply to the surface. This is assigned in the Material parameter of the Geometry component.
...
See [Geometry Viewer](../Interoperability/Geometry_Viewer.md "Geometry Viewer"), [Render Flag](Render_Flag.md "Render Flag") and [Display Flag](Display_Flag.md "Display Flag")

See also [POP Rotations](https://docs.derivative.ca/index.php?title=POP_Rotations&action=edit&redlink=1 "POP Rotations \(page does not exist\)").
...
[geometryCOMP_Class](../COMPs/Geometry_COMP_Class.md "GeometryCOMP Class")
...
Custom attributes allow arbitrary attributes to be assigned to instances, usable in a [GLSL MAT](../MATs/GLSL_MAT.md "GLSL MAT"). They can be accessed using `TDInstanceCustomAttrib0()`, `TDInstanceCustomAttrib1()` etc. For more information refer to [Write a GLSL Material](../Interoperability/Write_a_GLSL_Material.md "Write a GLSL Material"). These attributes will be ignored in other materials such as the [PBR MAT](../MATs/PBR_MAT.md "PBR MAT").
...
The Display parameter page controls the component's [material](https://docs.derivative.ca/index.php?title=Material&action=edit&redlink=1 "Material \(page does not exist\)") and [rendering](Rendering.md "Rendering") settings.
- Material `material` - Selects a [MAT](../MATs/MAT.md "MAT") to apply to the geometry inside.
- Render `render` - Whether the Component's geometry is visible in the [Render TOP](../TOPs/Render_TOP.md "Render TOP"). This parameter works in conjunction (logical AND) with the Component's [Render Flag](Render_Flag.md "Render Flag").
...
Extra Information for the Geometry COMP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
