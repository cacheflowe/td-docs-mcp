...
It is useful to follow the Filter CHOP with a [Trail CHOP](../CHOPs/Trail_CHOP.md "Trail CHOP") and connect the filtered and pre-filtered signal to its inputs.
...
The Filter CHOP can filter both motion and sound, but other CHOPs are more appropriate for filtering sound (see the [Audio Filter CHOP](../CHOPs/Audio_Filter_CHOP.md "Audio Filter CHOP"), [Band EQ CHOP](https://docs.derivative.ca/Band_EQ_CHOP "Band EQ CHOP"), and [Parametric EQ CHOP](https://docs.derivative.ca/Parametric_EQ_CHOP "Parametric EQ CHOP")).

[filterCHOP_Class](../CHOPs/Filter_CHOP_Class.md "FilterCHOP Class")
...
The one Euro Filter is especially useful when a person is in an interaction loop with TouchDeigner and wants quick response: It responds quickly to large changes in value, and it smooths out jitters in the input:
  ...
  * Slope Cutoff Frequency: Avoids high derivative bursts caused by jitter. (The research paper implementation fixes this value to 1Hz)
...
- Ramp Tolerance `ramptolerance` - When using a Ramp Preserve filter, if the input value deviates from the current output ramp value by this much, then the ramp will reset to the new input value. Otherwise the Ramp Preserve will continue climping at the specified 'Ramp Rate'.
