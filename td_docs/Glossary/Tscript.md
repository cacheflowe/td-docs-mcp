---
url: https://docs.derivative.ca/Tscript
category: Glossary
title: Tscript
---
...
# Tscript
  ...
  * [Tscript Commands](https://docs.derivative.ca/Tscript_Commands "Tscript Commands")
  * [Tscript Expressions](https://docs.derivative.ca/Tscript_Expressions "Tscript Expressions")
  * [Python vs Tscript Equivalents](https://docs.derivative.ca/Python_vs_Tscript_Equivalents "Python vs Tscript Equivalents")

##  TouchDesigner Scripting Language
**Tscript** is TouchDesigner's original built in scripting language, now alongside the [Python](https://docs.derivative.ca/Python "Python") language. A side-by-side list of common tasks in both languages can be found [here](https://docs.derivative.ca/Python_vs_Tscript_Equivalents "Python vs Tscript Equivalents").
Tscript is similar to C-Shell in its syntax and structure. TouchDesigner's [Expression](https://docs.derivative.ca/Expression "Expression") language is also used heavily in Tscript. As a result, when learning Tscript, don't focus solely on Tscript itself, but look at [Expressions](https://docs.derivative.ca/Expression "Expression") too.
[Tscript Commands](https://docs.derivative.ca/Tscript_Commands "Tscript Commands") and [Tscript Expressionsis](https://docs.derivative.ca/Tscript_Expressions "Tscript Expressions") a complete reference of Tscript.
...
###  Expansion
Expansion is done in the following order: History substitution, [Macro](https://docs.derivative.ca/Macro "Macro") expansion, [Variable](https://docs.derivative.ca/Variables "Variables") & [Expression](https://docs.derivative.ca/Expression "Expression") expansion.
...
##  Variables
There are many types of [Variables](https://docs.derivative.ca/Variables "Variables") in TouchDesigner. Script variables are variables that are set within a script using the `set` command. They are local to the script and exist only for the duration of the script. When the script terminates, these variables will automatically be unset.
...
Basic Pattern matching is also often used to match channel names. It uses the following kinds of patterns to select existing channels in an input CHOP, used in CHOPs like the [Select CHOP](https://docs.derivative.ca/Select_CHOP "Select CHOP") and the [Math CHOP](https://docs.derivative.ca/Math_CHOP "Math CHOP")'s Scope parameter, where you only want to affect certain channels and leave the rest as-is.
  ...
  * `^` - Do not match
  * `[_alphaset_]`- Match any one of alphabetic characters enclosed in the square brackets. In TouchDesigner, the`[a-g]` format is not currently supported, the characters must be listed as `[abcdefg]`
...
**Note:** There should be no spaces within the ranges, i.e. do `[1,3,0-10:2]` not `[1, 3, 0-10:2]`
###  Examples
`chan2` | Matches a single channel name
---|---
`chan3 tx ty tz` | Matches four channel names, each name is separated by spaces
...
###  Examples
`container1/@lights1` | Matches members of the `lights1` group defined inside `container1`.
---|---
##  Operator Patterns
...
`[0-15:2]` | Every second point from 0 to 15 is included
`[0-15:2:5]` | Take the first 2 numbers, every increment of 5, from 0 to 15, this selects 0,1,5,6,10,11,15
`[*:3]` | From 0 to the maximum index, every multiple of three is included. For example with 10 points, this selects 0,3,6,9
...
`[9-0]` | This selects 9,8,7,6,5,4,3,2,1,0 in that order
---|---
`[9-0:2]` | This selects 9,7,5,3,1 in that order
`[0-10:4,3,7]` | This selects 0,4,8,3,7 in that order
`[15-0:2:5]` | This selects 15,14,10,9,5,4,0 in that order
`^[*:3]` | With a maximum value of 10, this selects 1,2,4,5,7,8,10 in that order
...
`blend* ~ blend[0-9]*` | Matches everything that starts with `blend` except anything with a digit after `blend`, for example it would not match `blend0_light`, `blend2_dark` but would match `blend_light` and `blend_dark`.
##  Legacy Features
  * **Prefer using`|` set operator notation instead**
    * Example: use `tx | nx ` instead of `tx nx`
    * Previously for most nodes space and comma separated patterns implied patterns will be ORed together. This logic is still true for pattern not containing the new set operations for backwards compatibility. The set operator makes it more explicit
  * **Prefer using the updated index pattern syntax that is consistent with basic pattern matching**
    * Example: use `[100-120:2] ~ [0-9]` instead of `100-120:2 !0-9 `
    * Old index matching notation for CHOPs will be a legacy toggle that is now off by default for newly placed nodes. When on the new set operator notation will not be available. It is recommended to instead use the new syntax that is more consistent with existing pattern matching
  * **Prefer using`/path/to/node` instead of `op:/path/to/node`**
    * Example: use `/project1/geo` instead of `op:/project1/geo`
    * Mainstay of old path names
...
Some frequently used commands can be represented with a single word, a [Macro](https://docs.derivative.ca/Macro "Macro").
**Example**
```
...
echo "hello world" > FILE:$TOUCH/test.txt  # new file using Built-in Variable[](https://docs.derivative.ca/Variables "Variables") for the path to your project.
...
##  Examples
**Question:** How would I get all the container level parameters of the master component and reapply them to a [clone](https://docs.derivative.ca/Clone "Clone")?
```
 set master = $arg1
 set clone = $arg2
