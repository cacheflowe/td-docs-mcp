...
# Parameter Reference
A parameter reference can be setup between any two parameters by putting an expression in one parameter that refers to another parameter. This creates a [Link](https://docs.derivative.ca/Link "Link") between the two parameters such that when one changes, the other will change automatically. A Parameter reference is a type of [Link](https://docs.derivative.ca/Link "Link"), known in other software as a "constraint".
