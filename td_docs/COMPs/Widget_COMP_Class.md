---
url: https://docs.derivative.ca/WidgetCOMP_Class
category: COMPs
title: WidgetCOMP_Class
---

# widgetCOMP Class

This class inherits from the [ COMP class](../Python/COMP_Class.md "COMP Class"). It references a specific [Widget COMP](Widget_COMP.md "Widget COMP").
...
Simulate a mouse click on the panel at a specific location.
  ...
  * group - (Keyword, Optional) Specifies an optional panel group name. See the [Button COMP](Button_COMP.md "Button COMP") for a description of group labels.

```
op('widget1').click(0.4, 0.5) # Update U and V
```

`clickChild(childIndex, clickCount=1, force=False, left=False, middle=False, right=False, group=None)`→ `None`:

Simulate a mouse click of a sub-panel within the panel. This can be used to click radio buttons within a single panel.
...
```
op('widget1').clickChild(2) # Click the third child panel inside a panel.
```
