---
url: https://docs.derivative.ca/LightCOMP_Class
category: COMPs
title: LightCOMP_Class
---

# lightCOMP Class

This class inherits from the [ COMP class](../Python/COMP_Class.md "COMP Class"). It references a specific [Light COMP](../Glossary/Light_COMP.md "Light COMP").
...
`projectionInverse(x, y)`→ `tdu.Matrix`:
...
`projection(x, y)`→ `tdu.Matrix`:

Returns the projection [matrix](../Python/Matrix_Class.md "Matrix Class") for the light's view, given the X and Y aspect. In general these would be set to the width and height of your render.
  * x - The horizontal aspect ratio.
  * y - The vertical aspect ratio.
...
`mpcdiRegion(width, height)`→ `dict`:

Returns a dictionary of its transform and projection for use in a MPCDI definition file.
  * width - The horizontal resolution of the renderer.
  * height - The vertical resolution of the renderer.
