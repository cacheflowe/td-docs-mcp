---
url: https://docs.derivative.ca/GeotextCOMP_Class
category: COMPs
title: GeotextCOMP_Class
---

# geotextCOMP Class

This class inherits from the [ COMP class](../Python/COMP_Class.md "COMP Class"). It references a specific [Geo Text COMP](Geo_Text_COMP.md "Geo Text COMP").

## Members

`textHeight` → `float` **(Read Only)** :

The height of the text excluding padding. Only valid in Text mode.

`textWidth` → `float` **(Read Only)** :

The width of the text excluding padding. Only valid in Text mode

## Methods

`evalTextSize(str)`→ `Tuple[float, float]`:

Return a tuple with the width and height of the given string based on the current font settings in the Geo Text COMP.
  * str - The text to be measured. The current text value will be used if this argument is missing.

`layoutText(str)`→ `List`:

Returns a list of [TextLine](../Python/TextLine_Class.md "TextLine Class") objects for every visible character in the given text. The origin member of the TextLine will be the character's 2D layout position including all kerning, wrapping and alignment. This data is often used in a Spec DAT/CHOP for the Geo Text COMP after further transformations/projections are applied to the character position. With this technique, text can be projected onto arbitrary surfaces while still maintaining proper layout positioning.
  * str - The text to be laid out.
