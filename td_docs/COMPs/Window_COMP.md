---
url: https://docs.derivative.ca/Window_COMP
category: COMPs
title: Window_COMP
---
...
The Window Component allows you to create and maintain a separate floating or fixed window displaying the contents of any [Panel](../Glossary/Panel.md "Panel") or any [Node Viewer](../Glossary/Node_Viewer.md "Node Viewer").

Most frequently you are setting up the Window COMP `/perform` in the default TouchDesigner project. `/perform` is the default window for [Perform Mode](../Glossary/Perform_Mode.md "Perform Mode"). In the Parameter dialog of the Window component you adjust its settings such as resolution, centering, and which monitor(s) the window will get displayed on.

You then press F1 to go into [Perform Mode](../Glossary/Perform_Mode.md "Perform Mode") and operate/display the panel standalone.

Press Esc over a window to close it and go back to [Designer Mode](../Glossary/Designer_Mode.md "Designer Mode").
...
Attach an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP") to the Window component - it will show you the window's current location and size, and whether the window is actually open.

See also [Window](../Glossary/Window.md "Window"), [Multiple Monitors](../Glossary/Multiple_Monitors.md "Multiple Monitors").

[windowCOMP_Class](Window_COMP_Class.md "WindowCOMP Class")
...
- DPI Scaling `dpiscaling` - ⊞ - Options for managing DPI scaling on high DPI monitors. To inspect a monitor's DPI scaling setting, you can use the [Monitors DAT](../DATs/Monitors_DAT.md "Monitors DAT") and refer to the `dpi_scale` column.
...
- Window Pixel Format `windowpixelformat` - ⊞ - Controls the pixel format of the window that is created. This affects both the bit depth (8, 10, 16 bit) and the color space the window will given the content as. This settings only works if a working [Color Space](https://docs.derivative.ca/Color_Space "Color Space") is active.
  * SDR 8-Bit fixed `sdr8fixed` - The content will be converted from the working [Color Space](https://docs.derivative.ca/Color_Space "Color Space") to 8-bit fixed SDR (sRGB) when presented to the window.
  * SDR 10-Bit fixed `sdr10fixed` - The content will be converted from the working [Color Space](https://docs.derivative.ca/Color_Space "Color Space") to 10-bit fixed SDR (sRGB) when presented to the window.
...
Extra Information for the Window COMP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
