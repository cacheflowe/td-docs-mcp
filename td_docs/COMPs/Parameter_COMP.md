---
url: https://docs.derivative.ca/Parameter_COMP
category: COMPs
title: Parameter_COMP
---
...
[parameterCOMP_Class](Parameter_COMP_Class.md "ParameterCOMP Class")
...
- Labels `labels` - Include the labels for parameters in the display.
- Separators `separators` - Display the separating lines between sections of parameters.
- Input Editor `inputeditor` - Include the multi-input editor UI. This appears at the end of multi-input parameter dialogs such as the one used in the [Math CHOP](../CHOPs/Math_CHOP.md "Math CHOP") or [Merge SOP](../SOPs/Merge_SOP.md "Merge SOP").
[![InputEditor.png](https://docs.derivative.ca/images/c/c3/InputEditor.png)](https://docs.derivative.ca/File:InputEditor.png)
- Allow Expansion `allowexpand` - Turn On/Off the ability for parameters to be expanded showing the [parameter mode](../Glossary/Parameter.md#Parameter_Modes_and_Evaluation "Parameter") and expression editing field.
- Built-In `builtin` - Include the operator's built-in parameters.
- Custom `custom` - Include the operator's [Custom Parameters](https://docs.derivative.ca/Custom_Parameters "Custom Parameters").
- Combine Scopes `combinescopes` - ⊞ - Specify the how to combine the scope parameters below to make the parameter selection.
  * Any (Or) `any` -
  * All (And) `all` -
- Page Scope `pagescope` - ⊞ - Selects with pages of parameters to include. Use * for [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching") all.
  * * `*` -
  * Base `Base` -
  ...
  * Settings `Settings` -
- Parameter Scope `parscope` - ⊞ - Selects which parameters to include. Use * for [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching") all.
  * * `*` -
...
Extra Information for the Parameter COMP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
