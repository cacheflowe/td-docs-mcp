...
This class inherits from the [ COMP class](https://docs.derivative.ca/COMP_Class "COMP Class"). It references a specific [Geo Text COMP](https://docs.derivative.ca/Geo_Text_COMP "Geo Text COMP").
...
`layoutText(str)`→ `List`:
Returns a list of [TextLine](https://docs.derivative.ca/TextLine_Class "TextLine Class") objects for every visible character in the given text. The origin member of the TextLine will be the character's 2D layout position including all kerning, wrapping and alignment. This data is often used in a Spec DAT/CHOP for the Geo Text COMP after further transformations/projections are applied to the character position. With this technique, text can be projected onto arbitrary surfaces while still maintaining proper layout positioning.
