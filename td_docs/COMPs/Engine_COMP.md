---
url: https://docs.derivative.ca/Engine_COMP
category: COMPs
title: Engine_COMP
---
...
The Engine COMP will run a `.tox file` (component) in a separate process. It uses [TouchEngine](../Interoperability/TouchEngine.md "TouchEngine") to manage these processes and pass data between the loaded component and the main project.

The Engine COMP also allows components loaded in the Engine COMP to open a [Window COMP](../Glossary/Window_COMP.md "Window COMP") (and go into Perform Mode). The Clock Mode must be set Independent (on the Tune page) to enable this - which is required to allow UI events to be serviced.

**Inputs and Outputs**

The chosen component's top-level Inputs and Outputs are exposed as inputs and outputs on the Engine COMP. Only [TOP](../TOPs/TOP.md "TOP"), [CHOP](../Glossary/CHOP.md "CHOP") and [DAT](../Glossary/DAT.md "DAT") inputs and outputs are supported.

**Custom Parameters**

Any [custom parameters](https://docs.derivative.ca/Custom_Parameters "Custom Parameters") on the top-level component are added to the Engine COMP's parameters. Custom parameters which refer to other nodes in the network, such as [TOP](../TOPs/TOP.md "TOP"), [CHOP](../Glossary/CHOP.md "CHOP") and [DAT](../Glossary/DAT.md "DAT") parameters, are not supported - you should use inputs to connect those nodes instead.
...
It is useful to monitor the state and performance of the Engine COMP. By default, an [Info DAT](../DATs/Info_DAT.md "Info DAT") is docked to the Engine COMP and contains file path and process ID. A docked [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP") contains three sets of monitoring channels: The Info Type menu is by default set to TouchEngine Status which gives loading / running / error status of the Engine process. Changing the menu to TouchEngine Perform gives the engine's performance statistics, and changing the menu to Initialize/Start give ready / running state information specifically about your loaded component since it can be paused, timed and re-initialized.
...
Under TouchEngine, components are loaded into an already-running instance. For this reason, an [Execute DAT](../DATs/Execute_DAT.md "Execute DAT")'s `onStart()` and `onExit()` method will never be executed under TouchEngine. The `onCreate()` method will be executed when the component is loaded, and is a good place to do any setup you would normally do in `onStart()`.
...
[engineCOMP_Class](Engine_COMP_Class.md "EngineCOMP Class")
...
- Allow UI `allowui` - To use this feature, the 'Clock' parameter must be set to 'Independant'. When 'Allow UI' is On, components loaded in TouchEngine may open Window COMPs or enter perform mode. Use a parameter on the top-level component or a script to do this.
...
Extra Information for the Engine COMP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
