...
[environmentlightCOMP_Class](Environment_Light_COMP_Class.md "EnvironmentlightCOMP Class")
...
- Environment Map Quality `envlightmapquality` - Controls the number of samples used by the Environment Light which determines the quality of the result. This value is multiplied by the [PBR MATs](../MATs/PBR_MAT.md "PBR MAT") Env Light Quality parameter.
...
- Use Pre-Filter Maps `envlightmapprefilter` - ⊞ - Controls how the environment map is pre-filtered. A pre-filtered environment map is expensive to create, but results in much better rendering quality.
  ...
  * From Pre-Filter TOP `useprefiltertop` - The output of the [PreFilter Map TOP](../TOPs/PreFilter_Map_TOP.md "PreFilter Map TOP") can be used instead, this allows for manually creating pre-filtered maps, including sequences of images for animated maps. The maps from the [PreFilter Map TOP](../TOPs/PreFilter_Map_TOP.md "PreFilter Map TOP") should be saved in a lossless format.
- Pre-Filtered Specular Map `envlightspecmap` - The 'Environment Light Specular Map' output from the [PreFilter Map TOP](../TOPs/PreFilter_Map_TOP.md "PreFilter Map TOP") to use.
...
Extra Information for the Environment Light COMP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
