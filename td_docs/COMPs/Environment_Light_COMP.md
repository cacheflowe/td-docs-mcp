---
url: https://docs.derivative.ca/Environment_Light_COMP
category: COMPs
title: Environment_Light_COMP
---

# Environment Light COMP
## Summary

The Environment Light Component controls the color and intensity of an environmental light in a given scene. This light, unlike the [Light Component](https://docs.derivative.ca/Light_COMP "Light COMP"), has no particular position. It comes from outside all of the objects in the scene and lights them. The orientation of the objects and the Environment Light Map will affect how each side of the objects is lit. However the position of the objects in space will have no effect on how the environment light hits them.
[environmentlightCOMP_Class](https://docs.derivative.ca/EnvironmentlightCOMP_Class "EnvironmentlightCOMP Class")
...
- Environment Map Quality `envlightmapquality` - Controls the number of samples used by the Environment Light which determines the quality of the result. This value is multiplied by the [PBR MATs](https://docs.derivative.ca/PBR_MAT "PBR MAT") Env Light Quality parameter.
...
- Use Pre-Filter Maps `envlightmapprefilter` - ⊞ - Controls how the environment map is pre-filtered. A pre-filtered environment map is expensive to create, but results in much better rendering quality.
  ...
  * From Pre-Filter TOP `useprefiltertop` - The output of the [PreFilter Map TOP](https://docs.derivative.ca/PreFilter_Map_TOP "PreFilter Map TOP") can be used instead, this allows for manually creating pre-filtered maps, including sequences of images for animated maps. The maps from the [PreFilter Map TOP](https://docs.derivative.ca/PreFilter_Map_TOP "PreFilter Map TOP") should be saved in a lossless format.

- Pre-Filtered Specular Map `envlightspecmap` - The 'Environment Light Specular Map' output from the [PreFilter Map TOP](https://docs.derivative.ca/PreFilter_Map_TOP "PreFilter Map TOP") to use.
...
## Info CHOP Channels
Extra Information for the Environment Light COMP can be accessed via an [Info CHOP](https://docs.derivative.ca/Info_CHOP "Info CHOP").
###
