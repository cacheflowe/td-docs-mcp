---
url: https://docs.derivative.ca/Experimental:GlTF_Out_COMP
category: COMPs
title: GlTF_Out_COMP
---
...
The glTF Out COMP exports meshes, animations, blend shapes, textures, and scenes to a glTF 2.0 file (`.gltf` or `.glb`). glTF 1.0 files are not supported.

There are two main glTF file extensions:
  * `.gltf'`: ASCII formatted JSON structure, often with external texture files stored on disk, and an external .bin file for the buffer data.
  * `.glb`: A self-contained binary of the JSON scene structure, textures, and buffer data in a single lightweight compressed file.
...
The glTF Out COMP can export networks internally or networks referenced by its Component Path parameter. Two network traversal modes, Automatic and Manual, determine which OPs within the network are exported to the glTF file.

In Automatic mode glTF COMP traverses the network and will export any valid OPs that have their render flag enabled. Valid OPs are
  * Geometry COMP representing glTF nodes
  * Camera COMP representing glTF cameras
  * Null COMP representing a glTF node that corresponds to a joint in a skin
  * POPs (with any Blend POP, Skin Deform POP and Null POP being treated as special cases) representing meshes and mesh primitives

▸Animation Exporting Walkthrough

The glTF Out COMP supports animation exporting from the glTF In COMP as well as general networks (given they are formated properly). The following is a walkthrough exporting multiple animations from a glTF In COMP loading the `Fox.gltf` sample file provided in the samples folder.

#### Setup Component Reference

[![](https://docs.derivative.ca/images/thumb/8/83/Setup_Component_Reference.png/500px-Setup_Component_Reference.png)](https://docs.derivative.ca/File:Setup_Component_Reference.png)
[](https://docs.derivative.ca/File:Setup_Component_Reference.png "Enlarge")

Setting up Component Reference, the parameters of the glTF out COMP are shown on the right.

Change the "Export Method" parameter on the glTF Out COMP to Component Reference and reference `gltfin1` in Component Path.
  * The glTF Out COMP can also export nodes internally when "Export Method" is set to "Internal Network"
  * The top level component is not read, only nodes inside the component referenced are exported.

#### Setup Default Transformations

[![](https://docs.derivative.ca/images/thumb/8/80/Setting_up_Default_Transforms.png/500px-Setting_up_Default_Transforms.png)](https://docs.derivative.ca/File:Setting_up_Default_Transforms.png)
[](https://docs.derivative.ca/File:Setting_up_Default_Transforms.png "Enlarge")

Setting up default transforms, first setting animations to None:animation, and then referencing the Import Select CHOP in the Default Transforms CHOP parameter in the glTF Out COMP

Change the "Animation" parameter to `None:animation`. Any glTF files with animations will always have the `None:animation` that contains the default transforms of the nodes internally.
  * This can include default values for a Geometry node's translate (`tx ty tz`), rotate (`rx ry rz`), scale (`sx sy sz`), as well as quaternion (`qx qy qz qw`).

#### Setup Animation Channel CHOPs

[![](https://docs.derivative.ca/images/thumb/e/e0/Setup_up_Animation_CHOPs_Part1.png/500px-Setup_up_Animation_CHOPs_Part1.png)](https://docs.derivative.ca/File:Setup_up_Animation_CHOPs_Part1.png)
[](https://docs.derivative.ca/File:Setup_up_Animation_CHOPs_Part1.png "Enlarge")

Setting up Animation CHOPs inside `gltfin1`

Inside of `gltfin1` create three copies of the Import Select CHOP `animation`. For these three new CHOPs, turn off "Use Parent Animation", set "Play Mode" to Output Full Range, and then select each animation in this case Survey, Walk and Run. Reference these new CHOPs (named `animation1`,`animation2`, `animation3`) in glTF Out COMP Animation CHOP parameter as well as their names.
  * The glTF Out COMP can export multiple animations at once, which is why three individual CHOPs need to be created
  * Since the glTF Out COMP exports without loss, non time-sliced CHOPs are necessary
  * The animations can have keyframes channel (those imported with the glTF Out COMP will) which determines which samples will be exported. If no keyframe channel is present then all samples will be considered to be keyframe.

#### Setup Geometry COMPs / POPs

[![](https://docs.derivative.ca/images/thumb/3/3d/Setup_Animation_Chop_Part_2.png/500px-Setup_Animation_Chop_Part_2.png)](https://docs.derivative.ca/File:Setup_Animation_Chop_Part_2.png)
[](https://docs.derivative.ca/File:Setup_Animation_Chop_Part_2.png "Enlarge")

Assign the newly created Animation CHOPs the Animation CHOP parameter with it's corresponding name.

For exporting glTF In COMPs Mesh Parse Mode can be set to Automatic. This will automatically go through the network and determine which nodes / POPs to export. It follows the following rules
  * Geometry nodes with their render flag enabled will be exported as glTF Nodes
  * An exported Geometry node can have a Material which must be PBR MAT
  * Any POP within a Geometry node with their render flag enabled will be exported as a single mesh with primitive types being added as individual mesh primitives
    * Skin Deform POP will export as a Skeleton
    * The Blend POP will export as morph targets
  * Any Null POP with their render flag enabled will have the left most non null POP exported, (i.e. a chain of nulls will export the POP right before the start of the chain)

See also: [glTF](https://docs.derivative.ca/GlTF "GlTF"), [glTF In COMP](GlTF_In_COMP.md "GlTF In COMP")

[gltfoutCOMP_Class](GlTF_Out_COMP_Class.md "GltfoutCOMP Class")

## Parameters - glTF Out Page
- Extension `extension` - ⊞ - Specifies what format to export the glTF 2.0 (`.gltf` or `.glb`) as.
  * gltf `gltf` -
  * glb `glb` -
- glTF File `file` - Specifies the filename for the glTF 2.0 (`.gltf` or `.glb`) file to export.
- Export Method `exportmethod` - ⊞ - Specifies which network to export, either within the glTF Out COMP or a different external COMP.
  * Internal Network `internal` -
  * Component Reference `compref` -
- Export `exp` - When pulsed, will export the glTF file scene to the specififed filename.
- Cameras `cameras` - When enabled, the glTF Out COMP will export any cameras in the file.
- Network Mode `netmode` - ⊞ - Specify what mode glTF Out COMP uses to determine which nodes to export.
  * Automatic `automatic` -
  * Manual `manual` -
