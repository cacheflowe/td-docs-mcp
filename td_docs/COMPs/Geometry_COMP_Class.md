---
url: https://docs.derivative.ca/GeometryCOMP_Class
category: COMPs
title: GeometryCOMP_Class
---

# geometryCOMP Class

This class inherits from the [ COMP class](../Python/COMP_Class.md "COMP Class"). It references a specific [Geometry COMP](../Glossary/Geometry_COMP.md "Geometry COMP").
...
`bounds(display=False, render=False, selected=False, recurse=True)`→ `Tuple[Tuple[float, float, float], Tuple[float, float, float], Tuple[float, float, float], Tuple[float, float, float]]`:

Calculate the bounding box of the child geometry of this component. **Note:** This method is identical to the deprecated _computeBounds_ method except that the display and render keywords now default to False.

Returns a named tuple consisting of (min, max, center, size). Each component in turn is another named tuple with (x, y, z).
  * display - (Keyword, Optional) If set to True, only [SOP](../SOPs/SOP_Class.md "SOP Class") and [POP](../Python/POP_Class.md "POP Class") operators whose[ display flag](../Glossary/Display_Flag.md "Display Flag") are turned on are included in the calculation. Furthermore, if the SOP or POP is contained within a child Geometry Component, that operator must have its display flag turned on as well.
  * render - (Keyword, Optional) If set to True, only [SOP](../SOPs/SOP_Class.md "SOP Class") and [POP](../Python/POP_Class.md "POP Class") operators whose[ render flag](../Glossary/Render_Flag.md "Render Flag") set are included in the calculation. Furthermore, if the SOP or POP is contained within a child Geometry Component, that operator must have its render flag set as well.
  * selected - (Keyword, Optional) If set to True, only child [components](../Python/COMP_Class.md "COMP Class") that are currently selected are included in the calculation.
  * recurse - (Keyword, Optional) If set to True, child components are included in the calculation.

```
a=op('/project1/geo1').bounds()
print(a.min.x)
```
...
Sets the Translate, Rotate, Scale and Pivot parameters on the Xform page from the given [matrix](../Python/Matrix_Class.md "Matrix Class").
...
Sets the Translate, Rotate, Scale and Pivot parameters on the Pre-Xform page from the given [matrix](../Python/Matrix_Class.md "Matrix Class").
  * matrix - A matrix of values.
...
Returns a matrix that is the transform from the called object to the target object. This is, if you transform the called objects current transformation by the returned matrix, it will now be positioned at the target objects positions.
  * target - The target [ObjectCOMP](../Python/ObjectCOMP_Class.md "ObjectCOMP Class"). See also [Matrix Class](../Python/Matrix_Class.md "Matrix Class"), [Position Class](../Python/Position_Class.md "Position Class") and [Vector Class](../Python/Vector_Class.md "Vector Class").
