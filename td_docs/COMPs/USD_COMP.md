---
url: https://docs.derivative.ca/USD_COMP
category: COMPs
title: USD_COMP
---
...
See also: [USD](../Interoperability/USD.md "USD"), [USD In TouchDesigner](https://docs.derivative.ca/USD_In_TouchDesigner "USD In TouchDesigner"), [Import Select CHOP](../CHOPs/Import_Select_CHOP.md "Import Select CHOP"), [Import Select TOP](../TOPs/Import_Select_TOP.md "Import Select TOP"), [Import Select SOP](../SOPs/Import_Select_SOP.md "Import Select SOP"), [FBX COMP](FBX_COMP.md "FBX COMP")

[usdCOMP_Class](USD_COMP_Class.md "UsdCOMP Class")
...
- Import Method `importmethod` - ⊞ - Determines what to do when clicking the 'Import' button below.
  * Full Replacement `full` - Fully rebuilds the network inside the USD COMP and reloads the assets from the USD file.
  * Merge with Existing `merge` - Merges any changes in the USD file with the existing contents of the USD COMP.
  * Reload Assets (Import Selects) `reload` - Assets that are loaded into Import Select operators (geos, textures, animations) are reloaded into the existing USD COMP's network from the USD file.
...
- Import POPs `pops` - When On [Point Operators](../POPs/POP.md "POP") will be used or when Off [Surface Operators](../SOPs/SOP.md "SOP") will be used to load geometry inside the USD COMP.
- Direct to GPU `gpudirect` - A toggle to load the geometry directly to the GPU when using SOPs ie. 'Import POPs' above is Off. This makes the rendering much faster than CPU mode. However, currently the only supported geometries are mesh and point primitives. If a USD scene contains other prim types such as NURBs Patches, an error will be generated for each [Import Select SOP](../SOPs/Import_Select_SOP.md "Import Select SOP") that is not supported.
- Keep Parameter Values `keepparams` - A toggle to keep the parameters of the current network over parameters of the re-imported (via Update parameter) network.
...
Extra Information for the USD COMP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
