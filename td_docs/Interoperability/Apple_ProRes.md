...
TouchDesigner supports decoding and playback of ProRes in the [Movie File In TOP](../TOPs/Movie_File_In_TOP.md "Movie File In TOP"), and encoding of ProRes files via the [Movie File Out TOP](../TOPs/Movie_File_Out_TOP.md "Movie File Out TOP"). These operations are hardware accelerated on [macOS](https://docs.derivative.ca/MacOS "MacOS") (Turn on the Movie File In TOP's 'Hardware Decode' parameter toggle on the Tune page). There is no support for hardware decode/encode on Windows.
