...
SRT sent from the Video Stream Out TOP can include per-frame metadata making it easy to send and receive CHOP/DAT data in sync with video. It can be read with the [Video Stream In TOP](../TOPs/Video_Stream_In_TOP.md "Video Stream In TOP").
...
See also the [Video Stream In TOP](../TOPs/Video_Stream_In_TOP.md "Video Stream In TOP"), [RTMP](https://docs.derivative.ca/RTMP "RTMP"), [RTSP](RTSP.md "RTSP") and [Video Streaming User Guide](../Learn/Video_Streaming_User_Guide.md "Video Streaming User Guide"), [Movie File Out TOP](../TOPs/Movie_File_Out_TOP.md "Movie File Out TOP").
...
[videostreamoutTOP_Class](../TOPs/Video_Stream_Out_TOP_Class.md "VideostreamoutTOP Class")
...
- Mode `mode` - ⊞ - Selects if the mode works as an RTSP server, sends RTMP to a receiever such as a distribution service like YouTube or Twitch, or sends to an SRT destination.
  ...
  * WebRTC `webrtc` - Use a WebRTC peer. More info: [WebRTC](WebRTC.md "WebRTC"), [WebRTC DAT](../DATs/WebRTC_DAT.md "WebRTC DAT").
...
- Include Silent Audio Stream `includesilentaudio` - ⊞ - Some broadcasting services require an audio stream to be included. This will include a silent audio stream along with the video in the event there isn't actual audio being streamed video the CHOP parameter.
  ...
  * On `on` -
  * Off `off` -
- Per-Frame Metadata CHOP/DAT `perframemetadata` - Send metadata from this OP with each frame of the video stream. This data can be recevied from the [Video Stream In TOP](../TOPs/Video_Stream_In_TOP.md "Video Stream In TOP") using an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP") and [Info DAT](../DATs/Info_DAT.md "Info DAT").
- Per-Frame Metadata `perframemetadata` - Send metadata from this OP with each frame of the video stream. This data can be received from the [Video Stream In TOP](../TOPs/Video_Stream_In_TOP.md "Video Stream In TOP") using an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP") and [Info DAT](../DATs/Info_DAT.md "Info DAT").
