...
See also [Projection Mapping](https://docs.derivative.ca/Projection_Mapping "Projection Mapping"), [Palette:kantanMapper](https://docs.derivative.ca/Palette:kantanMapper "Palette:kantanMapper"), [Vioso](https://docs.derivative.ca/Vioso "Vioso"), [Scalable Display TOP](https://docs.derivative.ca/Scalable_Display_TOP "Scalable Display TOP"), [projectorBlend](https://docs.derivative.ca/Palette:projectorBlend "Palette:projectorBlend")
...
camSchnappr is located in the Palette in 'Mapping' folder. To begin, open the [Palette](https://docs.derivative.ca/Palette "Palette") and drag `camSchnappr` from Palette>Mapping onto your network pane.


## Parameters - camSchnappr Page
- Project `Project` - Specify a [Camera COMP](https://docs.derivative.ca/Camera_COMP "Camera COMP") that will be used as the camera to be calibrated. By default the internal Camera COMP called "project" is being used. When dropping a new Camera COMP onto this parameter, a selection of parameters will be created.
- Geo SOP `Geosop` - Assign the [SOP](https://docs.derivative.ca/SOP "SOP") which holds the geometry you are calibrating. _Do not specify a SOP that is a [Material SOP](https://docs.derivative.ca/Material_SOP "Material SOP") or is from a branch with a Material SOP in it as this would break the blending. Instead reference the Texture via the Color Map Parameter._
...
The Autoblend feature uses [Light COMPs](https://docs.derivative.ca/Light_COMP "Light COMP") to retrieve overlapping areas by comapring uvs from the lights projection area. Further it uses the Blend, Gamma and Luminance Parameters to adjust the blend region after the principles outlined in Paul Bourke's Paper: <http://paulbourke.net/miscellaneous/edgeblend/>
- camSchnappr Cameras `Camschnapprcams` - Specify all other camSchnappr [Camera COMPs](https://docs.derivative.ca/Camera_COMP "Camera COMP") in order to calculate the blend-mask for the projector.
