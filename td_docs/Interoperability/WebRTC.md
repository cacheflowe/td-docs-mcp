...
WebRTC in TouchDesigner is supported through the [WebRTC DAT](../DATs/WebRTC_DAT.md "WebRTC DAT"). A WebRTC DAT represents a peer on one end of any number of WebRTC peer-to-peer connections. The approach to the WebRTC DAT is python focused, following the callback-focused nature of the [WebRTC JavaScript library](https://developer.mozilla.org/en-US/docs/Web/API/RTCPeerConnection). Method/callback names are kept as close as possible to their JavaScript counterparts.

WebRTC video and audio input/output is done via the [Video Stream In TOP](../TOPs/Video_Stream_In_TOP.md "Video Stream In TOP"), [Video Stream Out TOP](../TOPs/Video_Stream_Out_TOP.md "Video Stream Out TOP"), [Audio Stream In CHOP](../CHOPs/Audio_Stream_In_CHOP.md "Audio Stream In CHOP"), and [Audio Stream Out CHOP](../CHOPs/Audio_Stream_Out_CHOP.md "Audio Stream Out CHOP") with reference to a [WebRTC DAT](../DATs/WebRTC_DAT.md "WebRTC DAT").

See [WebrtcDAT_Class](../DATs/WebRTC_DAT_Class.md "WebrtcDAT Class").

For users that wish to test, review, or demo the technology, TouchDesigner 2022.21000+ includes [Palette:signalingServer](https://docs.derivative.ca/Palette:signalingServer "Palette:signalingServer"), [Palette:signalingClient](https://docs.derivative.ca/Palette:signalingClient "Palette:signalingClient") and [Palette:webRTC](https://docs.derivative.ca/Palette:webRTC "Palette:webRTC") COMPs in the WebRTC folder of the [Palette](../Learn/Palette.md "Palette").
...
In TouchDesigner the local SDP will be passed through the [onOffer](../DATs/WebRTC_DAT_Class.md#Callbacks) and [onAnswer](../DATs/WebRTC_DAT_Class.md#Callbacks) callbacks after [createOffer](../DATs/WebRTC_DAT_Class.md#createOffer) and [createAnswer](../DATs/WebRTC_DAT_Class.md#createAnswer) are called respectively. It must be set locally on the [WebRTC DAT](../DATs/WebRTC_DAT.md "WebRTC DAT") using [setLocalDescription](../DATs/WebRTC_DAT_Class.md#setLocalDescription), then passed to the signaling server. Once the corresponding remote SDP is received, it should be set with [setRemoteDescription](../DATs/WebRTC_DAT_Class.md#setRemoteDescription).
...
The ICE candidate gathering process is typically executed after an offer or answer is created. Candidates will be passed through to the onIceCandidate callback on the [WebRTC DAT](../DATs/WebRTC_DAT.md "WebRTC DAT") where they should then be sent to the signaling server. Remote ICE candidates can be added using the [addIceCandidate](../DATs/WebRTC_DAT_Class.md#addIceCandidate) method as they are received from the signaling server.
