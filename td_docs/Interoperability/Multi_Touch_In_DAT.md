...
The Multi Touch In DAT is used for receiving messages and events from the Windows standard multi-touch API. It captures all the messages, where each new message changes the table it outputs. When a messages is added to the DAT, any script can be called pointing to the new message. The Multi Touch In DAT can be sent to the [Render Pick DAT](../DATs/Render_Pick_DAT.md "Render Pick DAT"), or used with interactTouch methods in the [Panel Component](../Glossary/Panel_Component.md "Panel Component") and [Web Render TOP](../TOPs/Web_Render_TOP.md "Web Render TOP"). Multi Touch DAT is not supported on macOS.
...
The Raw Events format creates a FIFO-type DAT (see also [FIFO DAT](../DATs/FIFO_DAT.md "FIFO DAT")) which, for each multi-touch event, has a row added to the bottom of the table while at the same time a row at the top is deleted.
...
`contactx`, `contacty` - the width of the contact area. `contactu`, `contactv` - the height of the contact area.
  ...
  * `aux` - user supplied data via the [PanelCOMP_Class](../Python/PanelCOMP_Class.md "PanelCOMP Class") method `interactTouch()`. When the event is triggered by the mouse via the Include Mouse option, `aux` will include the mouse buttons used (`1` for left, `2` for middle, `4` for right, can be tested bitwise).

You can use the attached callback DAT (named `mtouchin1_callbacks`) to react to multi-touch events. This is suitable for 2D interfaces that do not require a [Render Pick DAT](../DATs/Render_Pick_DAT.md "Render Pick DAT").

See the [Palette:multiTouch](https://docs.derivative.ca/Palette:multiTouch "Palette:multiTouch") example in the [Palette](../Learn/Palette.md "Palette") under Tools.

See also the [MultiTouch](../Glossary/MultiTouch.md "MultiTouch") page.

[multitouchinDAT_Class](../DATs/Multi_Touch_In_DAT_Class.md "MultitouchinDAT Class")
...
- Panel `panel` - The [Panel Component](../Glossary/Panel_Component.md "Panel Component") to capture the touch events from.
...
Extra Information for the Multi Touch In DAT can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
