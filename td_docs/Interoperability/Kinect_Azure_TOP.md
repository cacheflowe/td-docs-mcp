...
The TOP can be used to configure the settings of the camera (resolution, frame rate, synchronization, etc) as well to retrieve captured images from either its color or depth cameras. Image data from one camera can be remapped (aligned) to the other camera in order to match color and depth information. Only one Kinect Azure TOP can be connected to a single Kinect camera. To retrieve additional images from the same camera, use a [Kinect Azure Select TOP](../TOPs/Kinect_Azure_Select_TOP.md "Kinect Azure Select TOP").

The Kinect Azure can also extract body tracking information and skeleton positions using the depth camera image. To access this data, use a [Kinect Azure CHOP](../CHOPs/Kinect_Azure_CHOP.md "Kinect Azure CHOP") and set its Kinect TOP parameter to the primary Kinect Azure TOP that is connected to the device. **Note:** While both Microsoft and Orbbec hardware can be used in the same project, only one hardware type can have body tracking enabled at any one time. Body tracking is required for the Player Index image or to obtain skeleton data. If body tracking is not required from the Orbbec, it is recommended to use the native [Orbbec TOP](../TOPs/Orbbec_TOP.md "Orbbec TOP") instead.
...
See also [Kinect Azure CHOP](../CHOPs/Kinect_Azure_CHOP.md "Kinect Azure CHOP"), [Kinect Azure Select TOP](../TOPs/Kinect_Azure_Select_TOP.md "Kinect Azure Select TOP"), [Palette:pointRender](https://docs.derivative.ca/Palette:pointRender "Palette:pointRender"), [Palette:kinectCalibration](https://docs.derivative.ca/Palette:kinectCalibration "Palette:kinectCalibration"), [Palette:kinectRecorder](https://docs.derivative.ca/index.php?title=Palette:kinectRecorder&action=edit&redlink=1 "Palette:kinectRecorder \(page does not exist\)"), [Palette:kinectPointcloud](https://docs.derivative.ca/Palette:kinectPointcloud "Palette:kinectPointcloud").

[kinectazureTOP_Class](../TOPs/Kinect_Azure_TOP_Class.md "KinectazureTOP Class")

## Parameters - Kinect Azure Page
- Active `active` - Enable or disable the camera. **Note** Disabling this TOP will also disable any other operators ([Kinect Azure Select TOP](../TOPs/Kinect_Azure_Select_TOP.md "Kinect Azure Select TOP") or [Kinect Azure CHOP](../CHOPs/Kinect_Azure_CHOP.md "Kinect Azure CHOP")) that rely on it.
- Hardware Type `library` - ⊞ - Choose whether you are using Microsoft Kinect Azure hardware or a Kinect compatible Orbbec camera (Femto Mega, Femto Bolt, etc). Both camera types can be used together in the same project, but only one hardware type can enable body tracking at any one time.
  ...
  * Orrbec Compatible (Femto Mega, Femto Bolt, ...) `orbbec` - Selection this option for a Kinect compatible Orbbec camera e.g. Femto Mega, Femto Bolt, etc. Not all Orbbec cameras are compatible with the Kinect Azure. For general Orbbec support use an [Orbbec TOP](../TOPs/Orbbec_TOP.md "Orbbec TOP").
...
- Sync Image to Body Tracking `bodyimage` - When enabled, the image produced will be delayed so that it corresponds to the most recent data in the body tracking system. The amount of delay may fluctuate based on the power of the processor doing the body tracking and the complexity of the scene. A [Kinect Azure Select TOP](../TOPs/Kinect_Azure_Select_TOP.md "Kinect Azure Select TOP") can be used at the same time to retrieve the real-time image stream.
...
Extra Information for the Kinect Azure TOP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
