...
Alternately, channels can be automatically converted to attributes based on a naming standard. Set the Channels Selection menu to Autoconvert Precise Names. This is the same channel naming standard that [POP to CHOP](../CHOPs/POP_to_CHOP.md "POP to CHOP") produces when POP to CHOP's Name Format menu is set to Precise, which handles attributes that have `()` and `[]`:
  ...
  * [Array Attributes](../Glossary/Array_Attribute.md "Array Attribute") are create with channel names with a leading-following underscore - `MyArray_0_`. For example, channel names A`bc_0_` and `Abc_1_` create an array attribute named `Abc` with 2 float elements.
  * for vector arrays `MyVectorArray_0_1`.
...
See also [POP to CHOP](../CHOPs/POP_to_CHOP.md "POP to CHOP")

[choptoPOP_Class](CHOP_to_POP_Class.md "ChoptoPOP Class")
...
Extra Information for the CHOP to POP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
