...
Headers: If the file contains any additional header data, this can be viewed by attaching an [Info DAT](../DATs/Info_DAT.md "Info DAT"). Header data is stored as key-value pairs with the keys in the first column and the corresponding data in the second column, which can easily be interpreted with python.
...
Some file formats can be read by both the Point File In POP and [File In POP](File_In_POP.md "File In POP"), with the difference being the Point File In POP does not import any geometry data that may be in the file. This is a list of files that both OPs can read.
  * `.ply` file format can contain geometry data
  * `.obj` file format can contain geometry data and vertex data

See also Point File In POP, [glTF Out COMP](https://docs.derivative.ca/GlTF_Out_COMP "GlTF Out COMP"), [File Out POP](File_Out_POP.md "File Out POP"), [Alembic In POP](Alembic_In_POP.md "Alembic In POP"), [Alembic Out POP](https://docs.derivative.ca/Alembic_Out_POP "Alembic Out POP"), [pop File Format](https://docs.derivative.ca/Pop_File_Format "Pop File Format"), [Gaussian Splats](https://docs.derivative.ca/Gaussian_Splats "Gaussian Splats")

[pointfileinPOP_Class](Point_File_In_POP_Class.md "PointfileinPOP Class")
...
Extra Information for the Point File In POP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
