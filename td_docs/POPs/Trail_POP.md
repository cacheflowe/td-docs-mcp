...
If the number of points of the input is the same for all the slices, it adds one new [Dimension](../Glossary/Dimension.md "Dimension") whose size is the number of slices.
...
[trailPOP_Class](Trail_POP_Class.md "TrailPOP Class")
...
- Orientation Attributes `orientationattrs` - ⊞ - Outputs the tangent–normal–binormal frame orientation along the trail.
  * None `none` -
  * Tangent `tangent` -
  * Tangent and Normal `tangentnormal` -
  * Tangent, Normal and Binormal `tangentnormalbinormal` -
...
Extra Information for the Trail POP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
