...
The Particle POP is used for creating and controlling motion of "particles" for particle system simulations. A particle is simply a POP point with special attributes that are created by the Particle POP or added/modified manually by the user in a feedback-type loop, making it easy for customization. POPs are able to handle millions+ of points as they fully run on the GPUThe initial position comes from the input P attribute, parameters allow you to define initial states for mass, drag, and the Map page lets you get these states from the input attributes.

Particles are created with an initial position (`P`) and a velocity (`PartVel`) as well as mass (`PartMass`). (1) Position comes from the input POP's `P` attribute, (2) Velocity, mass, etc come from the Particle POP's parameters, (3) the Map page lets you get these initial states from some of the input attributes. Particles can be emitted based on a constant emission rate or can be emitted based on an input attribute. When emitting based on an input attribute, the Particle POP checks the emitting attribute for each input point and emits that many particles at that position every frame (the emission value is converted to an integer).
...
Any new attributes feed back from the target POP.

The particle system is open and highly customizable. It is based on a feedback loop where the output of the Particle POP is sent to standard POP nodes and the [Force Radial POP](Force_Radial_POP.md "Force Radial POP") node that adds force, constrain points, change states to dying/etc, leading to a Null POP at the end of the feedback chain. That Null POP is used by the Particle POP to retrieve the system's state for the next frame (specified by the Target Feedback Loop POP parameter). The Particle POP looks for the attributes `PartForce` and `PartVel` to compute the positions for the current frame.

The Particle POP is based on the features and behavior of the [Feedback POP](Feedback_POP.md "Feedback POP"). To Initialize, Start, end, pause, speed, pre-roll the particle system, it uses the TouchDesigner "Initialize / Start" standard based on the [Timer CHOP](../CHOPs/Timer_CHOP.md "Timer CHOP"). It shares several of the same parameters, and if you attach an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP") to the Particle POP, you will see the same state channels and timer channels as the Timer CHOP.
...
See also [Feedback POP](Feedback_POP.md "Feedback POP"), [POP](POP.md "POP")

[particlePOP_Class](Particle_POP_Class.md "ParticlePOP Class")
...
- Target Feedback Loop POP `targetpop` - A reference to a POP node downstream in the network. This reference will cause a feedback loop and re-injects the particles next frame
...
- Jitter Births During Frame `jitterbirthtime` - Randomizes the exact spawn time of particles within each frame.
- Jitter Birth Position `jitterbirthpos` - Randomly displaces particle birth positions within a defined radius around the source.
...
- Input Attributes `attrs` - ⊞ - Input attributes to copy to new particles.
  * * `*` -
...
Extra Information for the Particle POP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
