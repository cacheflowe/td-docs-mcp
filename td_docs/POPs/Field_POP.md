...
The Transition Range parameter softens the `Weight` transition from 1 to 0. It is the distance over which the weight goes from 1 to 0. Transition Align determines where the transition from 1 to 0 starts. By default the weight is .5 at the Radius for a sphere field. if Align is -.5 the weight has dropped to 0 at the radius, and if align is .5, the weight is 1 at the radius and drops to 0 outside the radius. Similar for other field shapes. The Transition Type allows for linear, ease-in ease-out, or smooth step transitions from 1 to 0.

**Multiple fields in one Field POP**: A parameter called Specification POP is a pointer to a POP where each point is a specification defining a separate field - one point per field. The attributes of the specification POP are named the same as parameters tokens in the Field POP, and override the parameters for each field. Menus and toggles are represented as integers, starting at 0 for the first menu entry. You need only to create a Point POP with attributes named as the Field POP parameters, and then add a point per field, and set their values. (An attribute `sizex` will override the Size X (`sizex`) parameter.) Alternately you can generate points procedurally with attributes that match the Field POP parameter names. The workflow is the same as multiple strings defined in a specification DAT of the [Geo Text COMP](../COMPs/Geo_Text_COMP.md "Geo Text COMP") and the [Text COMP](../COMPs/Text_COMP.md "Text COMP").
...
**Multiple weights raw or mixed** : When you specify multiple fields, their weights are mixed together into the `Weight` attribute (using a blend mode like Screen of the [Composite TOP](../TOPs/Composite_TOP.md "Composite TOP") - combining multi-fields increase the `Weight` but it never exceeds 1). Alternately the weight for each field can be output raw in a multi-value `Weights` array attribute.
...
[fieldPOP_Class](Field_POP_Class.md "FieldPOP Class")
...
- Transition Align `transitionalign` - With Transition Range > 0, by default the weight is .5 at the Radius. if Align is -.5 the weight has dropped to 0 at the radius, and if align is .5, the weight is 1 at the radius and drops to 0 outside the radius.
...
Extra Information for the Field POP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
