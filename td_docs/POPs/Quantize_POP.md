...
The Quantize POP takes any attribute of its input, quantizes the values to integers or any step size, or it converts values to logical 0 or 1 values, and then outputs the resuting values to the same attribute, any other existing attribute or a new attribute. It operates on either point, vertex or primitive [attributes](../Glossary/Attribute.md "Attribute").
...
[quantizePOP_Class](Quantize_POP_Class.md "QuantizePOP Class")
...
Extra Information for the Quantize POP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
