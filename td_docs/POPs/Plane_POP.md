...
Dimension: When the grid is passed to other POPs it cannot be known for certain how many rows and columns were specified. However the Plane POP (and other generator POPs) output its [Dimensions](../Glossary/Dimension.md "Dimension") as metadata to all POPs connected to it, indicating its organizations of points in the points list. A plane grid has dimensions `_numCols numRows_`, which can be used by built-in attributes like`_Dim[]` or by GLSL code.
...
[planePOP_Class](Plane_POP_Class.md "PlanePOP Class")
...
Extra Information for the Plane POP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
