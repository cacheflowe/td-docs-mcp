...
The Subdivide POP subdivides and smooths a surface. It currently uses the Loop method on triangles [[1]](https://en.wikipedia.org/wiki/Loop_subdivision_surface). It divides each quad into two triangles, and then for each triangle subdivides it into 4 triangles cutting at the midpoint of each edge. The pre-existing and new points are blended with their neighbors to produce a more smooth surface.
...
[subdividePOP_Class](Subdivide_POP_Class.md "SubdividePOP Class")
...
Extra Information for the Subdivide POP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
