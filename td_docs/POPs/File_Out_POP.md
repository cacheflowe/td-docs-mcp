...
When array attributes are written, if an array index is not specified (i.e. `Arr` instead of `Arr[4]`). It will loop through and replace `$i` in the output channel with the array index similar to the [Point File In POP](Point_File_In_POP.md "Point File In POP").

Frames per Second determines the number of files to output per second of the timeline.

See also [File In POP](File_In_POP.md "File In POP"), [Point File In POP](Point_File_In_POP.md "Point File In POP"), [glTF Out COMP](https://docs.derivative.ca/GlTF_Out_COMP "GlTF Out COMP"), [Alembic Out POP](https://docs.derivative.ca/Alembic_Out_POP "Alembic Out POP"), [pop File Format](https://docs.derivative.ca/Pop_File_Format "Pop File Format")

[fileoutPOP_Class](File_Out_POP_Class.md "FileoutPOP Class")
...
- FPS `fps` - The frame rate of the file sequence with respect to the timeline FPS.
...
- Record Pulse `recordpulse` - Instantly write out a file.
...
Extra Information for the Box POP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
