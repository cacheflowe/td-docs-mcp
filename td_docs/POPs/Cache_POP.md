...
The Cache POP receives POP data every frame in its input and holds the most recent frames of data (up to Cache Size) in GPU memory. It outputs any of these held frames, determined by Output Index, which acts like a time-delay of its input. Furthermore, [Cache Select POPs](Cache_Select_POP.md "Cache Select POP") and [Cache Blend POPs](Cache_Blend_POP.md "Cache Blend POP") can access any combination the Cache POP's set of held inputs, allowing for multi-frame blends.
...
You can freeze the set of data held in the Cache POP by turning off the Active toggle parameter. You can clear and hold the cache to zero frames while the Reset parameter is on (the input is simply sent to the output), and you can hit Reset Pulse to clear the cache and start growing the cache again right away. (An [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP") connected to a Cache POP will show the number of frames in the cache.)
...
Note that when frames are dropped, data in the cache may be older that they would be if frames were not dropped. The [Trail POP](Trail_POP.md "Trail POP"), though different in functionality, deals with frame drops more elegantly by filling in data for missing frames, and tagging each timeslice with an `Age` attribute. The Cache POP does not have this built-in.

See also [Cache Select POP](Cache_Select_POP.md "Cache Select POP"), [Cache Blend POP](Cache_Blend_POP.md "Cache Blend POP"), [Trail POP](Trail_POP.md "Trail POP"), [Cache TOP](../TOPs/Cache_TOP.md "Cache TOP"), [Texture 3D TOP](../TOPs/Texture_3D_TOP.md "Texture 3D TOP").

[cachePOP_Class](Cache_POP_Class.md "CachePOP Class")
...
Extra Information for the Cache POP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
