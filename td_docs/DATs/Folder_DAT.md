---
url: https://docs.derivative.ca/Folder_DAT
category: DATs
title: Folder_DAT
---
...
[folderDAT_Class](Folder_DAT_Class.md "FolderDAT Class")

## Parameters - Folder Page
- Active `active` - When off, the DAT outputs a single-row table with only the headings, useful when dormant or when sending the DAT to a [Replicator COMP](../Glossary/Replicator_COMP.md "Replicator COMP").
...
- Folders `folders` - Use [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching") to specify which folders are included. Matches the folder path. Delimiters used are spaces and commas. To match spaces, enclose the entire search term in double quotes.
- Names `names` - Use [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching") to specify which names are included. Delimiters used are spaces and commas. To match spaces, enclose the entire search term in double quotes.
- All Extensions `allextensions` - Includes all file extensions.
- Image Extensions `imageextensions` - Includes image contents that are supported by TouchDesigner. See supported [File Types](../Interoperability/File_Types.md "File Types").
- Movie Extensions `movieextensions` - Includes movie contents that are supported by TouchDesigner. See supported [File Types](../Interoperability/File_Types.md "File Types").
- Audio Extensions `audioextensions` - Includes audio contents that are supported by TouchDesigner. See supported [File Types](../Interoperability/File_Types.md "File Types").
- Extensions `extensions` - Use [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching") to specify which extensions are included. Extensions listed here should not include the period. E.g *txt, not *.txt.
...
Extra Information for the Folder DAT can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
