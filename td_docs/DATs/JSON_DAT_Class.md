---
url: https://docs.derivative.ca/JsonDAT_Class
category: DATs
title: JsonDAT_Class
---
...
This class inherits from the [ DAT class](../Python/DAT_Class.md "DAT Class"). It references a specific [JSON DAT](JSON_DAT.md "JSON DAT").

## Members

`source` → **(Read Only)** :

Returns the JSON-formatted input as a Python object.

`result` → **(Read Only)** :

Returns the first item from `jsonDAT.results`.

`results` → **(Read Only)** :

Returns a list of objects resulting from applying the [JSONPath](https://docs.derivative.ca/JSONPath "JSONPath") filter to the parsed JSON input.

`expr` → **(Read Only)** :

Returns the computed Expression as a Python object.
