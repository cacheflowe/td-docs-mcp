...
# SOP Class
A [SOP](https://docs.derivative.ca/SOP "SOP") describes a reference to a SOP operator, containing [points](https://docs.derivative.ca/Points_Class "Points Class") and [primitives](https://docs.derivative.ca/Prims_Class "Prims Class").
...
Returns an object with the bounds, center and size of the SOP's geometry. _Note:_ This method replaces the deprecated computeBounds method. The method works the same, but the keywords now default to false.
  * display - (Keyword, Optional) If set to True, only SOP operators whose[ display flag](https://docs.derivative.ca/Display_Flag "Display Flag") are turned on are included in the calculation. When False, the states of the display flag is ignored.
  * render - (Keyword, Optional) If set to True, only SOP operators whose[ render flag](https://docs.derivative.ca/Render_Flag "Render Flag") are turned on are included in the calculation. When False, the state of the render flag is ignored.
