---
url: https://docs.derivative.ca/MpcdiDAT_Class
category: DATs
title: MpcdiDAT_Class
---

# mpcdiDAT Class

This class inherits from the [ DAT class](../Python/DAT_Class.md "DAT Class"). It references a specific [MPCDI DAT](MPCDI_DAT.md "MPCDI DAT").
...
`cameraTransform(bufferId=None, regionId=None)`→ `tdu.Matrix`:

Gets the loaded camera transform matrix for the configuration. This should be referenced in the 'Xform Matrix/CHOP/DAT' parameter of the Camera COMP.
  * bufferId - (Keyword, Optional) The Buffer ID of the transform.
  * regionId - (Keyword, Optional) The Region ID of the transform.

```
op('mpcdi1').cameraTransform(bufferId="DomeMaster", regionId="0")
```

`projection(bufferId=None, regionId=None)`→ `tdu.Matrix`:

Gets the loaded projection matrix for the configuration. This should be referenced in the 'Proj Matrix/CHOP/DAT' parameter of the Camera COMP with the 'Projection' set to 'Custom Projection Matrix'.
  * bufferId - (Keyword, Optional) The Buffer ID of the projection.
  * regionId - (Keyword, Optional) The Region ID of the projection.

```
op('mpcdi1').projection(bufferId="DomeMaster", regionId="0")
```
