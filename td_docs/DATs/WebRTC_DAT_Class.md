---
url: https://docs.derivative.ca/WebrtcDAT_Class
category: DATs
title: WebrtcDAT_Class
---

# webrtcDAT Class

This class inherits from the [ DAT class](../Python/DAT_Class.md "DAT Class"). It references a specific [WebRTC DAT](WebRTC_DAT.md "WebRTC DAT").

## Members

`peerConnections` → **(Read Only)** :

Returns a list of all peer connection IDs.

## Methods

`addIceCandidate(connectionId, candidate, lineIndex, sdpMid)`→ `None`:

Add a remote ICE (Interactive Connectivity Establishment) candidate to the connection.

Typically ICE candidates will be received from the remote peer via a signalling server.
  * connectionId - The UUID of the connection.
  * candidate - The candidate description.
  * lineIndex - The zero-based SDP media line index associated with the candidate.
  * sdpMid - The SDP ID of the media associated with the candidate.

`addTrack(connectionId, trackId, type)`→ `bool`:

Adds a local media track to the connection and return true if it was successfully added.

This method should be called before createOffer/createAnswer so the appropriate media property can be added to the local SDP. If addTrack is called when a connection is already established then renegotiation may be needed.
  * connectionId - The UUID of the connection.
  * trackId - The ID of the track to add.
  * type - Either 'video' or 'audio'.

`closeConnection(connectionId)`→ `None`:

Closes the connection.
  * connectionId - The UUID of the connection.

`closeDataChannel(connectionId, channelName)`→ `bool`:

Closes the data channel. Returns true if successful.
  * connectionId - The UUID of the connection.
  * channelName - The name of the channel to close.

`createAnswer(connectionId)`→ `None`:

Creates a local answer for the connection associated with the ID.

If successful this will trigger the onAnswer callback with the local SDP. Typically this will be called when a remote offer is received.
  * connectionId - The UUID of the connection.

`createDataChannel(connectionId, channelName)`→ `bool`:

Creates a bidirectional data channel over the connection. Returns true if successful.

This method should be called before createOffer/createAnswer so the appropriate data-channel property can be added to the local SDP.
  * connectionId - The UUID of the connection.
  * channelName - The name of the channel to create.

`createOffer(connectionId)`→ `None`:

Creates a local offer for the connection associated with the ID.

If successful this will trigger the onOffer callback with the local SDP.
  * connectionId - The UUID of the connection.

`getConnectionState(connectionId)`→ `str`:
* connectionId - (Keyword, Optional) The UUID of the connection.

`getSignalingState(connectionId)`→ `str`:
* connectionId - (Keyword, Optional) The UUID of the connection.

`getIceGatheringState(connectionId)`→ `str`:
* connectionId - (Keyword, Optional) The UUID of the connection.

`getIceConnectionState(connectionId)`→ `str`:
* connectionId - (Keyword, Optional) The UUID of the connection.

`getStats(connectionId=None)`→ `str`:

Fetches the JSON-formated statistics of any or all connections.
  * connectionId - (Keyword, Optional) The UUID of the connection. When specified will return only stats for the specific connection, otherwise returns all.

`getTracks(connectionId, type, peer)`→ `list`:

Returns a list of track IDs.
  * connectionId - The UUID of the connection.
  * type - The track type: either 'video' or 'audio'.
  * peer - Which side of the peer-to-peer connection: either 'remote' or 'local'.

`openConnection()`→ `str`:

Opens the local end of a peer connection and returns the UUID.

`removeTrack(connectionId, trackId, type)`→ `bool`:

Removes a local media track from the connection and return true if it was successfully removed.

If removeTrack is called when a connection is already established then renegotiation may be needed.
  * connectionId - The UUID of the connection.
  * trackId - The ID of the track to remove.
  * type - The track type: either 'video' or 'audio'.

`restartIce(connectionId)`→ `None`:

Restarts the ICE gathering process.
  * connectionId - The UUID of the connection.

`sendBytes(connectionId, channelName, data)`→ `None`:

Send bytes over the data channel.
  * connectionId - The UUID of the connection.
  * channelName - The channel to send over.
  * data - Byte data.

`sendText(connectionId, channelName, data)`→ `None`:

Send text over the data channel.
  * connectionId - The UUID of the connection.
  * channelName - The channel to send over.
  * data - Text (ie. string) data.

`setLocalDescription(connectionId, type, sdp, stereo=False)`→ `None`:

Set the description (ie. SDP) of the local side of the peer-to-peer connection.

The SDP is generated internally by the WebRTC DAT and passed through to the onAnswer and onOffer callbacks. In order to establish a successful connection it must be called in the onOffer or onAnswer callback.
  * connectionId - The UUID of the connection.
  * type - The role of the local peer. Either 'answer' or 'offer'.
  * sdp - The session description specifying properties (media, streams, etc.) of the local end of the peer-to-peer connection.
  * stereo (Keyword, Optional) - Specify whether the local peer supports receiving of stereo audio.

`setRemoteDescription(connectionId, type, sdp)`→ `None`:

Set the description (ie. SDP) of the remote side of the peer-to-peer connection.

Typically the remote peer will provide their SDP through an answer or offer via a signalling server.
  * connectionId - The UUID of the connection.
  * type - The role of the remote peer. Either 'answer' or 'offer'.
  * sdp - The session description specifying properties (media, streams, etc.) of the remote end of the peer-to-peer connection.
...
Number of rows in the DAT table. This value updates independently of time dependent table content.

`numCols` → `int` **(Read Only)** :

Number of columns in the DAT table. This value updates independently of time dependent table content.
...
True if the DAT contains table formatted data. This value updates independently of time dependent table content.

`isText` → `bool` **(Read Only)** :

True if the DAT contains text formatted data. (ie, not table formatted). This value updates independently of time dependent table content.
...
Local dictionary used during python execution of scripts in this DAT. The dictionary attribute is read only, but not its contents. Its contents may be manipulated directly with scripts, or with an [Examine DAT](Examine_DAT.md "Examine DAT").
...
[Run](../Python/Run_Class.md "Run Class") the contents of the DAT as a script, returning a Run object which can be used to optionally modify its execution.
  ...
  * delayRef - (Keyword, Optional) Specifies an optional [operator](../Python/OP_Class.md "OP Class") which is controlled by a different [Time Component](../Glossary/Time_COMP.md "Time COMP"). If your own local timeline is paused, you can point to another timeline to ensure this script will still execute for example. If no delayRef is provided, this DAT will be used.
...
Copy the text or table from the specified [DAT](../Glossary/DAT.md "DAT") operator.
...
The following methods can be used to modify the contents of a table type DAT containing rows and columns. This can be done when the DAT is a basic [Table DAT](../Glossary/Table_DAT.md "Table DAT"), or [Script DAT](Script_DAT.md "Script DAT"). It can also be used to append rows to FIFO-style DATs such as the [Serial DAT](../Interoperability/Serial_DAT.md "Serial DAT").
...
Append a row to the end of the table, or after the specified row name/index. Returns the integer index of the new row.
  * vals - (Optional) If specified, will fill the row with the given values. It should be a list of items that can be expressed as strings. Each item will be copied to one [cell](../Python/Cell_Class.md "Cell Class").
...
[cells](../Python/Cell_Class.md "Cell Class") in a table may be accessed with the `[]` subscript operator.

The NameOrIndex may be an exact string name, or it may be a numeric index value. [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching") is _not_ supported.
...
Find a single [cell](../Python/Cell_Class.md "Cell Class") in the table, or None if none are found.
  * rowNameOrIndex/colNameOrIndex - If a string it specifies a row/column name. If it's numeric it specifies a row/column index. [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching") is supported for strings.
...
Returns a (possibly empty) list of [cells](../Python/Cell_Class.md "Cell Class") that match the given row/column names or indices. See DAT.cell method for similar usage.
...
Returns a list of [cells](../Python/Cell_Class.md "Cell Class") from the first row matching the name/index, or None if nothing is found.
  * nameOrIndexes - Include any number of these. If a string it specifies a row name, if it's numeric it specifies a row index. [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching") is supported.
  * caseSensitive - (Optional) Specifies whether or not case sensitivity is used.
...
Returns a (possibly empty) list of rows (each row being a list of cells). If no arguments are given it returns all rows in the table.
  * nameOrIndexes - (Optional) Include any number of these. If a string it specifies a row name, if it's numeric it specifies a row index. [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching") is supported.
  * caseSensitive - (Optional) Specifies whether or not case sensitivity is used.
  * val - (Optional) If set to true, returns list of cell item strings instead of list of Cell Class items.
...
Returns a list of [cells](../Python/Cell_Class.md "Cell Class") from the first col matching the name/index, or None if nothing is found.
  * nameOrIndexes - Include any number of these. If a string it specifies a column name, if it's numeric it specifies a column index. [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching") is supported.
  * caseSensitive - (Optional) Specifies whether or not case sensitivity is used.
  * val - (Optional) If set to true, returns list of cell item strings instead of list of Cell Class items.
...
Returns a (possibly empty) list of columns (each being a list themselves). If no arguments are given then all columns in the table are returned.
  * nameOrIndexes - (Optional) Include any number of these. If a string it specifies a column name, if it's numeric it specifies a column index. [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching") is supported.
  * caseSensitive - (Optional) Specifies whether or not case sensitivity is used.
  * val - (Optional) If set to true, returns list of cell item strings instead of list of Cell Class items.
...
[Storage](../Glossary/Storage.md "Storage") is dictionary associated with this operator. Values stored in this dictionary are persistent, and saved with the operator. The dictionary attribute is read only, but not its contents. Its contents may be manipulated directly with methods such as OP.fetch() or OP.store() described below, or examined with an [Examine DAT](Examine_DAT.md "Examine DAT").

`tags` → `set` :

Get or set a set of user defined strings. [Tags](../Glossary/Tag.md "Tag") can be searched using OP.findChildren() and the [OP Find DAT](OP_Find_DAT.md "OP Find DAT").
...
See the [Examine DAT](Examine_DAT.md "Examine DAT") for procedurally viewing the contents of storage.
