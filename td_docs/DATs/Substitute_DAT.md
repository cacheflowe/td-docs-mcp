...
[substituteDAT_Class](Substitute_DAT_Class.md "SubstituteDAT Class")
...
## Operator Inputs
  ...
  * Input 1:  - _(optional)_ A [Table DAT](../Glossary/Table_DAT.md "Table DAT") with a `before` and `after` column containing strings to sustitute in the first input. This doesn't have to be the whole cell content but can be any partial string as well.

## Info CHOP Channels

Extra Information for the Substitute DAT can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
