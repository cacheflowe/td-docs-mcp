...
[datexecuteDAT_Class](DAT_Execute_DAT_Class.md "DatexecuteDAT Class")
...
- Execute `execute` - ⊞ - Determines if the methods are executed at the start of the frame or end of the frame.
  * Start of Frame `start` - The method will be called from the start of the frame. If a row changes 4 times during a frame then the method will run 4 times as well (useful for [Multi Touch In DAT](../Interoperability/Multi_Touch_In_DAT.md "Multi Touch In DAT") events for example).
...
Extra Information for the DAT Execute DAT can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
