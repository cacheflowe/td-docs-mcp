---
url: https://docs.derivative.ca/SocketioDAT_Class
category: DATs
title: SocketioDAT_Class
---

# socketioDAT Class

This class inherits from the [ DAT class](../Python/DAT_Class.md "DAT Class"). It references a specific [SocketIO DAT](../Interoperability/SocketIO_DAT.md "SocketIO DAT").
...
Emits an event to the socket.io server.
  * event - The name of the server event.
  * data (Keyword, Optional) - The data to send along with the event. Data should be formatted to meet the server's expectations for the event. Eg. If the server expects a dictionary for an event then data should be formatted as a Python dictionary.

```
n.emit('Hello', data='World')
```
