---
url: https://docs.derivative.ca/WebclientDAT_Class
category: DATs
title: WebclientDAT_Class
---

# webclientDAT Class

This class inherits from the [ DAT class](../Python/DAT_Class.md "DAT Class"). It references a specific [Web Client DAT](../Interoperability/Web_Client_DAT.md "Web Client DAT").

## Members

`connections` → `list` **(Read Only)** :

Get a list of active connection identifiers.

## Methods

`request(url, method, header=None, data=None, pars=None, authType=None, username=None, password=None, appKey=None, appSecret=None, oauth1Token=None, oauth1Secret=None, oauth2Token=None, uploadFile=None, timeout=60000)`→ `int`:

Sends an HTTP request of the specified method to the url. Any headers and data can also be specified. Returns the connection identifier that will correspond with the id passed to onResponse callbacks.
  ...
  * header (Keyword, Optional) - A dictionary of headers to add to the request.
  * data (Keyword, Optional) - The data to send in the body of the request.
  * pars (Keyword, Optional) - Query parameters for the request. The parameters will be URL-encoded and appended to the URL.
  * authType (Keyword, Optional) - A case insensitive string for the authentication type. Blank by default (ie. no authentication), but if filled in it must be one of: basic, oauth1, oauth2.
  * username (Keyword, Optional) - Username string equivalent in use to the parameter. Used with Basic authentication.
  * password (Keyword, Optional) - Password string equivalent in use to the parameter. Used with Basic authentication.
  * appKey (Keyword, Optional) - App Key string equivalent in use to the parameter. Used with OAuth1 authentication.
  * appSecret (Keyword, Optional) - App Secret string equivalent in use to the parameter. Used with OAuth1 authentication.
  * oauth1Token (Keyword, Optional) - User OAuth1 Token string equivalent in use to the parameter. Used with OAuth1 authentication.
  * oauth1Secret (Keyword, Optional) - User OAuth1 Secret string equivalent in use to the parameter. Used with OAuth1 authentication.
  * oauth2Token (Keyword, Optional) - OAuth2 Token string equivalent in use to the parameter. Used with OAuth2 authentication.
  * uploadFile (Keyword, Optional) - The file path of the file to be uploaded in the request. Only valid with a PUT request method.
  * timeout (Keyword, Optional) - The timeout in milliseconds for the request.

`closeConnection(id)`→ `None`:

Closes the active connection.
  * id - The connection identifier. Should match the id returned from webclientDAT.request or the one passed into the onResponse callback.
