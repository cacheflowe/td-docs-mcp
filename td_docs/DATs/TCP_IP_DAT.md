---
url: https://docs.derivative.ca/TCP/IP_DAT
category: DATs
title: TCP/IP_DAT
---
...
Send messages using the [tcpipDAT_Class](TCP_IP_DAT_Class.md "TcpipDAT Class"). Handle received messages using the callback DAT attached to the TCP/IP DAT. See [Network Protocols](https://docs.derivative.ca/Network_Protocols "Network Protocols").

This DAT can be used to intercept all the raw information being sent from a Pipe Out CHOP for example. The [Pipe In CHOP](../CHOPs/Pipe_In_CHOP.md "Pipe In CHOP") and [Pipe Out CHOP](../CHOPs/Pipe_Out_CHOP.md "Pipe Out CHOP") also communicate through a TCP/IP connection, though they use a specific syntax.

See also [Peer Class](../Python/Peer_Class.md "Peer Class"), [UDP In DAT](../Interoperability/UDP_In_DAT.md "UDP In DAT"), [UDP Out DAT](UDP_Out_DAT.md "UDP Out DAT").

For Tscript. see Tscript `send` Command.

**NOTE for Windows OS - If experiencing connection issues make sure Windows Firewall is disabled.**

[tcpipDAT_Class](TCP_IP_DAT_Class.md "TcpipDAT Class")
...
- Local Address `localaddress` - Specify an IP address to listen on when in server mode, useful when the system has multiple NICs (Network Interface Card) and you want to select which one to use. When left blank, the TCP/IP DAT will listen on all interfaces.
...
Extra Information for the TCP/IP DAT can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
