---
url: https://docs.derivative.ca/WebserverDAT_Class
category: DATs
title: WebserverDAT_Class
---

# webserverDAT Class

This class inherits from the [ DAT class](../Python/DAT_Class.md "DAT Class"). It references a specific [Web Server DAT](../Interoperability/Web_Server_DAT.md "Web Server DAT").

## Members

`webSocketConnections` → **(Read Only)** :

A string list of all the Web Socket connections in the server.

## Methods

`authenticateBasic(token, userPasswords)`→ `bool`:

Performs Basic Authentication on a token from a Basic Authorization header and verifies it against the userPasswords dictionary. Returns true if the token is accepted, false otherwise.
  * token - The token to authenticate. The token should be from the Basic Authorization header of an HTTP request.
  * userPasswords - A dictionary of the accepted username/passwords.

`webSocketClose(client)`→ `None`:

Closes the Web Socket connection.
  * client - The address of the Web Socket connection to close.

`webSocketSendBinary(client, data)`→ `None`:

Sends binary data to the client's Web Socket.
  * client - The address to send the binary data. Address should be that of an open Web Socket connection.
  * data - The binary data to send.

`webSocketSendText(client, data)`→ `None`:

Sends text data to the client's Web Socket.
  * client - The address to send the text data. Address should be that of an open Web Socket connection.
  * data - The text data to send.

`webSocketSendPing(client, data=None)`→ `None`:

Sends ping data to the client's Web Socket.
  * client - The address to send the ping. Address should be that of an open Web Socket connection.
  * data - (Keyword, Optional) The data to send.

`webSocketSendPong(client, data=None)`→ `None`:

Sends pong data to the client's Web Socket.
  * client - The address to send the pong. Address should be that of an open Web Socket connection.
  * data - (Keyword, Optional) The data to send.
