...
---|---|---
[Text](../Glossary/Text_DAT.md "Text DAT") | A place to edit and hold any text data. |
[Table](../Glossary/Table_DAT.md "Table DAT") | Edit tables of text in rows and columns of "cells". |  [Convert](Convert_DAT.md "Convert DAT")
...
[CHOP to](CHOP_to_DAT.md "CHOP to DAT") | Converts CHOP channels to DATs. |  [SOP to](../Glossary/SOP_to_DAT.md "SOP to DAT")
[CHOP Execute](CHOP_Execute_DAT.md "CHOP Execute DAT") | Runs the DAT as a script when a CHOP changes. |
[Panel Execute](../Glossary/Panel_Execute_DAT.md "Panel Execute DAT") | Runs the DAT as a script when a [Panel](../Glossary/Panel.md "Panel") changes. |
...
[OSC In](OSC_In_DAT.md "OSC In DAT")/[UDP In](../Interoperability/UDP_In_DAT.md "UDP In DAT") | Receive data from other applications via Open Sound Control. |  [OSC Out](OSC_Out_DAT.md "OSC Out DAT")/[UDP Out](UDP_Out_DAT.md "UDP Out DAT")
[Web](Web_DAT.md "Web DAT") | Fetch a page on the internet based on a URL. |  [XML](../Interoperability/XML_DAT.md "XML DAT")
...
[MIDI In](MIDI_In_DAT.md "MIDI In DAT") | Get MIDI controller and button data. |  [Serial](../Interoperability/Serial_DAT.md "Serial DAT")
...
DATs that contain scripts can be triggered by events like mouse clicks and any operation of gadgets in TouchDesigner control panels. DAT scripts can also be triggered by channel changes in CHOPs.       **Script Example**: This will create a button that plays a sound when you click it: Create a Button component (Tab -> COMP -> Button). Go inside it (Enter) and create an Audio Play CHOP (Tab -> CHOP -> Audio Play).     Then create a Panel Execute DAT (Tab -> DAT -> Panel Execute) and make its [Viewer Active](../Glossary/Viewer_Active.md "Viewer Active"). Then click in its viewer and and change the `offToOn()` function to be:
```
...
Click outside the node to end text entry. Go back up (u), make the button a Momentary button via its Button page and Button Type parameter. Then make the button [Viewer Active](../Glossary/Viewer_Active.md "Viewer Active"). Click on the viewer of the button. It should make a Notify sound.
...
The DAT whose name is [Table DAT](../Glossary/Table_DAT.md "Table DAT") is used to define new tables. To manually add, delete rows and columns of tables, press RMB over the table when [Viewer Active](../Glossary/Viewer_Active.md "Viewer Active") is on and select from the menu.
...
You can also edit DATs in the [Textport](../Glossary/Textport.md "Textport") by right-clicking on a DAT and selecting **Edit Contents in Textport**.

Some DAT generators and most DAT filters can not be edited because their data is fed to them from their input. This is indicated in the viewer by a more muted text color compared to those DATs which can be edited. If you need to temporarily edit the contents of one of these DATs you can lock the DAT using the [Lock Flag](../Glossary/Lock_Flag.md "Lock Flag") and then edit it. However, these changes will be lost when the DAT is unlocked and locking it keeps it from [cooking](../Glossary/Cook.md "Cook"), so this is generally only useful for troubleshooting and debugging.
