...
The Text DAT lets you edit free-form, multi-line ASCII text. It is used for scripts, GLSL shaders, notes, XML and other purposes. "Free-form text" is one of the two forms of DATs (the other being tables of rows and columns of cells, each cell containing a text string as in a [Table DAT](../Glossary/Table_DAT.md "Table DAT")).

Text can be typed into the DAT when its [Viewer Active](../Glossary/Viewer_Active.md "Viewer Active") is on, or in an external text editor. The Text DAT can get its data from a file on disk or from a file on the web. Use `http://` when specifying a remote text file.

See also the Execute DATs which are specialized to run their text as a script: [CHOP Execute DAT](CHOP_Execute_DAT.md "CHOP Execute DAT") runs its script when a CHOP channel changes, [DAT Execute DAT](DAT_Execute_DAT.md "DAT Execute DAT") when a DAT changes, [Execute DAT](Execute_DAT.md "Execute DAT") when you start or end your TouchDesigner process or want to run a script every frame, [Panel Execute DAT](../Glossary/Panel_Execute_DAT.md "Panel Execute DAT") when a control panel changes, [Parameter Execute DAT](Parameter_Execute_DAT.md "Parameter Execute DAT") when a parameter of a node changes, and [OP Execute DAT](OP_Execute_DAT.md "OP Execute DAT") when anything else about an operator changes, including creation and deletion of nodes in a component's network.

Use the [Web DAT](Web_DAT.md "Web DAT") to fetch via a URL query.

[textDAT_Class](Text_DAT_Class.md "TextDAT Class")
