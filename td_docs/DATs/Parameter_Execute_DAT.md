---
url: https://docs.derivative.ca/Parameter_Execute_DAT
category: DATs
title: Parameter_Execute_DAT
---
...
The Parameter Execute DAT runs a script when a [parameter](../Glossary/Parameter.md "Parameter") of any operator changes state. There are 4 ways a parameter can trigger the script: if its value, expression, export, or enable state changes.

Parameter Execute DATs are created with [default python method placeholders](Parameter_Execute_DAT_Class.md#Callbacks "ParameterexecuteDAT Class"). Unlike other execute DATs, the Parameter Execute DAT does not cook the node that it is watching, to avoid issues with recursive updates.

[parameterexecuteDAT_Class](Parameter_Execute_DAT_Class.md "ParameterexecuteDAT Class")
...
- On Pulse `onpulse` - The onPulse() method executes when a 'pulse' type parameter is pulsed by clicking on it or via the [Par](../SOPs/Par_Class.md "Par Class").pulse() method.
...
- Mode Change `modechange` - The onModeChange() method executes if the specified [parameter's mode](../Glossary/Parameter_Mode.md "Parameter Mode") changes between the available constant, expression, export or bind mode.
- Custom `custom` - Monitor [Custom Parameters](https://docs.derivative.ca/Custom_Parameters "Custom Parameters").
...
Extra Information for the Parameter Execute DAT can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
