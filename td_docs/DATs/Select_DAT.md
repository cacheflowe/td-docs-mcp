---
url: https://docs.derivative.ca/Select_DAT
category: DATs
title: Select_DAT
---
...
[selectDAT_Class](Select_DAT_Class.md "SelectDAT Class")

## Parameters - Select Page
- DAT `dat` - The [path](../Glossary/Network_Path.md "Network Path") of the DAT being referenced.
...
- Row Select Values `rownames` - Specify actual row names that you want to select. You can use TouchDesigner's pattern matching (see wiki page [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching")), for example `row[1-4]` will select all the rows names `row1` thru `row4`. Some characters like `'` need a `\` in front of it.
...
Extra Information for the Select DAT can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
