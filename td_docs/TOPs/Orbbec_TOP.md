---
url: https://docs.derivative.ca/Orbbec_TOP
category: TOPs
title: Orbbec_TOP
---
...
**Hardware:** This node uses Orbbec SDK v1. Newer camera models may only be supported on Orbbec SDK v2. A list of [compatible camera models](https://github.com/orbbec/OrbbecSDK) can be found on the project's Github page.

The Orbbec TOP can be used to retrieve video streams and IMU data from an [Orbbec camera](../Interoperability/Orbbec.md "Orbbec"). The available video streams depend on the connected camera, but typically include color, depth, IR and point cloud data.

The cameras may be connected either by USB or ethernet depending on the model.

To obtain more than one video stream from a camera, you can use one or more [Orbbec Select TOPs](Orbbec_Select_TOP.md "Orbbec Select TOP") linked to the primary Orbbec TOP. All resolution and configuration is handled by the primary Orbbec TOP.

**Skeleton Data:** Skeleton tracking data can be obtained from compatible Kinect Orbbec cameras (Femto Mega, Femto Bolt, etc) using the [Kinect Azure TOP](Kinect_Azure_TOP.md "Kinect Azure TOP") and [Kinect Azure CHOP](../CHOPs/Kinect_Azure_CHOP.md "Kinect Azure CHOP"). **Note:** If using the Kinect Azure nodes with Orbbec hardware, you should not use the Orbbec TOP in the same project or it could lead to instabilities.
...
**MacOS:** Connecting to Orbbec cameras on Mac OS systems requires root privileges. You can launch TouchDesigner with root privileges by opening a Terminal window and using the sudo command, eg:

`sudo /Applications/TouchDesigner.app/Contents/MacOS/TouchDesigner /Users/Someone/Documents/Project.toe`

An [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP") can be used to view the current value of each supported camera property, and an [Info DAT](../DATs/Info_DAT.md "Info DAT") can be used to see the minimum, maximum, and default value of each supported camera property.

[orbbecTOP_Class](Orbbec_TOP_Class.md "OrbbecTOP Class")

## Parameters - Orbbec Page
- Active `active` - Turn this parameter off to disable communication with the camera. This will also stop any [Orbbec Select TOPs](Orbbec_Select_TOP.md "Orbbec Select TOP") that rely on this TOP.
...
- Device `device` - Use this parameter to select the serial number of the camera you wish to connect to. Only one Orbbec TOP can connect to any camera at one time. Use an [Orbbec Select TOP](Orbbec_Select_TOP.md "Orbbec Select TOP") to obtain additional video streams from the same camera. Both USB and ethernet connected cameras are discoverable by this menu. Cameras connected through this menu can automatically reconnect after unplugging and replugging. **Note:** It can take up to 30s for a camera to appear after it has been plugged in.
- IP `ip` - The IP address of the camera to connect to.
- Log to Console `logconsole` - ⊞ - Sets the log level to output to the console.
...
- Log to File `logfile` - ⊞ - Sets the log level to output to the log file.
  * Off `off` - Off (close logs).
  * Debug `debug` - Debug.
  * Info `info` - Information.
  * Warn `warn` - Warnings.
...
- Image `image` - ⊞ - Select which of the camera's video streams to display in the node's output image. Additional streams can be obtained for the same camera using the [Orbbec Select TOP](Orbbec_Select_TOP.md "Orbbec Select TOP").
...
- Enable Gyro Sensor `gyro` - Enable rotational IMU data from the gyro sensor if it is available on the current camera. The data is measured in degrees/sec and accessible as CHOP channels by attaching an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
- Enable Accel Sensor `accel` - Enable linear acceleration IMU data from the accel sensor if it is available on the current camera. The data is measured in m/s^2 and accessible as CHOP channels by attaching an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
- Properties CHOP `propschop` - The path of the properties CHOP being referenced. Each channel of the CHOP represents an [OBPropertyID](https://orbbec.github.io/OrbbecSDK/doc/api/English/Property_8h.html#a9e7aac1ea041c8958ea8c6e3aa53bbab) which can be used to control camera settings. The channel name must be all lowercase and cannot include the 'OB_PROP_' prefix. For example, a valid channel name for 'OB_PROP_COLOR_MIRROR_BOOL' would be 'color_mirror_bool'. Supported properties differ between each Orbbec camera model. The current value of each supported property can be viewed in an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP"). The minimum, maximum, and default values of each supported property can be viewed in an [Info DAT](../DATs/Info_DAT.md "Info DAT").
...
Extra Information for the Orbbec TOP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
