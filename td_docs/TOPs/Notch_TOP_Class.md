...
This class inherits from the [ TOP class](TOP_Class.md "TOP Class"). It references a specific [Notch TOP](Notch_TOP.md "Notch TOP").

## Members

`exposedProperties` → `dict` **(Read Only)** :

Return a dictionary of key/value pairs representing all the exposed properties imported from the Notch block. The key is the UUID and the value is the corresponding [ParGroup](../Glossary/ParGroup.md "ParGroup") representation of the property.
