---
url: https://docs.derivative.ca/Analyze_TOP
category: TOPs
title: Analyze_TOP
---
...
[analyzeTOP_Class](Analyze_TOP_Class.md "AnalyzeTOP Class")
...
- Analyze Channel `analyzechannel` - ⊞ - Determines what value the operation is being performed on. For example, when the operation is Minimum and the Analyze Channel is Red, the output will be the pixel with the lowest red value (not necessarily the lowest Blue or Green values). To separately find the lowest value in each channel, set this parameter to RGBA Independent.
  ...
  * RGB Maximum `rgbmax` - Calculations are based on the maximum value of the RGB channels.
  * RGBA Maximum `max` - Calculations are based on the maximum value of the RGBA channels.
...
- Scope `scope` - ⊞ - Determines how pixels are grouped together to calculate the results.
  ...
  * Rows `rows` - Calculations are performed on each row of the input separately, resulting in an output image that is one pixel wide and the same number of rows as the input. If the input image is 3D, each slice will be one pixel wide and the same number of rows as the input.
  * Columns `columns` - Calculations are performed on each column of the input separately, resulting in an output image that is one pixel tall and the same number of columns as the input. If the input image is 3D, each slice will be one pixel tall and the same number of columns as the input.
  * Slices `slices` - This scope is for 3D textures only. Calculations are performed on each slice of the input separately, resulting in an output image that is a single pixel for each slice.
...
Extra Information for the Analyze TOP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
