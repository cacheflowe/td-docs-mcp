---
url: https://docs.derivative.ca/OrbbecTOP_Class
category: TOPs
title: OrbbecTOP_Class
---

# orbbecTOP Class

This class inherits from the [ TOP class](TOP_Class.md "TOP Class"). It references a specific [Orbbec TOP](Orbbec_TOP.md "Orbbec TOP").

## Members

`colorCameraIntrinsics` → `Tuple[float, float, float, float, float, float, float, float, float, float, float, float, int, int]` **(Read Only)** :

A named tuple containing the intrinsic camera parameters of the color camera: `cx, cy, fx, fy, k1, k2, k3, k4, k5, k6, p2, p1, width, height`. These constants can be used in the [Lens Distort TOP](Lens_Distort_TOP.md "Lens Distort TOP") and are also compatible with the Brown-Conrady model used in OpenCV.

`depthCameraIntrinsics` → `Tuple[float, float, float, float, float, float, float, float, float, float, float, float, int, int]` **(Read Only)** :

A named tuple containing the intrinsic parameters of the depth camera: `cx, cy, fx, fy, k1, k2, k3, k4, k5, k6, p2, p1, width, height`. These constants can be used in the [Lens Distort TOP](Lens_Distort_TOP.md "Lens Distort TOP") and are also compatible with the Brown-Conrady model used in OpenCV.

`cameraTransform` → `TDU.Matrix` **(Read Only)** :

A matrix that transforms the coordinate spaces from depth camera to color camera.
