---
url: https://docs.derivative.ca/MoviefileoutTOP_Class
category: TOPs
title: MoviefileoutTOP_Class
---

# moviefileoutTOP Class

This class inherits from the [ TOP class](https://docs.derivative.ca/TOP_Class "TOP Class"). It references a specific [Movie File Out TOP](https://docs.derivative.ca/Movie_File_Out_TOP "Movie File Out TOP").
...
Returns the generated file suffix. It will be generated based on the values of the parameters Unique Suffix and N, plus the file extension. It will take one of two forms: `_N_._ext_`or` _N_._i_._ext_`where` _N_`is the suffix index (uniquely generated if Unique Suffix is enabled),`_i_`is the image sequence index (used only for the image sequence type), and` _ext_`is the image/movie extension.
## Methods
