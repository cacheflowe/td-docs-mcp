...
It can also render text strings from a [Table DAT](../Glossary/Table_DAT.md "Table DAT") via the Specification DAT parameter, where column headings are the parameter names that are to ve overidden when rendering a line of text for reach row of the table.

Any [TrueType](http://en.wikipedia.org/wiki/TrueType) or [OpenType](http://en.wikipedia.org/wiki/OpenType) font that has been loaded into Windows can be rendered by the Text TOP. To import a new font into your Windows system, open the **Fonts** folder in the **Control Panel** , then drag and drop your [ font files](../Interoperability/File_Types.md "File Types") in (`.ttf/.otf` file format). Fonts can also be specified as `.ttf/.otf` file paths in the Font File parameter.

You can render **Unicode** text by reading the text as a python string. See [Unicode](../Glossary/Unicode.md "Unicode").

**Tip:** You can use the [textTOP_Class](Text_TOP_Class.md "TextTOP Class").textWidth member to calculate the width of the text in the Text TOP.

See also: [Field COMP](../COMPs/Field_COMP.md "Field COMP"), [Text SOP](../SOPs/Text_SOP.md "Text SOP"), [Unicode](../Glossary/Unicode.md "Unicode").
[![TextTOP.jpg](https://docs.derivative.ca/images/3/36/TextTOP.jpg)](https://docs.derivative.ca/File:TextTOP.jpg)
[textTOP_Class](Text_TOP_Class.md "TextTOP Class")

## Parameters - Text Page
- Field Component `field` - Specifies a [Field Component](../COMPs/Field_COMP.md "Field COMP") to use as the source of the text. The font and style of the text displayed in the Field Component are set using the parameters in the Text TOP.
...
- Display Method `dispmethod` - ⊞ - The display method used.
  ...
  * Scalable `scalable` - Uses [Slug Library](../Interoperability/Slug_Library.md "Slug Library") to render high-quality, scalable, resolution independent text.
...
## Info CHOP Channels
Extra Information for the Text TOP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
###
