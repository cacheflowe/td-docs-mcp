---
url: https://docs.derivative.ca/Luma_Blur_TOP
category: TOPs
title: Luma_Blur_TOP
---
...
[lumablurTOP_Class](Luma_Blur_TOP_Class.md "LumablurTOP Class")
...
- Type `type` - ⊞ - Determines the mathematical function used to create the blur.
  ...
  * Sinc `sinc` - A sharpening filter. Some pixels contribute a negative weight to the result, artifacts may occur in the form of "ringing". Keeps small details and edges better than Guassian.
  * Hanning `hanning` - A cosine approximation to a Guassian kernel.
  * Blackman `blackman` - A higher order cosine approximation to a Guassian kernel.
...
- Black Value `blackvalue` - The pixel luminance value used for the Black Filter Size parameter below.
- White Value `whitevalue` - The pixel luminance value used for the White Filter Size parameter below.
...
- Extend `extend` - ⊞ - Sets the extend conditions to determine what happens to the blur at the edge of the image.
  ...
  * Zero `zero` - The blue is calculated as if the image does not extend past its edges.
...
Extra Information for the Luma Blur TOP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
