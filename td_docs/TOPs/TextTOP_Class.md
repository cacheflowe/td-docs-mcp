...
This class inherits from the [ TOP class](https://docs.derivative.ca/TOP_Class "TOP Class"). It references a specific [Text TOP](https://docs.derivative.ca/Text_TOP "Text TOP").


## Members
`curText` → `str` **(Read Only)** :
Current text contents, when used with a [Field COMP](https://docs.derivative.ca/Field_COMP "Field COMP").

`cursorEnd` → `int` :
Get or set cursor end position, when used with a [Field COMP](https://docs.derivative.ca/Field_COMP "Field COMP").

`cursorStart` → `int` :
Get or set cursor start position, when used with a [Field COMP](https://docs.derivative.ca/Field_COMP "Field COMP").

`selectedText` → `str` **(Read Only)** :
Selected contents, when used with a [Field COMP](https://docs.derivative.ca/Field_COMP "Field COMP").
...
`lines()`→ `List[TextLine]`:
Returns a list of [TextLine Class](https://docs.derivative.ca/TextLine_Class "TextLine Class") objects. This list of lines is formed after operations such as word-wrap have been applied.
#  TOP Class
## Members
`width` → `int` **(Read Only)** :
