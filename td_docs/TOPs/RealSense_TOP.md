---
url: https://docs.derivative.ca/RealSense_TOP
category: TOPs
title: RealSense_TOP
---
...
[RealSense](../Interoperability/RealSense.md "RealSense") is a tracking device from Intel. The RealSense TOP connects to Intel [RealSense](../Interoperability/RealSense.md "RealSense") devices and outputs color, depth and IR data from it.

See also [RealSense](../Interoperability/RealSense.md "RealSense") for hardware information and installation instruction, and the [RealSense CHOP](../CHOPs/RealSense_CHOP.md "RealSense CHOP").

[realsenseTOP_Class](RealSense_TOP_Class.md "RealsenseTOP Class")
...
- Image `image` - ⊞ - Select the image type to output.
  ...
  * Depth To Color UV Map `depthtocoloruv` - A RG 32-bit float texture that is the UV values needed to remap the Depth image to line up with the Color image. Use a [Remap TOP](Remap_TOP.md "Remap TOP") with this and the Depth image to produce something that will be aligned with the Color image. (RealSense SDK for Windows only)
  * Color To Depth UV Map `colortodepthuv` - A RG 32-bit float texture that is the UV values needed to remap the Color image to line up with the Depth image. Use a [Remap TOP](Remap_TOP.md "Remap TOP") with this and the Color image to produce something that will be aligned with the Depth image. (RealSense SDK for Windows only)
...
- Options CHOP `optionschop` - Channels specified in this CHOP allow for setting all of the options that the RealSense camera supports. Channel names should be the same as the C enumeration, with the RS2_OPTION_ prefix removed, and all lowercase. E.g RS2_OPTION_ENABLE_AUTO_EXPOSURE can be set by using a channel named "enable_auto_exposure". A list of options can be found [here.](https://intelrealsense.github.io/librealsense/doxygen/rs__option_8h.html)
- Skeleton Tracking `skeltracking` - When enabled, performs skeleton tracking using the [Cubemos Skeleton Tracking API](https://www.intelrealsense.com/skeleton-tracking/). The results can be fetched via the [RealSense CHOP](../CHOPs/RealSense_CHOP.md "RealSense CHOP"). A Cubemos license is required for usage. A trial license is available.
...
Extra Information for the RealSense TOP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
