---
url: https://docs.derivative.ca/Render_Simple_TOP
category: TOPs
title: Render_Simple_TOP
---
...
Render Simple does not require a [Camera COMP](../Glossary/Camera_COMP.md "Camera COMP"), [Geometry COMP](../Glossary/Geometry_COMP.md "Geometry COMP") or [Light COMP](../Glossary/Light_COMP.md "Light COMP") that is required by the more full-featured [Render TOP](Render_TOP.md "Render TOP").

See also [Render TOP](Render_TOP.md "Render TOP")

[rendersimpleTOP_Class](Render_Simple_TOP_Class.md "RendersimpleTOP Class")
...
- Camera Distance `camdistance` - The distance along the Z-axis the camera is placed at. Similar to the Translate Z of a Camera COMP.
- Normalize Geo `normalizegeo` - Translate and uniformly scale the POP geometry to fit in a cube of size 2 at the origin.
- Background Color `bgcolor` - ⊞ - The background color to place behind the rendered geometry. Note that this is always pre-multiplied by the alpha, so set the alpha to 1.0 to get a non-transparent background.
  * Background Color `bgcolorr` -
  * Background Color `bgcolorg` -
  * Background Color `bgcolorb` -
  * Background Color `bgcolora` -
- POP `pop` - The path to a single POP to render.
- Geometry Translate `geotranslate` - ⊞ - Translate the geometry relative to it's original position.
  * Geometry Translate `geotranslatex` -
  * Geometry Translate `geotranslatey` -
  * Geometry Translate `geotranslatez` -
- Geometry Rotate `georotate` - ⊞ - Rotate the geometry relative to it's original position.
  * Geometry Rotate `georotatex` -
  * Geometry Rotate `georotatey` -
  * Geometry Rotate `georotatez` -
- Geometry Scale `geoscale` - Apply a uniform scale to the geometry relative to it's original position.
- Light Translate `lighttranslate` - ⊞ - Translate the single point light that is applied to this render in world space.
  * Light Translate `lighttranslatex` -
  * Light Translate `lighttranslatey` -
  * Light Translate `lighttranslatez` -
- Material Source `materialsource` - ⊞ - Select where the material that is applied to this POP comes from, some internal parameters, or a MAT node.
...
- Wireframe `wireframe` - When a Internal Phong is used, this turns on it's wire-frame mode.
- Constant `constant` - ⊞ - Contant color value for the Internal Phong Mode.
  * Constant `constantr` -
  * Constant `constantg` -
  * Constant `constantb` -
- Diffuse `diffuse` - ⊞ - Diffuse Color for the Internal Phong Mode.
  * Diffuse `diffuser` -
  * Diffuse `diffuseg` -
  * Diffuse `diffuseb` -
- Color Map `colormap` - A TOP that will be used for the 'Color Map' for the Internal Phong Mode.
...
Extra Information for the Render Simple TOP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
