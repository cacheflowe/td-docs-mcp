...
The GLSL TOP renders a GLSL [shader](../Glossary/Shader.md "Shader") into a TOP image. Use the Info DAT to check for compile errors in your shaders.

The GLSL TOP can act as a pixel shader, or the more general and complex [Compute Shader](../Interoperability/Compute_Shader.md "Compute Shader"). Caveat: Compute Shaders need GLSL 4.30 or later.
...
See the [GLSL Category](https://docs.derivative.ca/Category:GLSL "Category:GLSL") for more information, and [Compute Shader](../Interoperability/Compute_Shader.md "Compute Shader").

[glslTOP_Class](GLSL_TOP_Class.md "GlslTOP Class")
...
- Vertex Shader `vertexdat` - Points to the [DAT](../Glossary/DAT.md "DAT") holding the Vertex Shader. Drag & Drop a DAT here, or manually enter the path to the DAT.
- Pixel Shader `pixeldat` - Points to the [DAT](../Glossary/DAT.md "DAT") holding the Pixel Shader. Drag & Drop a DAT here, or manually enter the path to the DAT.
- Compute Shader `computedat` - Points to the [DAT](../Glossary/DAT.md "DAT") holding the [Compute Shader](../Interoperability/Compute_Shader.md "Compute Shader"). Drag & Drop a DAT here, or manually enter the path to the DAT.
- Compile Behavior `compilebehavior` - ⊞ -
  * Stall Until Done `stalluntildone` -
  * Threaded, Show Checkerboard Until Done `threadedcheckerboard` -
  * Threaded, Show Black Until Done `threadedblack` -
  * Threaded, Show Previous Shader Until Done `threadedprevious` -
- Error Behavior `errorbehavior` - ⊞ -
  * Show Checkerboard `showcheckerboard` -
  * Show Black `showblack` -
  * Show Previous Shader `showprevious` -
...
- Output Access `outputaccess` - ⊞ - Controls how the output textures will be accessed. If the textures will be read from (such as using previous frame's values), then the access should be changed to Read-Write instead of Write Only.
  * Write Only `writeonly` - The texture will only be written to by the shader, not read.
  * Read-Write `readwrite` - The texture will be both written and read by the shader.
...
Like the Vectors page, these are simple uniforms. These uniforms should always be declared as `vec4`. However, the values in these parameters are interpreted according to the Parameter Color Space menus for this node. They will convert the color values in the parameters to the [Working Color Space](https://docs.derivative.ca/Working_Color_Space "Working Color Space") before passing them to the shader, if a Working Color Space is active.
- Pre-Multiply RGB by Alpha `premultrgbbyalpha` - Multiplies the RGB values in all the Color parameters by their Alpha value, before passing them to the shader.
- Color `color` -
- Name `color0name` - The name of the uniform in the shader to assign this color value to.
- RGB `color0rgb` - ⊞ - The RGB values for this uniform.
  * RGB `color0rgbr` -
  * RGB `color0rgbg` -
  * RGB `color0rgbb` -
- Alpha `color0alpha` - The Alpha value for this uniform.
...
- Atomic Counter `ac` - Sequence of atomic counter uniforms. Refer to [Atomic Counters](https://docs.derivative.ca/Write_a_GLSL_TOP#Atomic_Counters "Write a GLSL TOP").
...
## Parameters - Buffers Page

This page allows you to pass entire attribute buffers from POPs into the shader. A function is declared to access each attribute buffer, see [POP Attributes](https://docs.derivative.ca/Write_a_GLSL_TOP#POP_Attributes "Write a GLSL TOP") for syntax.
- Buffer `buffer` -
- POP `buffer0pop` - The path to the POP to get the attribute buffer from.
- Attribute Class `buffer0attrclass` - ⊞ - The class of the attribute that is being referenced
  * Point `point` -
  * Vertex `vertex` -
  * Primitive `primitive` -
- Attribute `buffer0attr` - The name of the attribute in the POP.
- Name `buffer0name` - The name you want to reference this attribute by inside the shader.
...
Extra Information for the GLSL TOP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
