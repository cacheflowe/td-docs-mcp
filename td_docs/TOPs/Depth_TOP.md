---
url: https://docs.derivative.ca/Depth_TOP
category: TOPs
title: Depth_TOP
---
...
The Depth TOP reads an image containing depth information from a scene described in a specified [Render TOP](Render_TOP.md "Render TOP"). The resulting image is black (0) at pixels where the surface is at the near depth value (Camera's parameter "Near"). It is white (1) at pixels where the surface is at the far depth value (parameter "Far").
...
You can use the [Level TOP](Level_TOP.md "Level TOP") to re-range the Depth TOP's values. However make sure you set the Pixel Format of the Level TOP to 16 or 32-bit, or you'll lose a lot of information from the Depth TOP's data (The Depth TOP has 24-bit data).
...
[depthTOP_Class](Depth_TOP_Class.md "DepthTOP Class")
...
- Peel Layer Index `peellayerindex` - When using the 'Depth-Peeling' feature in the [Render TOP](Render_TOP.md "Render TOP"), this chooses which peel layer to select.
...
Extra Information for the Depth TOP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
