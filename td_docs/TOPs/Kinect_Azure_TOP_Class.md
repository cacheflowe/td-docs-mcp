---
url: https://docs.derivative.ca/KinectazureTOP_Class
category: TOPs
title: KinectazureTOP_Class
---

# kinectazureTOP Class

This class inherits from the [ TOP class](TOP_Class.md "TOP Class"). It references a specific [Kinect Azure TOP](Kinect_Azure_TOP.md "Kinect Azure TOP").
...
A named tuple containing the intrinsic camera parameters of the color camera: `cx, cy, fx, fy, k1, k2, k3, k4, k5, k6, codx, cody, p2, p1`. These constants can be used in the [Lens Distort TOP](Lens_Distort_TOP.md "Lens Distort TOP") and are also compatible with the Brown-Conrady model used in OpenCV.

`depthCameraIntrinsics` → `Tuple[float, float, float, float, float, float, float, float, float, float, float, float, float, float]` **(Read Only)** :

A named tuple containing the intrinsic parameters of the depth camera: `cx, cy, fx, fy, k1, k2, k3, k4, k5, k6, codx, cody, p2, p1`. These constants can be used in the [Lens Distort TOP](Lens_Distort_TOP.md "Lens Distort TOP") and are also compatible with the Brown-Conrady model used in OpenCV.

## Methods

`getRemapTransform(src, target)`→ `TDU.Matrix`:

Returns a matrix that transforms between the coordinate spaces of the source and target devices.
  * src - A number from 0 to 3 indicating the source device space (0 = depth camera, 1 = color camera, 2 = imu, 3 = gyro).
  * target - A number from 0 to 3 indicating the source device space (0 = depth camera, 1 = color camera, 2 = imu, 3 = gyro).

`transformWorldToColorImage(worldPos)`→ `Tuple[float, float, bool]`:

Transforms a 3D world position to a u and v position in the color camera image. The success member of the return value will be true if the position was transformed successfully. Returns a named tuple with these fields: `u, v, success`.
  * worldPos - A tdu.Vector containing a 3D world position.

`transformWorldToDepthImage(worldPos)`→ `Tuple[float, float, bool]`:

Transforms a 3D world position to a u and v position in the depth camera image. The success member of the return value will be true if the position was transformed successfully. Returns a named tuple with these fields: `u, v, success`.
  * worldPos - A tdu.Vector containing a 3D world position.
