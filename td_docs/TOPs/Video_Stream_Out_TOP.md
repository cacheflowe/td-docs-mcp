---
url: https://docs.derivative.ca/Video_Stream_Out_TOP
category: TOPs
title: Video_Stream_Out_TOP
---
...
The Video Stream Out TOP acts as an [RTMP](https://docs.derivative.ca/RTMP "RTMP") or [SRT](../Interoperability/SRT.md "SRT") sender, can create an [RTSP](../Interoperability/RTSP.md "RTSP") server, to send compressed video and audio across the network. It uses Nvidia's hardware encoder. For RTSP, it can handle multiple clients connecting to it at the same time. Multiple Video Stream Out TOPs using the same port will be handled using the same underlying RTSP server. The Video Stream Out TOP can also be used to send a video stream through [WebRTC](../Interoperability/WebRTC.md "WebRTC") video/audio tracks.
...
SRT sent from the Video Stream Out TOP can include per-frame metadata making it easy to send and receive CHOP/DAT data in sync with video. It can be read with the [Video Stream In TOP](Video_Stream_In_TOP.md "Video Stream In TOP").
...
See also the [Video Stream In TOP](Video_Stream_In_TOP.md "Video Stream In TOP"), [RTMP](https://docs.derivative.ca/RTMP "RTMP"), [RTSP](../Interoperability/RTSP.md "RTSP") and [Video Streaming User Guide](../Learn/Video_Streaming_User_Guide.md "Video Streaming User Guide"), [Movie File Out TOP](Movie_File_Out_TOP.md "Movie File Out TOP").

For other protocols over IP see [NDI (Network Data Interface)](../Interoperability/NDI.md "NDI"), and [Touch Out TOP](Touch_Out_TOP.md "Touch Out TOP") / [Touch In TOP](Touch_In_TOP.md "Touch In TOP").

**NOTE for Windows OS - If experiencing connection issues make sure Windows Firewall is disabled.**

[videostreamoutTOP_Class](Video_Stream_Out_TOP_Class.md "VideostreamoutTOP Class")
...
- Mode `mode` - ⊞ - Selects if the mode works as an RTSP server, sends RTMP to a receiever such as a distribution service like YouTube or Twitch, or sends to an SRT destination.
  ...
  * WebRTC `webrtc` - Use a WebRTC peer. More info: [WebRTC](../Interoperability/WebRTC.md "WebRTC"), [WebRTC DAT](../DATs/WebRTC_DAT.md "WebRTC DAT").
...
- Audio CHOP `audiochop` - A timesliced audio source to send along with the video. Depending on the protocol and audio codec, you same need to resample the audio using a [Resample CHOP](../CHOPs/Resample_CHOP.md "Resample CHOP") to either 44100 or 48000Hz.
...
- Include Silent Audio Stream `includesilentaudio` - ⊞ - Some broadcasting services require an audio stream to be included. This will include a silent audio stream along with the video in the event there isn't actual audio being streamed video the CHOP parameter.
  ...
  * On `on` -
  * Off `off` -
- Per-Frame Metadata CHOP/DAT `perframemetadata` - Send metadata from this OP with each frame of the video stream. This data can be recevied from the [Video Stream In TOP](Video_Stream_In_TOP.md "Video Stream In TOP") using an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP") and [Info DAT](../DATs/Info_DAT.md "Info DAT").
- Per-Frame Metadata `perframemetadata` - Send metadata from this OP with each frame of the video stream. This data can be received from the [Video Stream In TOP](Video_Stream_In_TOP.md "Video Stream In TOP") using an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP") and [Info DAT](../DATs/Info_DAT.md "Info DAT").

## Parameters - WebRTC Page
- WebRTC `webrtc` - Set the [WebRTC DAT](../DATs/WebRTC_DAT.md "WebRTC DAT") (ie. peer) to send the video stream over. Setting this will automatically populate the WebRTC Connection parameter menu with available connections.
- WebRTC Connection `webrtcconnection` - Select the [WebRTC](../Interoperability/WebRTC.md "WebRTC") peer-to-peer connection. Selecting this will automatically population the WebRTC Track parameter menu with available video output tracks.
...
Extra Information for the Video Stream Out TOP can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
