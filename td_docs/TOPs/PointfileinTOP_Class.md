---
url: https://docs.derivative.ca/PointfileinTOP_Class
category: TOPs
title: PointfileinTOP_Class
---

# pointfileinTOP Class

This class inherits from the [ TOP class](https://docs.derivative.ca/TOP_Class "TOP Class"). It references a specific [Point File In TOP](https://docs.derivative.ca/Point_File_In_TOP "Point File In TOP").


## Members
`numFields` → **(Read Only)** :
Get the number of channels defined per point e.g. a point may have XYZ and/or RGB channels.
`numPoints` → **(Read Only)** :
Get the number of points available in the current image. This could be less than the number of points that were loaded if the image resolution has been reduced or is limited by the current license. This number may also be less than the number of pixels in the image if fewer points were loaded than the image can store.
`numSrcPoints` → **(Read Only)** :
Get the number of points in the original file. It is possible that the graphics card cannot create an image large enough to fit all of the available points in the file. In these cases the numSrcPoints will be greater than numLoadedPoints.
`numLoadedPoints` → **(Read Only)** :
Get the number of points loaded from the file. This could be less than the available number of points in the source file if the graphics card does not support images large enough to store all of the point data. The maximum number of points is determined by the maximum image resolution and the number of channels per point i.e. Max Points = Max Res * Max Res * Num_Images_Channels (4) / Num_Point_Channels. For example, on a card that supports 16k textures, you could store 16,384 x 16,384 x 4 / 3 = 357 million points with XYZ data or 119 million points with XYZ and RGB data.
## Methods
