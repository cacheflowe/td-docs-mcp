---
url: https://docs.derivative.ca/BlobtrackTOP_Class
category: TOPs
title: BlobtrackTOP_Class
---

# blobtrackTOP Class

This class inherits from the [ TOP class](https://docs.derivative.ca/TOP_Class "TOP Class"). It references a specific [Blob Track TOP](https://docs.derivative.ca/Blob_Track_TOP "Blob Track TOP").
...
```
# me - this DAT.
# blobTrackTop - the connected Blob Track TOP
# blobs - A list of named tuples describing the blob, with the following fields:
#	id		- the ID of the blob
#	u 		- blob x position (normalized)
#	v 		- blob y position (normalized)
#	width	- blob width (normalized)
#	height	- blob height (normalized)
#	age		- time in seconds that the blob has been active

def onBlobTrack(blobTrackTop, blobs):
	return

# onBlobStateChange is triggered when the state of any blob changes
# In addition to the fields of blobs in onBlobTrack, blobs passed to the onBlobStateChange callback will have an additional 'state' field:
# 'new' 	- a newly tracked blob with a new id
# 'revived' - a previously lost blob that has come back to life with its old id
# 'lost' 	- a blob that is no longer tracked; lost blobs still have the potential to be revived if Revive Blobs is enabled
# 'expired' - the time spent lost exceeded Revive Time and the blob can no longer be revived
