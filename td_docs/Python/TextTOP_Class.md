---
url: https://docs.derivative.ca/TextTOP_Class
category: Python
title: TextTOP_Class
---
...
This class inherits from the [ TOP class](../TOPs/TOP_Class.md "TOP Class"). It references a specific [Text TOP](../TOPs/Text_TOP.md "Text TOP").

## Members

`curText` → `str` **(Read Only)** :

Current text contents, when used with a [Field COMP](../COMPs/Field_COMP.md "Field COMP").

`cursorEnd` → `int` :

Get or set cursor end position, when used with a [Field COMP](../COMPs/Field_COMP.md "Field COMP").

`cursorStart` → `int` :

Get or set cursor start position, when used with a [Field COMP](../COMPs/Field_COMP.md "Field COMP").

`selectedText` → `str` **(Read Only)** :

Selected contents, when used with a [Field COMP](../COMPs/Field_COMP.md "Field COMP").
...
Returns a list of [TextLine Class](TextLine_Class.md "TextLine Class") objects. This list of lines is formed after operations such as word-wrap have been applied.
...
Returns the pixel Format of the TOP. The returned string format resembles the pixel format on the operator's common page. This value is only useful for display purposes, not for python handling or interacting with parms. For the latter, use `pixelFormatName`.

`pixelFormatName` → `str` **(Read Only)** :
...
Saves the image to the file system. Support file formats are: `.tif`, `.tiff`, `.jpg`, `.jpeg`, `.bmp`, `.png`, `.exr` and `.dds`. Returns a [FileSaveStatus Class](https://docs.derivative.ca/FileSaveStatus_Class "FileSaveStatus Class") object that can be cast to a str to return filename and path used, and the `isCompleted()` member can be queried to check if file finished saving which is useful for asynchronous saving.
