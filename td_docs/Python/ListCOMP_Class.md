---
url: https://docs.derivative.ca/ListCOMP_Class
category: Python
title: ListCOMP_Class
---

# listCOMP Class

This class inherits from the [ COMP class](COMP_Class.md "COMP Class"). It references a specific [List COMP](../COMPs/List_COMP.md "List COMP").

## Members

`attribs` → `td.ListAttribute` **(Read Only)** :

The table [attributes](ListAttribute_Class.md "ListAttribute Class"). The members of these attributes can be directly written to / updated with new values. Any row/col or cell attributes defined will override these values. Define global per-table attributes here.
...
`colAttribs` → `td.ListAttributes` **(Read Only)** :

The set of row [attributes](ListAttributes_Class.md "ListAttributes Class"). Accessed by row index. The members of these attributes can be directly written to / updated with new values and take priority over any attribute members defined a the table level.
...
`rowAttribs` → `td.ListAttributes` **(Read Only)** :
...
`cellAttribs` → `td.ListAttributes` **(Read Only)** :

The set of cell [attributes](ListAttributes_Class.md "ListAttributes Class"). Accessed by row and column. The members of these attributes can be directly written to / updated with new values and take priority over any attribute members defined at the row/col or table level.
...
`displayAttribs` → `td.ListAttributes` **(Read Only)** :
...
`focusCol` → `int` **(Read Only)** :

Last column with focus for editing.

`focusRow` → `int` **(Read Only)** :

Last row with focus for editing.

`radioCol` → `int` **(Read Only)** :

The last selected column.

`radioRow` → `int` **(Read Only)** :

The last selected row.

`rolloverCol` → `int` **(Read Only)** :

The last column rolled over.

`rolloverRow` → `int` **(Read Only)** :

The last row rolled over.

`selectCol` → `int` **(Read Only)** :

The currently selected column.

`selectRow` → `int` **(Read Only)** :

The currently selected row.

`selectionBorderColor` → `Tuple[float, float, float, float]` :

Get or set the border color for the separate selection, expressed as a 4-tuple (r,g,b,a), representing its red, green, blue and alpha value.

`selectionColor` → `Tuple[float, float, float, float]` :

Get or set the background color for the separate selection, expressed as a 4-tuple (r,g,b,a), representing its red, green, blue and alpha value.

`selections` → `List[Tuple[int, int, int, int]]` :

Get or set the row and column coordinates for separate selection formatting, expressed as a list of 4-tuples, each containing: (startrow, startcol, endrow, endcol).

`dragRow` → `int` **(Read Only)** :

The currently dragged row.

`dragCol` → `int` **(Read Only)** :

The currently dragged column.

`dropRow` → `int` **(Read Only)** :

The current row about to be dropped over.

`dropCol` → `int` **(Read Only)** :

The current col about to be dropped over.
...
`setKeyboardFocus(row, col, selectAll=False)`→ `None`:
...
`displlayColWidth()`→ `int`:

Return the actual column width including stretching.

`displayRowHeight()`→ `int`:

Return the actual row height including stretching.
...
The [Panel](Panel_Class.md "Panel Class") from which [Panel Values](../Glossary/Panel_Value.md "Panel Value") and the [PanelValue Class](PanelValue_Class.md "PanelValue Class") may be accessed. (The second form is usually sufficient.)
...
Returns a tuple containing the mouse coordinates relative to the [Panel Component](../Glossary/Panel_Component.md "Panel Component"). If the mouse is not over a window containing the panel, None is returned instead.

`locateMouseUV()`→ `Tuple[float, float]`:

Returns a tuple containing the normalized mouse coordinates relative to the [Panel Component](../Glossary/Panel_Component.md "Panel Component"). If the mouse is not over a window containing the panel, None is returned instead.
...
A list of [extensions](../Glossary/Extensions.md "Extensions") attached to this component.
...
A dictionary of [internal operator shortcuts](../Glossary/Internal_Operators.md "Internal Operators") found in this component. See also [OP.iop](OP_Class.md#General "OP Class")

`internalPars` → `Dict` **(Read Only)** :

A dictionary of [internal parameters shortcuts](../Glossary/Internal_Parameters.md "Internal Parameters") found in this component. See also [OP.ipar](OP_Class.md#General "OP Class")

`clones` → `List[COMP]` **(Read Only)** :

A list of all [components](COMP_Class.md "COMP Class") cloned to this component.

`componentCloneImmune` → `bool` :

Get or set [component clone Immune flag](../Glossary/Immune_Flag.md "Immune Flag"). This works together with the cloneImmune member of the [OP_Class](OP_Class.md "OP Class"). When componentCloneImmune is True, everything inside the clone is [immune](../Glossary/Immune.md "Immune"). When componentCloneImmune is False, it uses the [OP_Class](OP_Class.md "OP Class") cloneImmune member to determine if just the component is immune (its parameters etc, but not the component's network inside).

`vfs` → `VFS` **(Read Only)** :

An intermediate [VFS object](VFS_Class.md "VFS Class") from which embedded [VFSFile objects](VFSFile_Class.md "VFSFile Class") can be accessed. For more information see [Virtual File System](../Glossary/Virtual_File_System.md "Virtual File System").
...
The child [operator](OP_Class.md "OP Class") that is currently selected. To make an operator current, use its own [OP.current](OP_Class.md#Common_Flags "OP Class") method.

`selectedChildren` → `List[OP]` **(Read Only)** :

The list of currently selected [children](OP_Class.md "OP Class"). To change an individual operator's selection state, use its own [OP.selected](OP_Class.md#Common_Flags "OP Class") method.
...
List of input [connectors](Connector_Class.md "Connector Class") (on the top) associated with this component.

`outputCOMPs` → `List[COMP]` **(Read Only)** :

List of output [components](COMP_Class.md "COMP Class") from this component through its bottom connector.
...
Move all selected operators into a new [Base COMP](../COMPs/Base_COMP.md "Base COMP"). Equivalent to right-click on the network background and choosing Collapse Selected. Returns reference to the created Base COMP
...
Return a list of [operators](OP_Class.md "OP Class") matching the specified criteria.
  ...
  * name - (Keyword, Optional) Specify the name of the OP. [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching") supported. Example: name='project*'
  * path - (Keyword, Optional) Specify the path of the OP. [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching") supported. Example: path='*/pics/*'
  ...
  * text - (Keyword, Optional) Specify the DAT contents of the OP. [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching") supported. Example: text='*import*'
  * comment - (Keyword, Optional) Specify the OP comment. [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching") supported. Example: comment='*todo*'
  * tags - (Keyword, Optional) Specify a list of tags to search. [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching") supported. Example: tags=['*sequencer*', '*interface*']
  * allTags - (Keyword, Optional) When True, only include OPs where all specified tags are matched.
  * parValue - (Keyword, Optional) Specify the value of any parameters in the OP. [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching") supported. Example: parValue='500'
  * parExpr - (Keyword, Optional) Specify the expression of any parameters in the OP. [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching") supported. Example: parExpr='*sin*'
  * parName - (Keyword, Optional) Specify the name of any parameters in the OP. [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching") supported. Example: parName='clone'
...
This frees memory of a component, over the time of several frames to minimize the perceivable impact. `progressiveUnload()` allows you to try to release the CPU and GPU memory allocated for the child nodes of this component, with the unloading staggered over several frames. For each unloaded node, the memory will get reallocated next time the node cooks, so make sure nothing is still using it to keep it released.
...
See COMP.`progressiveUnloader` to observe and control thge unloading. The component's `progressiveUnloader` [ member](https://docs.derivative.ca/ProgressiveUnloader_Class "ProgressiveUnloader Class") contains status info regarding the latest unload operation, and has methods for modifying the unload operation such as `kill()` which terminates the unloading.
...
Initialize the components [extensions](../Glossary/Extensions.md "Extensions"). To initialize an individual extension, specify its index.
...
Add a new [page](Page_Class.md "Page Class") of custom parameters. See [Page Class](Page_Class.md "Page Class") for more details. See [Custom Parameters](https://docs.derivative.ca/Custom_Parameters "Custom Parameters") for the procedure.
...
Return a list of all component variables in this COMP. Optional name patterns may be specified.
  * patterns - (Optional) The name(s) of variables whose values should be returned. [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching") can be used.
...
Get a [module on demand](MOD_Class.md "MOD Class") object that searches for DAT modules relative to this operator.
...
A list of [operators](OP_Class.md "OP Class") contained within this operator. Only [component](COMP_Class.md "COMP Class") operators have children, otherwise an empty list is returned.

`numChildren` → `int` **(Read Only)** :

Returns the number of children contained within the operator. Only [component](COMP_Class.md "COMP Class") operators have children.

`numChildrenRecursive` → `int` **(Read Only)** :

Returns the number of operators contained recursively within this operator. Only [component](COMP_Class.md "COMP Class") operators have children.
...
An intermediate [parameter collection](ParCollection_Class.md "ParCollection Class") object, from which a specific [parameter](../SOPs/Par_Class.md "Par Class") can be found.
...
A list of all [ParGroups](ParGroup_Class.md "ParGroup Class"), where a ParGroup is a set of parameters all drawn on the same line of a dialog, sharing the same label.

`seq` → `SequenceCollection` **(Read Only)** :

An intermediate [sequence collection](SequenceCollection_Class.md "SequenceCollection Class") object, from which a specific [sequence](Sequence_Class.md "Sequence Class") can be found.
...
A list of all [custom pages](Page_Class.md "Page Class").

`currentPage` → `Page[](Page_Class.md "Page Class")` :
...
Get or set node height, expressed in [network editor](NetworkEditor_Class.md "NetworkEditor Class") units.

`nodeWidth` → `int` :

Get or set node width, expressed in [network editor](NetworkEditor_Class.md "NetworkEditor Class") units.

`nodeX` → `int` :

Get or set node X value, expressed in [network editor](NetworkEditor_Class.md "NetworkEditor Class") units, measured from its left edge.

`nodeY` → `int` :

Get or set node Y value, expressed in [network editor](NetworkEditor_Class.md "NetworkEditor Class") units, measured from its bottom edge.

`nodeCenterX` → `int` :

Get or set node X value, expressed in [network editor](NetworkEditor_Class.md "NetworkEditor Class") units, measured from its center.

`nodeCenterY` → `int` :

Get or set node Y value, expressed in [network editor](NetworkEditor_Class.md "NetworkEditor Class") units, measured from its center.

`dock` → `OP` :

Get or set the [operator](OP_Class.md "OP Class") this operator is docked to. To clear docking, set this member to None.
...
See also the `OP.parent` methods. To connect components together see [COMP_Class#Connection](COMP_Class.md#Connection "COMP Class") section.

`inputs` → `List[OP]` **(Read Only)** :

List of input [operators](OP_Class.md "OP Class") (via left side connectors) to this operator. To get the number of inputs, use len(OP.inputs).

`outputs` → `List[OP]` **(Read Only)** :

List of output [operators](OP_Class.md "OP Class") (via right side connectors) from this operator.

`inputConnectors` → `List[Connector]` **(Read Only)** :

List of input [connectors](Connector_Class.md "Connector Class") (on the left side) associated with this operator.
...
**Deprecated** The total accumulated cook time of all children of this operator during the last frame. Zero if the operator is not a [COMP](COMP_Class.md "COMP Class") and/or has no children.

`childrenCPUCookTime` → `float` **(Read Only)** :

The total accumulated cook time of all children of this operator during the last frame. Zero if the operator is not a [COMP](COMP_Class.md "COMP Class") and/or has no children.
...
True if the operators is a [TOP](../TOPs/TOP.md "TOP").
...
**NOTE** : `create()`, `copy()` and `copyOPs()` is done by the parent operator (a component). For more information see [COMP.create, COMP.copy and COMP.copyOPs methods](COMP_Class.md#Methods "COMP Class").
