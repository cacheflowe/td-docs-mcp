---
url: https://docs.derivative.ca/DAT_Class
category: Python
title: DAT_Class
---
...
A [DAT](../Glossary/DAT.md "DAT") describes a reference to a DAT operator. This class inherits from the [ OP class](OP_Class.md "OP Class").
...
Local dictionary used during python execution of scripts in this DAT. The dictionary attribute is read only, but not its contents. Its contents may be manipulated directly with scripts, or with an [Examine DAT](../DATs/Examine_DAT.md "Examine DAT").
...
`appendRow(vals, nameOrIndex=None, sort=None)`→ `int`:
...
`appendRows(vals, nameOrIndex=None, sort=None)`→ `int`:
...
Returns a (possibly empty) list of [cells](Cell_Class.md "Cell Class") that match the given row/column names or indices. See DAT.cell method for similar usage.
