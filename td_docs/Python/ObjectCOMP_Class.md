---
url: https://docs.derivative.ca/ObjectCOMP_Class
category: Python
title: ObjectCOMP_Class
---

# ObjectCOMP Class

This class inherits from the COMP class. It is the parent class of these subclasses.

## Members

`localTransform` → `tdu.Matrix` **(Read Only)** :

The current local transform of the Object. This is the combination of both the parameters from the Xform and Pre-Xform page, without taking into account any parent or constraint transforms. See also [Matrix Class](Matrix_Class.md "Matrix Class"), [Position Class](Position_Class.md "Position Class") and [Vector Class](Vector_Class.md "Vector Class").

`worldTransform` → `tdu.Matrix` **(Read Only)** :
...
Sets the Translate, Rotate, Scale and Pivot parameters on the Xform page from the given [matrix](Matrix_Class.md "Matrix Class").
  * matrix - A matrix of values.

`preTransform()`→ `tdu.Matrix`:
...
`setPreTransform(matrix)`→ `None`:

Sets the Translate, Rotate, Scale and Pivot parameters on the Pre-Xform page from the given [matrix](Matrix_Class.md "Matrix Class").
  * matrix - A matrix of values.

`relativeTransform(target)`→ `tdu.Matrix`:
...
Returns a matrix that is the transform from the called object to the target object. This is, if you transform the called objects current transformation by the returned matrix, it will now be positioned at the target objects positions.
  * target - The target ObjectCOMP. See also [Matrix Class](Matrix_Class.md "Matrix Class"), [Position Class](Position_Class.md "Position Class") and [Vector Class](Vector_Class.md "Vector Class").

`importABC(filepath, lights=True, cameras=True, mergeGeometry=True, gpuDeform=True, rate=None, textureFolder=None, geometryFolder=None, animationFolder=None)`→ `None`:
...
`importFBX(filepath, lights=True, cameras=True, mergeGeometry=True, gpuDeform=True, rate=None, textureFolder=None, geometryFolder=None, animationFolder=None)`→ `None`:
