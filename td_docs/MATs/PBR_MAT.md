...
It also supports most [Adobe Substance 3D Designer](https://substance3d.adobe.com/community-assets) PBR materials loaded in the [Substance TOP](../TOPs/Substance_TOP.md "Substance TOP").

You can output its [GLSL shader](https://docs.derivative.ca/Category:GLSL "Category:GLSL") into two [DATs](../Glossary/DAT.md "DAT") for further adaptation in a [GLSL MAT](GLSL_MAT.md "GLSL MAT") by using the Output Shader parameter.
...
You can get more PBR materials from PBR texture libraries such as [Quixel](http://quixel.se/) and [Poliigon](http://www.poliigon.com/) and [Game Textures](https://gametextures.com/freebies). See also: [Substance TOP](../TOPs/Substance_TOP.md "Substance TOP").

[pbrMAT_Class](PBR_MAT_Class.md "PbrMAT Class")
...
- Channel Source `ambientocclusionmapchannelsource` - ⊞ -
  ...
  * RGB Average `rgbaverage` -
  * RGBA Average `average` -
- Normal Map (Bump) `normalmap` - ⊞ - Uses a [Normal Map](../TOPs/Normal_Map_TOP.md "Normal Map TOP") from TOPs to create a 'bump map' effect. Bump-mapping simulates bumps or wrinkles in a surface to give it a 3D depth effect. Your geometry must have tangent attributes created for this feature to work (T[4]). Create these using the [Attribute Create SOP](../SOPs/Attribute_Create_SOP.md "Attribute Create SOP").
...
- Instance Texture `instancetexture` - ⊞ - When provider per-instance textures in the [Geometry COMP](../Glossary/Geometry_COMP.md "Geometry COMP"), this parameter selects which map the instance texture will be applied as.
...
Extra Information for the PBR MAT can be accessed via an [Info CHOP](../CHOPs/Info_CHOP.md "Info CHOP").
