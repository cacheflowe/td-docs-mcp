...
This class inherits from the [ CHOP class](https://docs.derivative.ca/CHOP_Class "CHOP Class"). It references a specific [Expression CHOP](https://docs.derivative.ca/Expression_CHOP "Expression CHOP").


## Members
`chanIndex` → **(Read Only)** :
`me.chanIndex` can be used in any parameter to give a different value for each [channel](https://docs.derivative.ca/Channel_Class "Channel Class") being generated, for example `[3, 4, 5][me.chanIndex]`.
`inputVal` → **(Read Only)** :
The current value of the input sample being evaluated. To access channels from other inputs use the [operator's inputs](https://docs.derivative.ca/OP_Class#Connection "OP Class"). Example: me.inputs[1]['chan4'] will access chan4 of the second input.
