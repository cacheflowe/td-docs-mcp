---
url: https://docs.derivative.ca/SyncoutCHOP_Class
category: CHOPs
title: SyncoutCHOP_Class
---

# syncoutCHOP Class

This class inherits from the [ CHOP class](../Python/CHOP_Class.md "CHOP Class"). It references a specific [Sync Out CHOP](Sync_Out_CHOP.md "Sync Out CHOP").

## Members

`timecode` → `tdu.Timecode` **(Read Only)** :

Get a Timecode object for the timecode data representation of the last sent index. See [Timecode Class](../TOPs/Timecode_Class.md "Timecode Class").

## Methods

`send(message)`→ `None`:

Send an auxiliary message. The message must be serializable. The message will come through the `onReceive` callback of the [Sync In CHOP](Sync_In_CHOP.md "Sync In CHOP").
