...
The output is calculated by adding the input's channel values for every sample, divided by the [samples per second](../Glossary/CHOP.md#Parts_of_a_CHOP "CHOP") of the Speed CHOP (typically 60), starting with the sample at the Start index. Negative input values reduce the output, positive values increase it. The cumulative values are put in the output channels.

When the CHOP is reset, its output can be set to any value via the Reset Value parameter.

When the second input is used to reset the output, where the second input is greater than 0, the area is reset and held to the reset value. For example, a [Wave CHOP](Wave_CHOP.md "Wave CHOP") (which is negative half the time) when passed into the second input causes the output to be reset for half a cycle.

See also: [Lookup CHOP](Lookup_CHOP.md "Lookup CHOP"), [Timer CHOP](Timer_CHOP.md "Timer CHOP"), [Count CHOP](Count_CHOP.md "Count CHOP"), [Event CHOP](Event_CHOP.md "Event CHOP")

[speedCHOP_Class](Speed_CHOP_Class.md "SpeedCHOP Class")
...
Extra Information for the Speed CHOP can be accessed via an [Info CHOP](Info_CHOP.md "Info CHOP").
