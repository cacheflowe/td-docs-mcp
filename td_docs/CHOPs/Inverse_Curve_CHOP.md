---
url: https://docs.derivative.ca/Inverse_Curve_CHOP
category: CHOPs
title: Inverse_Curve_CHOP
---
...
The Inverse Curve CHOP calculates an inverse kinematics simulation for [bone objects](../COMPs/Bone_COMP.md "Bone COMP") using a curve object.

The Inverse Curve CHOP will stretch and position a set of bones to follow a curve defined by another set of [objects](../Glossary/Object.md "Object") (guide).
...
You can view this curve by using the [Inverse Curve SOP](../SOPs/Inverse_Curve_SOP.md "Inverse Curve SOP"). Just supply it with the name of the Inverse Curve CHOP and it will extract the geometry for viewing/debugging purposes. It actually will contain 3 curves (the original guide, and the guide displaced in +X and +Y). These curves can be skinned with a [Skin SOP](../SOPs/Skin_SOP.md "Skin SOP") for better feedback, etc.
...
[inversecurveCHOP_Class](Inverse_Curve_CHOP_Class.md "InversecurveCHOP Class")
...
- Span `span` - ⊞ -
  * `span1` - The beginning fraction of the guide curve to align to.
  * `span2` - The end fraction of the guide curve to align to.
- Interpolation `interpolation` - ⊞ - The type of guide curve to create with the guide components.
...
- Order `order` - The curve order when using NURBs or Bezier interpolation.
- Up Vector `upvector` - ⊞ - Control bone twist with this parameter.
...
Extra Information for the Inverse Curve CHOP can be accessed via an [Info CHOP](Info_CHOP.md "Info CHOP").
