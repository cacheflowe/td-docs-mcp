...
The Audio Stream Out CHOP can stream audio out to any [rtsp](http://en.wikipedia.org/wiki/Real_Time_Streaming_Protocol) client such as VideoLAN's VLC media player and Apple's Quicktime, or to a [WebRTC](../Interoperability/WebRTC.md "WebRTC") peer.
...
[audiostreamoutCHOP_Class](Audio_Stream_Out_CHOP_Class.md "AudiostreamoutCHOP Class")

## Parameters - Audio Stream Out Page
- Active `active` - Toggle's the rtsp server on or off.
- Mode `mode` - ⊞ - Select the stream out mode: either [RTSP](../Interoperability/RTSP.md "RTSP") or [WebRTC](../Interoperability/WebRTC.md "WebRTC").
  * RTSP `rtsp` -
  * WebRTC `webrtc` -
...
- WebRTC `webrtc` - Set the [WebRTC DAT](../DATs/WebRTC_DAT.md "WebRTC DAT") (ie. peer) to send the audio stream over. Setting this will automatically populate the WebRTC Connection parameter menu with available connections.
- WebRTC Connection `webrtcconnection` - Select the [WebRTC](../Interoperability/WebRTC.md "WebRTC") peer-to-peer connection. Selecting this will automatically population the WebRTC Track parameter menu with available audio output tracks.
...
Extra Information for the Audio Stream Out CHOP can be accessed via an [Info CHOP](Info_CHOP.md "Info CHOP").
