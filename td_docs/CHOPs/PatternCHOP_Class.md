...
This class inherits from the [ CHOP class](https://docs.derivative.ca/CHOP_Class "CHOP Class"). It references a specific [Pattern CHOP](https://docs.derivative.ca/Pattern_CHOP "Pattern CHOP").


## Members
`chanIndex` → `int` **(Read Only)** :
The index of the current [channel](https://docs.derivative.ca/Channel_Class "Channel Class") being evaluated. For example, if Pattern generates three channels you can put `[1, 3, 7][me.chanIndex]` in the Amplitude parameter to customize the amplitude for each channel.
