---
url: https://docs.derivative.ca/Constant_CHOP
category: CHOPs
title: Constant_CHOP
---
...
You can use [Pattern Expansion](../Glossary/Pattern_Expansion.md "Pattern Expansion") like `geo[1-4][xyz]` to generate multiple channels in one line.
...
The second input can be used to add offsets to the constant values. When the second input (Active) is greater than zero, any change to the first input will be added to the output of the CHOP. This is useful for adjusting Constant CHOP values from external input devices like a MIDI slider box. For example you can connect the mouse or a MIDI slider box to a [Mouse In CHOP](Mouse_In_CHOP.md "Mouse In CHOP") or [MIDI In CHOP](MIDI_In_CHOP.md "MIDI In CHOP"), and you can raise/lower the Constant CHOP values by holding the Active input On, while moving the mouse or sliders.

[constantCHOP_Class](Constant_CHOP_Class.md "ConstantCHOP Class")
...
in the first channel field instead of "`chan1`". See [Pattern Expansion](../Glossary/Pattern_Expansion.md "Pattern Expansion"), [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching").
...
You can simulate the pressing of the Snapshot Input button from a script. To simulate the clicking of a CHOP dialog box button from a script use the [Par Class](../SOPs/Par_Class.md "Par Class") pulse() Method:
...
Extra Information for the Constant CHOP can be accessed via an [Info CHOP](Info_CHOP.md "Info CHOP").
