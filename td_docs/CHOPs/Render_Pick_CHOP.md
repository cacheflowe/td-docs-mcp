---
url: https://docs.derivative.ca/Render_Pick_CHOP
category: CHOPs
title: Render_Pick_CHOP
---
...
The Render Pick CHOP samples a rendering (from a [Render TOP](../TOPs/Render_TOP.md "Render TOP") or a [Render Pass TOP](../TOPs/Render_Pass_TOP.md "Render Pass TOP")) and returns 3D information from the geometry at that particular pick location. Values sampled can include position, normals, point color, texture coordinates, depth and the object's path. An [Info DAT](../DATs/Info_DAT.md "Info DAT") should be used to retrieve the object's path.

The pick location is specified through uv coordinates of the rendering. These uv coordinates can be selected by clicking on a [Panel Component](../Glossary/Panel_Component.md "Panel Component") or explicitly setting them in the U and V parameters in the Render Pick CHOP.
...
See also the multi-sample [Render Pick DAT](../DATs/Render_Pick_DAT.md "Render Pick DAT").

[renderpickCHOP_Class](Render_Pick_CHOP_Class.md "RenderpickCHOP Class")
...
- Use Pickable Flags `usepickableflags` - When turned on only geometry whose [Pickable Flag](../Glossary/Flag.md "Flag") is on can be selected by the Render Pick CHOP. The [Pickable Flag](https://docs.derivative.ca/Pickable_Flag "Pickable Flag") is found on all [Object](../Glossary/Object.md "Object") components.
...
- Fetch Normal `normal` - ⊞ - Returns the normals of the point picked on the geometry. Channels _nx, ny, nz_.
...
- Fetch Point Color `color` - Returns the point color of the point picked on the geometry. Channels _cr, cg, cb, ca_.
- Fetch Texture UV `uv` - Returns the texture coordinates of the point picked on the geometry. Channels _mapu, mapv, mapw_.
- Fetch Object Path `path` - Return the path to the object that is picked. This result requires and [Info DAT](../DATs/Info_DAT.md "Info DAT") with its **Node Path** parameter referecning the Render Pick CHOP.
- Fetch Depth `depth` - Returns the depth of the point picked on the geometry. This value a non-linear ratio of the point's position between the near and far planes of the [Depth Buffer](https://docs.derivative.ca/index.php?title=Depth_Buffer&action=edit&redlink=1 "Depth Buffer \(page does not exist\)"). Channel is _depth_.
- Fetch Instance ID `instanceid` - Returns the Instance ID of the object. This will always be 0 if instancing is off. Channel is _instance_.
...
Extra Information for the Render Pick CHOP can be accessed via an [Info CHOP](Info_CHOP.md "Info CHOP").
