---
url: https://docs.derivative.ca/Pattern_CHOP
category: CHOPs
title: Pattern_CHOP
---
...
The Pattern CHOP generates a sequence of samples in a channel. Unlike the [Wave CHOP](Wave_CHOP.md "Wave CHOP") its purpose is generating arrays of samples that have no reference to time (seconds or frames).

It is useful for curve generation like lookup tables, simple shapes that can be converted to SOPs, generating channels that are passed to the [Geometry COMP](../Glossary/Geometry_COMP.md "Geometry COMP") to generate instances, and other non-time-based curve-generation, with as little need to make expressions in Python.
...
**Tip** : To display a CHOP viewer in the units of samples, make the [Viewer Active](../Glossary/Viewer_Active.md "Viewer Active") and change [RMB](../Glossary/Mouse_Click.md "Mouse Click") -> Units to be Samples.
...
[patternCHOP_Class](Pattern_CHOP_Class.md "PatternCHOP Class")
...
- Integer `integer` - ⊞ - A round-off menu to convert all numbers to integers.
  ...
  * Floor `floor` -
  * Round `round` -
- Reverse `reverse` - Reverses the final order of the samples as in the [Stretch CHOP](Stretch_CHOP.md "Stretch CHOP").
...
Extra Information for the Pattern CHOP can be accessed via an [Info CHOP](Info_CHOP.md "Info CHOP").
