---
url: https://docs.derivative.ca/Blob_Track_CHOP
category: CHOPs
title: Blob_Track_CHOP
---
...
Input data should be have two channels named _tx_ and _ty_ describing the 2D points in cartesian coordinates. See the [OP Snippets](../Learn/OP_Snippets.md "OP Snippets") examples.

Good sources for input scans are the [Hokuyo CHOP](../Interoperability/Hokuyo_CHOP.md "Hokuyo CHOP") (in Cartesian coordinate mode) and the [Leuze ROD4 CHOP](../Interoperability/Leuze_ROD4_CHOP.md "Leuze ROD4 CHOP"). These devices naturally output a radial scan from the device as the center point, where each point represents (angle, distance). Outputting each scan point in Cartesian coordinate mode gives an (x,y) for each point which is the format required for the Blob Track CHOP.
...
See also [Blob Track TOP](../TOPs/Blob_Track_TOP.md "Blob Track TOP")

[blobtrackCHOP_Class](Blob_Track_CHOP_Class.md "BlobtrackCHOP Class")
...
- Search Mode `searchmode` - ⊞ - Controls how searching for blobs is done across all the points.
  ...
  * Consecutive Points `consecutive` - Points are only compared against points in neighbouring samples of the CHOP data to find blobs. This is much faster but the data much be setup such that points that are close to each other in space are next to each other in the CHOP samples. Data coming from devices such as the [Hokuyo CHOP](../Interoperability/Hokuyo_CHOP.md "Hokuyo CHOP") and [Leuze ROD4 CHOP](../Interoperability/Leuze_ROD4_CHOP.md "Leuze ROD4 CHOP") output data in this format.
