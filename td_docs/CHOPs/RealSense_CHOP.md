---
url: https://docs.derivative.ca/RealSense_CHOP
category: CHOPs
title: RealSense_CHOP
---
...
**DEPRECATED:** Skeleton tracking features no longer licensable by Cubemos. [RealSense](../Interoperability/RealSense.md "RealSense") is a tracking device from Intel. The RealSense CHOP references a [RealSense TOP](../TOPs/RealSense_TOP.md "RealSense TOP") camera and fetches tracking data from it. The RealSense CHOP supports skeleton tracking through the [Cubemos Skeleton Tracking API](https://www.intelrealsense.com/skeleton-tracking/). [Cubemos](https://docs.derivative.ca/Cubemos "Cubemos") is Windows only. A Cubemos license is required to use the skeleton tracking.
...
See also: [RealSense](../Interoperability/RealSense.md "RealSense"), [RealSense TOP](../TOPs/RealSense_TOP.md "RealSense TOP")

[realsenseCHOP_Class](RealSense_CHOP_Class.md "RealsenseCHOP Class")
...
- Max Players `maxplayers` - Specify the max number of players tracked.
- Image Position Pixels `pos` - When enabled, will display positions of each landmark in image pixels as x/y channels.
- Image Positions Normalized `norm` - When enabled, will display normalized position values of each landmark as u/v channels.
...
Extra Information for the RealSense CHOP can be accessed via an [Info CHOP](Info_CHOP.md "Info CHOP").
