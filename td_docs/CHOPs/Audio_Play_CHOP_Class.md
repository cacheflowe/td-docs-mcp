---
url: https://docs.derivative.ca/AudioplayCHOP_Class
category: CHOPs
title: AudioplayCHOP_Class
---

# audioplayCHOP Class

This class inherits from the [ CHOP class](../Python/CHOP_Class.md "CHOP Class"). It references a specific [Audio Play CHOP](Audio_Play_CHOP.md "Audio Play CHOP").
...
`play(index, start=True, loop=False,  delaySeconds=0.0, pan=None, rolloff=None, volume=None, fadeSeconds=0.0)`→ `None`:

Trigger the playing of audio samples specified in an [Audio Play CHOP](Audio_Play_CHOP.md "Audio Play CHOP").
  ...
  * fadeSeconds - (Keyword, Optional) The number of seconds to fade to the specified volume when starting or stopping.

`stop(index)`→ `None`:

Stop the playing of audio samples specified in an [Audio Play CHOP](Audio_Play_CHOP.md "Audio Play CHOP"). Uses fadeSeconds specificed in play().
  * index - (Optional) The index of the audio sample to stop, or all if not specified.
