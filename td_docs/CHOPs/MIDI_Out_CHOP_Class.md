---
url: https://docs.derivative.ca/MidioutCHOP_Class
category: CHOPs
title: MidioutCHOP_Class
---

# midioutCHOP Class

This class inherits from the [ CHOP class](../Python/CHOP_Class.md "CHOP Class"). It references a specific [MIDI Out CHOP](MIDI_Out_CHOP.md "MIDI Out CHOP").
...
**Note:** the following event generation methods do not require any CHOP inputs connected to the MIDI Out CHOP.

`send(*messages)`→ `None`:

Send a sequence of bytes through this CHOP. To serialize non-byte values (example floats or integers) there are several python modules to do this, such as pickle or struct.
  * `*messages` - any number of messages to send
...
`sendBalance(channel, value)`→ `None`:
...
`sendNoteOn(channel, index, value)`→ `None`:
...
`sendPolyKeyPressure(channel, index, value)`→ `None`:
...
`sendPitchBend(channel, value)`→ `None`:
...
`sendEffectsDepth(channel, index,value)`→ `None`:
...
`sendDamperPedal(channel,value)`→ `None`:
...
`sendChannelPressure(channel, value)`→ `None`:
...
`sendControl(channel, index, value)`→ `None`:
...
`sendDataDecrement(channel,value)`→ `None`:
...
`sendAllNotesOff(channel,value)`→ `None`:
...
`sendSoundController(channel, index,value)`→ `None`:
...
`sendBankSelect(channel,value)`→ `None`:
...
`sendMainVolume(channel, value)`→ `None`:
...
`sendSoftPedal(channel,value)`→ `None`:
...
`sendLocalControl(channel,value)`→ `None`:
...
`sendResetAllControllers(channel,value)`→ `None`:
...
`sendBreathController(channel, value)`→ `None`:
...
`sendPortamentoTime(channel, value)`→ `None`:
...
`sendDataIncrement(channel,value)`→ `None`:
...
`sendFootController(channel, value)`→ `None`:
...
`sendSostenuto(channel,value)`→ `None`:
...
`sendOmniOff(channel,value)`→ `None`:
...
`sendGeneralPurposeController(channel, index,value)`→ `None`:
...
`sendNoteOff(channel, index, value)`→ `None`:
...
`sendExclusive(*messages)`→ `None`:

Send a sytem exclusive message through this CHOP. The System Exclusive start and end character are added to the message.

Messages can any combination of strings, byte arrays, or individual single-byte numeric values. To serialize non-byte values (example floats or integers) there are several python modules to do this, such as pickle or struct.
  * `*messages` - any number of messages to send
...
`sendPortamento(channel,value)`→ `None`:
...
`sendEffectControl(channel, index,value)`→ `None`:
...
`sendLegatoFootswitch(channel,value)`→ `None`:
...
`sendModulationWheel(channel,value)`→ `None`:
...
`sendProgram(channel, value)`→ `None`:
...
`sendPortamentoControl(channel,value)`→ `None`:
...
`sendTimecode(timecode)`→ `None`:

Sends the timecode to the device as a full MTC message.
  * timecode - The timecode value to send to the device. Must be a tdu.Timecode object. Timecode rate must be 24, 25, 29.97 (drop-frame), or 30. See [Timecode Class](../TOPs/Timecode_Class.md "Timecode Class").

```
n.sendTimecode(tdu.Timecode(100, fps=30))
```
...
Get or set the number of samples (or indices) per channel. You can change the number of samples by setting this value, only in a [scriptCHOP](Script_CHOP_Class.md "ScriptCHOP Class").

`start` → `float` :

Get or set the start index of the channels. This can be modified only when the CHOP is a [scriptCHOP](Script_CHOP_Class.md "ScriptCHOP Class").

`end` → `float` :

Get or set the end index of the channels. This can be modified only when the CHOP is a [scriptCHOP](Script_CHOP_Class.md "ScriptCHOP Class").

`rate` → `float` :

Get or set the sample rate of the CHOP. This can be modified only when the CHOP is a [scriptCHOP](Script_CHOP_Class.md "ScriptCHOP Class"). This value updates independently of time dependent content.

`isTimeSlice` → `bool` :

Get or set the last cooked [Time Slice](../Glossary/Time_Slicing.md "Time Slicing") value. True if the CHOP last cooked as a Time Slice. This can be modified only when the CHOP is a [scriptCHOP](Script_CHOP_Class.md "ScriptCHOP Class")
...
[Channels](../Python/Channel_Class.md "Channel Class") may be easily accessed from a CHOP using the [] subscript operator.
  * nameOrIndex - Must be an exact string name, or it may be a numeric channel index. Wildcards are not supported. Refer to the help on channels to see how to use the returned [Channel](../Python/Channel_Class.md "Channel Class") object.
...
Returns the first [Channel](../Python/Channel_Class.md "Channel Class") that matches the given name or index or None if none are found.
...
Returns a (possibly empty) list of [Channels](../Python/Channel_Class.md "Channel Class") that match that specified names or indices. Multiple names and indices may be provided.
