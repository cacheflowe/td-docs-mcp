---
url: https://docs.derivative.ca/EventCHOP_Class
category: CHOPs
title: EventCHOP_Class
---

# eventCHOP Class

This class inherits from the [ CHOP class](../Python/CHOP_Class.md "CHOP Class"). It references a specific [Event CHOP](Event_CHOP.md "Event CHOP").
...
```
op('event1').releaseEvent(id=4)
```

`createEvent(hold=False, samples=[], index=0, attackTime=None, attackLevel=None, decayTime=None, sustainTime=None, sustainMin=None, sustainMax=None, releaseTime=None, releaseLevel=None, speed=None)`→ `int`:
...
# me - this DAT
#
# eventOp - the OP which is cooking
# id - the event ID

def onCreate(eventOp, id):
	return
def onDestroy(eventOp, id):
	return
```
