---
url: https://docs.derivative.ca/Beat_CHOP
category: CHOPs
title: Beat_CHOP
---
...
See also: [Timeline CHOP](Timeline_CHOP.md "Timeline CHOP"), [Time COMP](../Glossary/Time_COMP.md "Time COMP").

[beatCHOP_Class](Beat_CHOP_Class.md "BeatCHOP Class")
...
- Use Reference Time Slice `reftimeslice` - Turn on this checkbox to use the [Time Slicing](../Glossary/Time_Slicing.md "Time Slicing") used by the Reference Node.
- Play Mode `playmode` - ⊞ - Specifies the method used to playback the output.
  * Locked to Timeline `locked` - This mode locks the output to the local component [Timeline](../Glossary/Timeline.md "Timeline"). Scrubbing or jumping in the local time will update the output accordingly. When the local timeline loops the output is reset and starts over. The output will be the same for each pass through the local timeline.
...
- Period `period` - Number of beats it takes to generate one cycle of ramp. Use the drop down menu to select from some commonly used Period lengths.
...
- Count `count` - Increases the count each period.
...
Extra Information for the Beat CHOP can be accessed via an [Info CHOP](Info_CHOP.md "Info CHOP").
