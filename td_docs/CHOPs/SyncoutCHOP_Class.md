...
This class inherits from the [ CHOP class](https://docs.derivative.ca/CHOP_Class "CHOP Class"). It references a specific [Sync Out CHOP](https://docs.derivative.ca/Sync_Out_CHOP "Sync Out CHOP").


## Members
`timecode` → `tdu.Timecode` **(Read Only)** :
Get a Timecode object for the timecode data representation of the last sent index. See [Timecode Class](https://docs.derivative.ca/Timecode_Class "Timecode Class").
## Methods
`send(message)`→ `None`:
Send an auxiliary message. The message must be serializable. The message will come through the `onReceive` callback of the [Sync In CHOP](https://docs.derivative.ca/Sync_In_CHOP "Sync In CHOP").
