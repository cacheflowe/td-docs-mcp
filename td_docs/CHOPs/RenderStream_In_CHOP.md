---
url: https://docs.derivative.ca/RenderStream_In_CHOP
category: CHOPs
title: RenderStream_In_CHOP
---
...
See also [RenderStream Out TOP](../TOPs/RenderStream_Out_TOP.md "RenderStream Out TOP"), [RenderStream In TOP](../TOPs/RenderStream_In_TOP.md "RenderStream In TOP"), [RenderStream](https://docs.derivative.ca/RenderStream "RenderStream").

[renderstreaminCHOP_Class](RenderStream_In_CHOP_Class.md "RenderstreaminCHOP Class")
...
- Schema DAT `schemadat` - This should be a DAT with a table containing the schema information to create sequencable parameters in Disguise. The Schema DAT supports the columns: `group, label, name, type, default, min, max, step`, where `group, label, name, type` are required. The `type` column should be one of: `float, image, text`. Float data will come in via the primary channels in this node. Image data can be obtained using a [RenderStream In TOP](../TOPs/RenderStream_In_TOP.md "RenderStream In TOP"). Text data can be obtained using an [Info DAT](../DATs/Info_DAT.md "Info DAT") pointing to this node.
...
Extra Information for the RenderStream CHOP can be accessed via an [Info CHOP](Info_CHOP.md "Info CHOP").
