---
url: https://docs.derivative.ca/Pipe_In_CHOP
category: CHOPs
title: Pipe_In_CHOP
---
...
The connection can be from a user-written program that outputs to the Pipe In CHOP's port, or from another TouchDesigner process, where the data is coming from a [Pipe Out CHOP](Pipe_Out_CHOP.md "Pipe Out CHOP").
...
**TIP:** The [TCP/IP DAT](https://docs.derivative.ca/TCP/IP_DAT "TCP/IP DAT") or [UDP In DAT](../Interoperability/UDP_In_DAT.md "UDP In DAT") is the preferred way to send script commands and other data packets over the network.

This node differs from the [Touch In CHOP](Touch_In_CHOP.md "Touch In CHOP") as it functions using a stream of commands, instead of a simple stream of channel data like the [Touch In CHOP](Touch_In_CHOP.md "Touch In CHOP"). Information comes in in various commands which are documented in the files located in your installation directory `C:/Program Files/Derivative/TouchDesigner*/touch/docs/pipe`.
...
See also [Shared Mem In CHOP](Shared_Mem_In_CHOP.md "Shared Mem In CHOP").

[pipeinCHOP_Class](Pipe_In_CHOP_Class.md "PipeinCHOP Class")
...
- Callbacks DAT `callbacks` - Path to a DAT containing callbacks for each event received. See [pipeinCHOP_Class](Pipe_In_CHOP_Class.md "PipeinCHOP Class") for usage.
...
Extra Information for the Pipe In CHOP can be accessed via an [Info CHOP](Info_CHOP.md "Info CHOP").
