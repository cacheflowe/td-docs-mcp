...
The [Sync In CHOP](Sync_In_CHOP.md "Sync In CHOP") and Sync Out CHOP are used to keep timelines in two or more TouchDesigner processes within a single frame of each other. One process will contain a Sync Out CHOP while one or more other processes contain [Sync In CHOPs](Sync_In_CHOP.md "Sync In CHOP"). The processes with [Sync In CHOPs](Sync_In_CHOP.md "Sync In CHOP") should have their **Realtime flag checked off** , as their frame rates will be determined by the Sync Out CHOP. Also note, all monitors (including all clients and server) should be set to the **same rate**. Note that unplugging or re-adding monitors may sometime change previously configured settings.

The CHOPs synchronize by pausing their own timeline until all Sync In/Out CHOPs have cooked. The Sync Out CHOP will be ahead of the [Sync In CHOPs](Sync_In_CHOP.md "Sync In CHOP"). If any CHOPs fail to communicate, the others will timeout, causing all processes to run slowly.
...
In addition any extra CHOP channels sent through the Sync Out CHOP is received by the [Sync In CHOPs](Sync_In_CHOP.md "Sync In CHOP").

**NOTE for Windows OS - If experiencing connection issues make sure Windows Firewall is disabled.**

An [Info DAT](../DATs/Info_DAT.md "Info DAT"), pointing to the Sync Out CHOP, provides a detailed list of all clients:
...
Similarly, an [Info CHOP](Info_CHOP.md "Info CHOP") will reveal further information: [Info CHOP Channels](#Info_CHOP_Channels)

##  Example Files

Here are two simple example files that use Sync In and Out CHOPs. You would run the SyncServer.toe only once, and run the SyncClient.toe for every machine you want to display on. Note that you may need to adjust the [Window COMP](../Glossary/Window_COMP.md "Window COMP") located at /perform to output to your desired monitor. Also note that [Hardware Frame Lock](https://docs.derivative.ca/Hardware_Frame_Lock "Hardware Frame Lock") is not enabled in these examples, so if you want to use that feature you'll need to turn that parameter on in the [Window COMP](../Glossary/Window_COMP.md "Window COMP") for SyncClient.toe as well.
...
Whenever the Sync Out CHOP first starts any and all clients are accepted, and nothing is banned. Pressing `Clear Stats` in the Sync Out CHOP also returns to this state, clearing all banned lists, and totals, even in the remote client [Info CHOPs](Info_CHOP.md "Info CHOP").
...
[syncoutCHOP_Class](Sync_Out_CHOP_Class.md "SyncoutCHOP Class")
...
- Clear Stats `clearstats` - Pressing this button will clear all banned lists, as well as totals reported in an [Info CHOP](Info_CHOP.md "Info CHOP").
...
Extra Information for the Sync Out CHOP can be accessed via an [Info CHOP](Info_CHOP.md "Info CHOP").
