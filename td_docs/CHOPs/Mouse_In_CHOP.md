...
The Mouse In CHOP and [Keyboard In CHOPs](Keyboard_In_CHOP.md "Keyboard In CHOP") are sometimes connected to the Position and Active inputs respectively of the [Record CHOP](Record_CHOP.md "Record CHOP") to enable the recording of channels.

[mouseinCHOP_Class](Mouse_In_CHOP_Class.md "MouseinCHOP Class")

## Parameters - Control Page
- Active `active` - ⊞ - While **On** , the mouse movement will be output from and the CHOP will cook every frame. When set to **Off** it will not cook and the current mouse X or Y values will not be output. **While Playing** will capture mouse events only when the [Timeline](../Glossary/Timeline.md "Timeline") is playing forward.
  * Off `off` -
  * On `on` -
  * While Playing `play` -
...
Extra Information for the Mouse In CHOP can be accessed via an [Info CHOP](Info_CHOP.md "Info CHOP").
