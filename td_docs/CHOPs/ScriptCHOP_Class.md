...
`copy(chop)`→ `None`:
Match all of this CHOPs channel data to the given [CHOP](https://docs.derivative.ca/CHOP "CHOP"). This includes sample rate, length, channel names and channel data.
  * chop - The CHOP to copy. This should be a [CHOP Class](https://docs.derivative.ca/CHOP_Class "CHOP Class") instance, not a path to the CHOP.
...
`isTimeSlice` → `bool` :
Get or set the last cooked [Time Slice](https://docs.derivative.ca/Time_Slicing "Time Slicing") value. True if the CHOP last cooked as a Time Slice. This can be modified only when the CHOP is a scriptCHOP
