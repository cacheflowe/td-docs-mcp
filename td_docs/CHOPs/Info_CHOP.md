...
Any OP that has a `.timecode` member will also have a Timecode Info Type available to view the timecode value as a set of CHOP channels. See [OPs with a timecode member](../Interoperability/Timecode.md#Operators_with_a_.timecode_Member "Timecode") and the [Timecode](../Interoperability/Timecode.md "Timecode") page for more info.

[infoCHOP_Class](Info_CHOP_Class.md "InfoCHOP Class")
...
- Scope `iscope` - The names of the attributes to keep. Name patterns may be used as detailed in [Pattern Matching](../Glossary/Pattern_Matching.md "Pattern Matching").
...
- Children Cook Time `childcooktime` - When the Info CHOP is monitoring a [Component](../Glossary/Component.md "Component"), this toggle will enable a channel called `children_cpu_cook_time` which is the total cooktime of all the component's children. This is off by default, enabling it for a large network may impact performance as all cooktimes need to be summed.
