---
url: https://docs.derivative.ca/MIDI_In_CHOP
category: CHOPs
title: MIDI_In_CHOP
---
...
'Note-On Only Channels' parameter for MIDI devices that only send Note On events, allowing for Note Off events to automatically be appended, giving single-sample pulses.

#### High Frequency MIDI Events with Timestamps

The MIDI events arriving faster than the timeline frame rate (typically 60 frames per second) can be captured and represented as CHOP channels that are set to 1000 samples per second.

Create a MIDI In CHOP, set the Mode to "Automatic High Frequency", connect to a Trail CHOP, turn Dots on in the viewer, and move a slider quickly to see the density of sample changes.

Look at the [MIDI In DAT](../DATs/MIDI_In_DAT.md "MIDI In DAT") and [MIDI Event DAT](../DATs/MIDI_Event_DAT.md "MIDI Event DAT") which capture all the MIDI events and reports their time stamps in various forms in their table columns.
...
See also the [MIDI In DAT](../DATs/MIDI_In_DAT.md "MIDI In DAT"), [MIDI Event DAT](../DATs/MIDI_Event_DAT.md "MIDI Event DAT"), [MIDI In Map CHOP](MIDI_In_Map_CHOP.md "MIDI In Map CHOP"), [MIDI Out CHOP](MIDI_Out_CHOP.md "MIDI Out CHOP").

[midiinCHOP_Class](MIDI_In_CHOP_Class.md "MidiinCHOP Class")

## Parameters - Source Page
- Active `active` - Enable or disable the MIDI In CHOP.
- Mode `mode` - ⊞ - Set the behavior mode for this CHOP.
  * Automatic `auto` - Channels are automatically created when MIDI signal is detected from the selected MIDI device. This was previously achieved by toggling on the 'Simplified' parameter.
  * Automatic High Frequency `autohighf` - This mode allows for the capturing of high frequency MIDI events i.e. events incoming faster than the timeline frame rate. Set the CHOP sample rate to a high value like 1000hz to be able to capture multiple MIDI events each frame. The 'Resample Mode' menu further controls how all the MIDI events received in a frame are spread out across the samples.
  * Manual `manual` - This mode allows for manual control over which MIDI channels are created and the values captured. This was previously achieved by toggling off the 'Simplified' parameter.
- Resample Mode `resamplemode` - ⊞ - Set how the high frequency MIDI events received in a frame are to be spaced out across the length of the CHOP. Only enabled for 'Automatic High Frequency' mode.
  * Evenly Spaced `even` - All the MIDI events received in a frame are spaced evenly across the length of the CHOP. This mode doesn't preserve the timing delta between the events of the frame.
  * Preserve Duty `preserveduty` - The MIDI events received in a frame are spaced approximately based on the timing delta between the events. Timing delta of the events between two frames is also preserved better. This mode works well for fast pulse or square wave cases.
...
- Note-On Only Channels `noteonchan` - For the channels specified in this parameter, a Note-Off event is added after every received MIDI event. This is useful for cases where MIDI channels only receive a Note-On event without a following Note-Off event.

1 Based Index `onebased` - Make the index 1 based instead of the default 0 based, which means that the indices will belong to the range 1-128 instead of 0-127. The value of this parameter affects the Note and Controller indices.
- MIDI Channels `channel` - The CHOP may read from any number of MIDI channels, numbered 1-16. Ranges and multiple entries are supported (i.e. "1 4 6", "1-7 12", "1-5:2").     If Channel Prefix is left blank, then the input streams from multiple MIDI channels will be combined into one set of CHOP channels (i.e., a "note 64 on" event on channel 12 followed by a "note 64 off" event on channel 8 appears in the note 64 CHOP channel as a single note).
...
The incoming messages must follow this specification:
  ...
  * If a single message in the range 0-63 is received, then nothing will be outputted.
...
Extra Information for the MIDI In CHOP can be accessed via an [Info CHOP](Info_CHOP.md "Info CHOP").
