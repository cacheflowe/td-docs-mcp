---
url: https://docs.derivative.ca/ScriptCHOP_Class
category: CHOPs
title: ScriptCHOP_Class
---

# scriptCHOP Class

This class inherits from the [ CHOP class](../Python/CHOP_Class.md "CHOP Class"). It references a specific [Script CHOP](Script_CHOP.md "Script CHOP").

## Members

`timeSliceDefault` → `bool` **(Read Only)** :

Get the default [Time Slice](https://docs.derivative.ca/index.php?title=Time_Slice&action=edit&redlink=1 "Time Slice \(page does not exist\)") for the [Script CHOP](Script_CHOP.md "Script CHOP"). Equal to the first input's `isTimeSlice`.

## Methods

These methods can be used by including them in the onCook callback of the Script CHOP. Using them outside the callback may cause unexpected results.
...
Remove all custom parameters from the Script CHOP. Returns number of destroyed pars.
...
Match all of this CHOPs channel data to the given [CHOP](../Glossary/CHOP.md "CHOP"). This includes sample rate, length, channel names and channel data.
  * chop - The CHOP to copy. This should be a [CHOP Class](../Python/CHOP_Class.md "CHOP Class") instance, not a path to the CHOP.
...
Get or set the sample rate of the CHOP. This can be modified only when the CHOP is a scriptCHOP. This value updates independently of time dependent content.

`isTimeSlice` → `bool` :

Get or set the last cooked [Time Slice](../Glossary/Time_Slicing.md "Time Slicing") value. True if the CHOP last cooked as a Time Slice. This can be modified only when the CHOP is a scriptCHOP
